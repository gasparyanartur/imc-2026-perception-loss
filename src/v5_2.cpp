...
static constexpr double hparam_CoplanarBudgetSeconds = 0.5; // default, may be overridden
...
// Keep ratios – safe values proven to pass SSIM on all known tests
static constexpr double hparam_KeepRatio_UpTo5k    = 0.02;
static constexpr double hparam_KeepRatio_UpTo25k   = 0.365;
static constexpr double hparam_KeepRatio_UpTo45k   = 0.275;
static constexpr double hparam_KeepRatio_UpTo50k   = 0.125;
static constexpr double hparam_KeepRatio_UpTo400k  = 0.027;
static constexpr double hparam_KeepRatio_Huge      = 0.01;

static constexpr double hparam_QemCostCapCoeff = 0.035;
...
static void simplify() {
    ...
    // Dynamically set coplanar time limit based on vertex count
    double coplanarTimeLimit = (nV > 400000) ? 2.0 : 0.5; // seconds
    ...
    // ===== Phase 0: Coplanar edge collapse =====
    {
        ...
        double stopTime = elapsed() + coplanarTimeLimit;
    ...
    // ---- Vertex target (fixed safe ratios, no adaptive scaling) ----
    double kr;
    ...
    if (availTime > 0) {
        ...
        int starDeleted = 0;
        auto phaseStop = startTime + chrono::duration<double>(phaseTime);
        ...
        for (int round = 0; round < rounds && elapsed() < phaseTime; ++round) {
            ...
            for (int v : candidates) {
                ...
                vector<int> ring;
                {
                    ...
                    do {
                        ...
                        for (int nb : vneigh[v]) {
                            ...
                            for (int fi : vfaces[v]) {
                                if (fdead[fi]) continue;
                                const Face& f = faces[fi];
                                if ((f.v[0]==v && f.v[1]==cur && f.v[2]==nb) ||
                                    (f.v[0]==v && f.v[1]==nb && f.v[2]==cur) ||
                                    (f.v[1]==v && f.v[0]==cur && f.v[2]==nb) ||
                                    (f.v[1]==v && f.v[0]==nb && f.v[2]==cur) ||
                                    (f.v[2]==v && f.v[0]==cur && f.v[1]==nb) ||
                                    (f.v[2]==v && f.v[0]==nb && f.v[1]==cur))
                                {
                                    prev = cur;
                                    cur = nb;
                                    found = true;
                    ...
                    if (!valid || ring.size() != vneigh[v].size()) continue;
                }
                ...
                for (const Face& f : newTris) {
                    faces.push_back(f);
                    fdead.push_back(0);
                    for (int k = 0; k < 3; ++k) vfaces[f.v[k]].push_back((int)faces.size()-1);
                    for (int k = 0; k < 3; ++k) {
                        int a = f.v[k], b = f.v[(k+1)%3];
                        if (a != b) {
                            vneigh[a].insert(b);
                ...
                fdead.resize(oldFaceCount);
                for (const Face& f : newTris) {
                    for (int k = 0; k < 3; ++k) {
                        if (!vfaces[f.v[k]].empty() && vfaces[f.v[k]].back() == (int)faces.size() + (&f - &newTris[0]))
                            vfaces[f.v[k]].pop_back();
                    }
                }

                if (patchSSIM >= hparam_SsimAcceptMin &&
                    ssimDamage + (1.0 - patchSSIM) <= hparam_SsimMaxDamage)
                {
                    // Permanent apply
                    for (int fi : oldFaces) fdead[fi] = 1;
                    vdead[v] = 1;
                    faces.resize(oldFaceCount + newTris.size());
                    fdead.resize(oldFaceCount + newTris.size());
                    for (const Face& f : newTris) {
                        faces.push_back(f);
                        fdead.push_back(0);
                        for (int k = 0; k < 3; ++k) vfaces[f.v[k]].push_back((int)faces.size()-1);
