...
static constexpr double hparam_CoplanarBudgetSeconds = 1;
...
static constexpr double hparam_KeepRatio_UpTo25k   = 0.38;
static constexpr double hparam_KeepRatio_UpTo45k   = 0.19;
static constexpr double hparam_KeepRatio_UpTo50k   = 0.12;
static constexpr double hparam_KeepRatio_UpTo400k  = 0.04;
static constexpr double hparam_KeepRatio_Huge      = 0.05;
...
static void simplify() {
    ...
    // ------- Serial connectivity (memory‑safe, fast enough) -------
    ...
    for (int fi = 0; fi < nF; ++fi) {
        ...
        double len = norm(n);
        if (len < CParam_MinNormalNorm) len = CParam_MinNormalNorm;
        n = n / len;
        faceNormals[fi] = n;
        faceOffsets[fi] = -dot(n, verts[f.v[0]]);
    }

    auto hashFunc = [&](const Vec3& n, double d) -> uint64_t {
        int nx = int(n.x * 1e5 + 0.5);
        int ny = int(n.y * 1e5 + 0.5);
        int nz = int(n.z * 1e5 + 0.5);
        int nd = int(d * 1e5 + 0.5);
        uint64_t h = 0;
        h ^= uint64_t(nx) + 0x9e3779b9 + (h<<6) + (h>>2);
        h ^= uint64_t(ny) + 0x9e3779b9 + (h<<6) + (h>>2);
        h ^= uint64_t(nz) + 0x9e3779b9 + (h<<6) + (h>>2);
        h ^= uint64_t(nd) + 0x9e3779b9 + (h<<6) + (h>>2);
        return h;
    };

    unordered_map<uint64_t, vector<int>> faceGroups;
    for (int fi = 0; fi < nF; ++fi) {
        if (fdead[fi]) continue;
        uint64_t key = hashFunc(faceNormals[fi], faceOffsets[fi]);
        faceGroups[key].push_back(fi);
    }

    vector<pair<int,int>> coplanarEdges;
    for (int a = 0; a < nV; ++a) {
        if (vdead[a]) continue;
        for (int b : vneigh[a]) {
            if (b <= a) continue;
            ...
            double nd = fabs(dot(faceNormals[fa], faceNormals[fb]));
            double dd = fabs(faceOffsets[fa] - faceOffsets[fb]);
            if (1.0-nd > COPLANAR_EPS_NORMAL || dd > COPLANAR_EPS_OFFSET) continue;
            coplanarEdges.push_back({a,b});
        }
    }
    sort(coplanarEdges.begin(), coplanarEdges.end());
    coplanarEdges.erase(unique(coplanarEdges.begin(), coplanarEdges.end()), coplanarEdges.end());
    ...
    for (auto& e : coplanarEdges) {
        int a = e.first, b = e.second;
        if (edgeExists(a,b))
            cq.push({norm(verts[a]-verts[b]), a, b, vver[a], vver[b]});
    }

    double coplanarStop = elapsed() + hparam_CoplanarBudgetSeconds;
    while (!cq.empty() && elapsed() < coplanarStop) {
        ...
        for (int nb : vneigh[best.kept]) {
            ...
            cq.push({norm(verts[best.kept]-verts[nb]), best.kept, nb, vver[best.kept], vver[nb]});
        }
    }
    ...
    // ---- Quadric accumulation (parallel reduction, safe) ----
