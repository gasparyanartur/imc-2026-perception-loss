//lynx v5 76pt with T3 Wrong Answer: SSIM is too low
#include "Eigen/Dense"
...
static constexpr double hparam_KeepRatio_UpTo25k   = 0.4;
    ...
    if (availTime > 0) {
        ...
        for (int round = 0; round < rounds && elapsed() < phaseTime; ++round) {
            ...
            for (int v : candidates) {
                ...
                // Render current and compute SSIM
                ...
                for (const Face& f : newTris) {
                    for (int k = 0; k < 3; ++k) {
                        // remove the appended face index from vfaces (last element)
                        if (!vfaces[f.v[k]].empty() && vfaces[f.v[k]].back() == (int)faces.size() + (&f - &newTris[0]))
                            vfaces[f.v[k]].pop_back();
                    }
                }
                // Remove added neighborhood edges (imperfect, but acceptable)
                // We'll just continue; if candidate rejected, the extra edges remain but won't be used.

                // Accept or permanently reject
