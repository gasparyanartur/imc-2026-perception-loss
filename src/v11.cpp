...
struct Quadric {
    ...
    static Quadric fromTriangle(const Vec3& p0, const Vec3& p1, const Vec3& p2, double w = 1.0) {
        ...
        q.scale(w * area * 0.5);  // Garland & Heckbert weighting
        return q;
    }
...
// ============================================================================
// Small ordered set for adjacency (with clear())
// ============================================================================
struct SmallSet {
    vector<int> data;
    void insert(int v) {
...
// ============================================================================
// Parameters (conservative, SSIM‑safe)
// ============================================================================
...
static constexpr double TotalQEMTime  = 19.0;
static constexpr double CoplanarNormalEps = 1e-5;
static constexpr double CoplanarOffsetEps = 1e-5;

// Conservative keep ratios – higher than before to protect SSIM
static double targetKeepRatio(int nV) {
    if (nV <= 5000)   return 0.50;   // keep half the vertices
    if (nV <= 25000)  return 0.60;
    if (nV <= 50000)  return 0.70;
    if (nV <= 400000) return 0.75;
    return 0.80;
}
...
static void simplify() {
    ...
    vector<Vec3> fnorm(nF);
    vector<double> farea(nF);
    for (int fi = 0; fi < nF; ++fi) {
        Vec3 n = cross(verts[faces[fi].v[1]] - verts[faces[fi].v[0]],
                       verts[faces[fi].v[2]] - verts[faces[fi].v[0]]);
        double len = norm(n);
        farea[fi] = 0.5 * len;
        fnorm[fi] = len > 1e-30 ? n / len : Vec3(0, 0, 0);
    }

    // View‑dependent face weights
    vector<double> faceWeight(nF, 1.0);
    #pragma omp parallel for schedule(static)
    for (int fi = 0; fi < nF; ++fi) {
        const Vec3& n = fnorm[fi];
        double area = farea[fi];
    ...
    // Accumulate quadrics (parallel reduction)
    ...
    // Collapse candidate and cost
    struct Collapse {
        int a, b;           // edge endpoints (a will be absorbed, b kept)
        int verA, verB;
        double cost;
        Vec3 newPos;
    };
    auto computeCollapse = [&](int a, int b) -> Collapse {
        ...
        for (const Vec3& p : cands) {
            ...
            // Enforce Hausdorff bound (forward direction only, but safe enough)
            ...
            // Try both absorption directions, take cheapest valid
    ...
    // Apply a collapse
    auto applyCollapse = [&](int absorbed, int kept, const Vec3& newPos) {
        verts[kept] = newPos;
        vradius[kept] = max(vradius[absorbed] + norm(verts[absorbed] - newPos), vradius[kept]);
        vdead[absorbed] = 1;
        ...
        for (int fi : af) {
            ...
            bool touched = false;
            for (int k = 0; k < 3; ++k)
                if (f.v[k] == absorbed) { f.v[k] = kept; touched = true; }
            if (!touched) continue;
    ...
    // ========================================================================
    {
        ...
        for (int a = 0; a < nV; ++a) {
            ...
            for (int b : vneigh[a].get()) {
                ...
                for (int fi : vfaces[a]) {
                    ...
                    if (cnt < 2) { norms[cnt] = n; offsets[cnt] = d; ++cnt; }
                }
                if (cnt == 2 &&
        ...
        while (!cq.empty() && elapsed() < CoplanarTime) {
            ...
            auto best = computeCollapse(a, b);
            if (best.a < 0 || best.cost > costCap) continue;
            applyCollapse(best.a, best.b, best.newPos);
            for (int nb : vneigh[best.b].get()) {
                if (vdead[nb] || nb == best.b) continue;
                auto nc = computeCollapse(best.b, nb);
                if (nc.a >= 0) cq.push({nc.cost, best.b, nb, nc.verA, nc.verB});
            }
        }
    }

    // ========================================================================
    // Phase 1: QEM edge collapse (conservative keep ratio)
    // ========================================================================
    ...
    int targetVerts = max(10, (int)(nV * targetKeepRatio(nV)));
    ...
    while (accepted < collapseLimit && !pq.empty() && elapsedQEM() < TotalQEMTime) {
        ...
        if (c.verA != vver[a] || c.verB != vver[b]) {
            auto nc = computeCollapse(a, b);
            if (nc.a >= 0) pq.push(nc);
            continue;
        }
