...
/*
IMC2026 Problem B — simplifygeometry_v37_collapse_relax.cpp

V25 tail batching plus conservative collapse-relax-collapse vertex relocation.
...
static constexpr double HParam_RelaxAlpha = 0.50;
static constexpr double HParam_RelaxSlackFrac = 0.75;
...
private:
    ...
    bool localFacesAllowMove(int v, const Vec3& newPos) const {
        for (int fi : vertexFaces[v]) {
            if (faceDead[fi]) continue;
            int ids[3] = {faces[fi].v[0], faces[fi].v[1], faces[fi].v[2]};
            Vec3 oldP[3] = {vertices[ids[0]], vertices[ids[1]], vertices[ids[2]]};
            Vec3 newP[3] = {oldP[0], oldP[1], oldP[2]};
            for (int k = 0; k < 3; ++k) {
                if (ids[k] == v) newP[k] = newPos;
            }
            Vec3 oldN = cross(oldP[1] - oldP[0], oldP[2] - oldP[0]);
            Vec3 newN = cross(newP[1] - newP[0], newP[2] - newP[0]);
            if (norm2(newN) < 1e-24) return false;
            if (norm2(oldN) > 1e-24 && dot(oldN, newN) <= 0.0) return false;
        }
        return true;
    }

    bool relaxVertex(int v) {
        if (v < 0 || v >= (int)vertices.size() || vertexDead[v]) return false;
        Vec3 optimum;
        if (!solveQem3x3(vertexQuadrics[v], optimum)) return false;
        if (!finiteVec(optimum)) return false;

        Vec3 current = vertices[v];
        Vec3 delta = (optimum - current) * HParam_RelaxAlpha;
        double moveLen = norm(delta);
        if (moveLen <= 1e-14) return false;

        double slack = HParam_RelaxSlackFrac * max(0.0, hausdorffLimit - clusterRadius[v]);
        if (slack <= 1e-14) return false;
        if (moveLen > slack) {
            delta = delta * (slack / moveLen);
            moveLen = slack;
        }

        Vec3 candidate = current + delta;
        if (!finiteVec(candidate)) return false;
        if (clusterRadius[v] + moveLen > hausdorffLimit) return false;
        double oldCost = vertexQuadrics[v].evaluate(current);
        double newCost = vertexQuadrics[v].evaluate(candidate);
        if (!(newCost + 1e-18 < oldCost)) return false;
        if (!localFacesAllowMove(v, candidate)) return false;

        vertices[v] = candidate;
        clusterRadius[v] += moveLen;
        ++vertexVersion[v];
    ...
    void relaxTouchedVertices(const vector<int>& touched) {
        for (int v : touched) relaxVertex(v);
    }
    ...
    int runTailBatch() {
        ...
        vector<int> touchedKept;
        touchedKept.reserve(selected.size());
        for (const EdgeBatchCandidate& e : selected) {
            ...
            touchedKept.push_back(best.kept);
            ++acceptedCollapses;
            ++acceptedInBatch;
        }
        if (acceptedInBatch > 0) relaxTouchedVertices(touchedKept);
