...
static constexpr double hparam_RootNudgeFrac = 0.08;
static constexpr double hparam_RootNudgeMaxDiagFrac = 0.0018;
static constexpr double hparam_RootNudgeRadiusFrac = 0.94;
...
public:
    void run(){
        ...
        runDragonfruitPostSchedule();

        if (lensReady && elapsed() < hparam_TotalBudgetSeconds - hparam_LensPassMinTimeSeconds) {
            stageTimeBudget = hparam_TotalBudgetSeconds;
            lensOriginalStarDeletePass();
        }
...
private:
    ...
    vector<vector<SsimPixel>> lensOrig;
    ...
    int runStarCandidates(const StarDeleteParams& sp, vector<StarCandidate> cands,int maxExtra,int& extra,double stopTime){
    ...
    void generalStarDeletePostPass(){
        ...
        for(int round=0;round<sp.rounds&&extra<maxExtra&&elapsed()<stopTime;++round){
            ...
            int gain=runStarCandidates(sp,cands,maxExtra,extra,stopTime);
            bool progress=gain>0;
    ...
    struct SsimDeleteCandidate{
        int v=-1,root=0;
        double ssim=1.0,score=CParam_Inf,patchWeight=0.0;
    ...
    bool vertexMoveKeepsIncidentFaces(int v,const Vec3& np)const{
        if(v<0||v>=(int)vfaces.size()||vdead[v])return false;
        for(int fi:vfaces[v]){
            ...
            for(int k=0;k<3;++k)if(f.v[k]==v)newP[k]=np;
            Vec3 oldN=cross(oldP[1]-oldP[0],oldP[2]-oldP[0]);
            Vec3 newN=cross(newP[1]-newP[0],newP[2]-newP[0]);
            double oldL=norm(oldN),newL=norm(newN);
            if(oldL<CParam_MinNormalNorm||newL<CParam_MinNormalNorm)return false;
            if(dot(oldN,newN)<=0.20*oldL*newL)return false;
        }
        return true;
    }

    bool tryNudgeVertexToward(int v,const Vec3& target){
        if(v<0||v>=(int)verts.size()||vdead[v])return false;
        Vec3 delta=(target-verts[v])*hparam_RootNudgeFrac;
        double dl=norm(delta);
        if(dl<1e-14)return false;
        double maxStep=diag*hparam_RootNudgeMaxDiagFrac;
        if(dl>maxStep&&maxStep>0.0){delta=delta*(maxStep/dl);dl=maxStep;}
        if(crad[v]+dl>hausd*hparam_RootNudgeRadiusFrac)return false;
        Vec3 np=verts[v]+delta;
        if(!finiteVec(np))return false;
        if(!vertexMoveKeepsIncidentFaces(v,np))return false;
        verts[v]=np;
        crad[v]+=dl;
        ++vver[v];
        return true;
    ...
    bool applyStarDeleteExactWithParams(int v,int root,const StarDeleteParams& sp){
        ...
        tryNudgeVertexToward(rr[0],verts[v]);
        return true;
    }
    ...
    void runDragonfruitPostSchedule(){
        stageTimeBudget=hparam_GeneralStarPostBudgetSeconds;
        int postTier=originalTier();
        if(elapsed()<stageTimeBudget-1.2)generalStarDeletePostPass();
        if(postTier!=2&&postTier!=3&&elapsed()<stageTimeBudget-0.8)generalStarDeletePostPass();
    ...
    bool lensEnabledForCurrentTier() const{
        return hparam_EnableLensOriginalPass;
    }

    bool renderLensOriginalView(int view,vector<SsimPixel>& buf)const{
        const int R=hparam_LensResolution;
        SsimPixel bg=ssimBackgroundPixel();
        buf.assign(R*R,bg);
        ...
        for(const Face& f:faces){
            ...
            SsimProj p0=ssimProjectPoint(verts[ia],view,R);
            SsimProj p1=ssimProjectPoint(verts[ib],view,R);
            SsimProj p2=ssimProjectPoint(verts[ic],view,R);
            if(!p0.ok||!p1.ok||!p2.ok)continue;
            double area2=(p1.u-p0.u)*(p2.v-p0.v)-(p1.v-p0.v)*(p2.u-p0.u);
            if(fabs(area2)<1e-12)continue;
            Vec3 nr=cross(verts[ib]-verts[ia],verts[ic]-verts[ia]);
            double nl=norm(nr);
            if(nl<CParam_MinNormalNorm)continue;
            nr=nr/nl;
    ...
    void buildLensOriginalCache(){
        lensReady=false;
        lensOrig.clear();
        if(!lensEnabledForCurrentTier())return;
        if(elapsed()>hparam_LensBuildMaxSeconds*0.65)return;
        lensOrig.resize(6);
        for(int view=0;view<6;++view){
            if(elapsed()>hparam_LensBuildMaxSeconds){
                lensOrig.clear();
                return;
            }
            renderLensOriginalView(view,lensOrig[view]);
        }
        lensReady=true;
    }

    bool renderLensPatch(const vector<SsimTri>& tris,int view,int x0,int y0,int w,int h,vector<SsimPixel>& buf)const{
        SsimPixel bg=ssimBackgroundPixel();
        buf.assign(w*h,bg);
        vector<float> zbuf(w*h,numeric_limits<float>::infinity());
        const int R=hparam_LensResolution;
    ...
    bool lensOriginalScoresForStarCandidate(int v,int root,const StarDeleteParams& sp,double& currentSsim,double& originalNewSsim,double* patchWeight=nullptr)const{
        currentSsim=1.0;
        originalNewSsim=1.0;
        if(!lensReady||lensOrig.size()!=6)return false;
        vector<SsimTri> oldTris,newTris;
        if(!buildStarPatchTris(v,root,sp,oldTris,newTris))return false;
        const int R=hparam_LensResolution;
        double curTotal=0.0,origTotal=0.0;
        int usedViews=0;
        int totalMaskCount=0;
        vector<SsimPixel> oldBuf,newBuf,origBuf;
        SsimPixel bg=ssimBackgroundPixel();
        for(int view=0;view<6;++view){
            double mnU=1e100,mnV=1e100,mxU=-1e100,mxV=-1e100;
            bool any=false;
            ...
            for(const SsimTri& t:newTris)includeTri(t);
            if(!any)continue;
            int pad=hparam_LensPatchPaddingPixels;
            int x0=max(0,(int)floor(mnU)-pad);
            int y0=max(0,(int)floor(mnV)-pad);
            int x1=min(R-1,(int)ceil(mxU)+pad);
            int y1=min(R-1,(int)ceil(mxV)+pad);
            if(x0>x1||y0>y1)continue;
            int w=x1-x0+1,h=y1-y0+1;
            if(w*h>hparam_LensPatchMaxPixels)return false;
            renderLensPatch(oldTris,view,x0,y0,w,h,oldBuf);
            renderLensPatch(newTris,view,x0,y0,w,h,newBuf);
            origBuf.assign(w*h,bg);
            int maskCount=0;
            for(int py=0;py<h;++py){
                for(int px=0;px<w;++px){
                    int idx=py*w+px;
                    if(oldBuf[idx].fg||newBuf[idx].fg){
                        origBuf[idx]=lensOrig[view][(y0+py)*R+(x0+px)];
                        ++maskCount;
                    }
                }
            }
            if(maskCount<4)continue;
            totalMaskCount+=maskCount;
            curTotal+=ssimBuffers(oldBuf,newBuf);
            origTotal+=ssimBuffers(origBuf,newBuf);
            ++usedViews;
        }
        if(usedViews==0)return false;
        currentSsim=curTotal/double(usedViews);
        originalNewSsim=origTotal/double(usedViews);
        if(patchWeight)*patchWeight=double(totalMaskCount)/double(6*R*R);
        return true;
    }

    void lensOriginalStarDeletePass(){
        if(!lensReady)return;
        StarDeleteParams sp=ssimStarParams();
        int lensTier=originalTier();
        double lensOrigMin=hparam_LensOrigAcceptMin;
        double lensCurrentMin=hparam_LensCurrentAcceptMin;
        double lensMaxSeconds=hparam_LensPassMaxSeconds;
        double lensExtraScale=hparam_LensExtraScale;
        double lensHardCapScale=hparam_LensHardCapScale;
        if(lensTier==3||lensTier==4||lensTier==5||lensTier==6){
            lensOrigMin=hparam_LensOrigAcceptByTier[lensTier];
            lensCurrentMin=hparam_LensCurrentAcceptByTier[lensTier];
            lensMaxSeconds=hparam_LensMaxSecondsByTier[lensTier];
            lensExtraScale=hparam_LensExtraScaleByTier[lensTier];
            lensHardCapScale=hparam_LensHardCapScaleByTier[lensTier];
        }
        double timeLeft=stageTimeBudget-elapsed();
        if(timeLeft<hparam_LensPassMinTimeSeconds)return;
        double stopTime=elapsed()+min(lensMaxSeconds,timeLeft*hparam_LensPassTimeFrac);
        int maxExtra=min(max(0,(int)floor(sp.hardCap*lensHardCapScale)),
                         max(0,(int)floor(nV*sp.extraFrac*lensExtraScale)));
        if(maxExtra<=0)return;
        vector<SsimDeleteCandidate> cands;
        cands.reserve(min(hparam_LensCandidatePoolCap,maxExtra*8+128));
        int scanned=0;
        for(int v=0;v<(int)verts.size()&&scanned<sp.scanVertices&&elapsed()<stopTime;++v){
            if(vdead[v])continue;
            ++scanned;
            StarCandidate geom=computeStarCandidateWithParams(v,sp);
            if(!geom.valid())continue;
            double curS=1.0,origS=1.0,patchW=0.0;
            if(!lensOriginalScoresForStarCandidate(geom.v,geom.root,sp,curS,origS,&patchW))continue;
            if(curS<lensCurrentMin||origS<lensOrigMin)continue;
            SsimDeleteCandidate dc;
            dc.v=geom.v;
            dc.root=geom.root;
            dc.ssim=origS;
            dc.patchWeight=patchW;
            double impact=max(1e-7,patchW);
            dc.score=impact*((1.0-origS)+0.25*(1.0-curS))+hparam_LensScoreGeomWeight*geom.score;
            cands.push_back(dc);
            if((int)cands.size()>=hparam_LensCandidatePoolCap)break;
        }
        if(cands.empty())return;
        sort(cands.begin(),cands.end());
        int extra=0;
        for(const SsimDeleteCandidate& c:cands){
            ...
            if(c.v<0||c.v>=(int)vdead.size()||vdead[c.v])continue;
            double curS=1.0,origS=1.0,patchW=0.0;
            if(!lensOriginalScoresForStarCandidate(c.v,c.root,sp,curS,origS,&patchW))continue;
            if(curS<lensCurrentMin||origS<lensOrigMin)continue;
            if(applyStarDeleteExactWithParams(c.v,c.root,sp)){
                ++accepted;
                ++extra;
