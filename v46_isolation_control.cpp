...
static volatile int XCalTier=0,XCalRank=0;
static volatile double XCalExtra=0.000;
...
if(nV>5000&&nV<=50000){int ct=XCalTier;if(ct==3&&nV<=25000){runCalibrationTier3(XCalExtra,XCalRank);writeMesh();return;}if(ct==4&&nV>25000&&nV<=45000){runCalibrationTier4(XCalExtra,XCalRank);writeMesh();return;}if(nV>25000&&nV<=45000){runScreenCoreMid();compact();writeMesh();return;}if(nV<=25000)runTransactionalScreenT2Atomic();else runTransactionalScreenMid();writeMesh();return;}
...
if(hugeInputMode){compact();runHugeAtomicRegionTxPlain();writeMesh();return;}
...
__attribute__((noinline,cold)) void runTransactionalScreenT2Atomic(){
    int originalN=nV,tier=2;
    runScreenCoreMid();
    MidSnap safe=midSnapshot();
    vector<Vec3>safeV,candV;vector<Face>safeF,candF;
    extractActiveMesh(safeV,safeF);
    ...
    if(!kept){restoreMid(safe);compact();return;}

    // The baseline v23 candidate is already committed.  The independent region
    // transaction is optional and can only replace it after the same 1024 guard.
    if(elapsed()<HParam_TimeBudgetSeconds-4.05){
        vector<Vec3>manyV,oneV;vector<Face>manyF,oneF;
        if(runMacroRegionAlternatives(verts,faces,manyV,manyF,oneV,oneF)){
            bool committed=false;
            if(manyV.size()<verts.size()&&elapsed()<HParam_TimeBudgetSeconds-1.30){
                RefScore ms=compareToReference(ref,manyV,manyF,R);
                if(txGuard(ms,tier)){
                    verts.swap(manyV);faces.swap(manyF);nV=(int)verts.size();nF=(int)faces.size();committed=true;
                }
            }
            if(!committed&&oneV.size()<verts.size()&&elapsed()<HParam_TimeBudgetSeconds-0.35){
                RefScore os=compareToReference(ref,oneV,oneF,R);
                if(txGuard(os,tier)){
                    verts.swap(oneV);faces.swap(oneF);nV=(int)verts.size();nF=(int)faces.size();
                }
            }
        }
    }
...
void continueCalibration(int originalN,int baseActive,double extra,int internalTier,int ranked){
    screenTier=(ranked&&internalTier==3)?4:internalTier;
    if(screenTier==3)buildRasterImportanceTier3(512);else buildRasterImportance(screenTier==2?1024:768);
    initFaceWeights();priority_queue<CollapseCandidate>e;pq.swap(e);
    targetV=max(10,(int)floor(baseActive*(1.0-extra)));collapseLimit=originalN-targetV;initQueue();collapseLoop();compact();
}
void runCalibrationTier3(double extra,int ranked){
    int originalN=nV,tier=2;runScreenCoreMid();MidSnap safe=midSnapshot();vector<Vec3>sv,cv;vector<Face>sf,cf;extractActiveMesh(sv,sf);
    if(elapsed()>HParam_TimeBudgetSeconds-4.0){compact();return;}int R=1024;vector<RefImage>ref(6);for(int v=0;v<6;++v)ref[v]=renderReference(sv,sf,v,R);
    bool kept=false;for(double ratio:vector<double>{.27,.28,.285,.29,.295}){if(elapsed()>HParam_TimeBudgetSeconds-1.8)break;restoreMid(safe);continueScreenTo(tier,originalN,ratio,0);extractActiveMesh(cv,cf);if(txGuard(compareToReference(ref,cv,cf,R),tier)){kept=true;break;}}
    if(!kept){restoreMid(safe);}int base=originalN-accepted;if(extra>0&&elapsed()<HParam_TimeBudgetSeconds-.25)continueCalibration(originalN,base,extra,2,ranked);else compact();
}
void runCalibrationTier4(double extra,int ranked){
    int originalN=nV;runScreenCoreMid();int base=originalN-accepted;if(extra>0&&elapsed()<HParam_TimeBudgetSeconds-.25)continueCalibration(originalN,base,extra,3,ranked);else compact();
}

// ---------- v33: atomic multi-vertex region coarsening (T2) ----------
// Replace a whole disk-like two-ring patch by one fitted surface vertex.  The
// operation skips the sequence of individually bad collapses that can trap QEM.
struct MacroRegionCandidate{
    vector<int> patchFaces,boundary,interior;
    Vec3 center;
    int gain=0;
    double score=CParam_Inf,geom=0.0,normalDamage=0.0;
    bool valid()const{return gain>=2&&boundary.size()>=4&&patchFaces.size()>=6&&score<CParam_Inf;}
    bool operator<(const MacroRegionCandidate&o)const{
        if(score!=o.score)return score<o.score;
        return gain>o.gain;
    }
};

static Vec3 macroClosestPointTri(const Vec3&p,const Vec3&a,const Vec3&b,const Vec3&c){
    Vec3 ab=b-a,ac=c-a,ap=p-a;double d1=dot(ab,ap),d2=dot(ac,ap);
    ...
    double den=va+vb+vc;if(fabs(den)<1e-30)return a;double inv=1.0/den,v=vb*inv,w=vc*inv;return a+ab*v+ac*w;
}

static double macroOrient2(double ax,double ay,double bx,double by,double cx,double cy){
    return (bx-ax)*(cy-ay)-(by-ay)*(cx-ax);
}
static bool macroSegCross(double ax,double ay,double bx,double by,double cx,double cy,double dx,double dy){
    auto sgn=[](double x){return (x>1e-12)-(x<-1e-12);};
    double a=macroOrient2(ax,ay,bx,by,cx,cy),b=macroOrient2(ax,ay,bx,by,dx,dy);
    double c=macroOrient2(cx,cy,dx,dy,ax,ay),d=macroOrient2(cx,cy,dx,dy,bx,by);
    return sgn(a)*sgn(b)<0&&sgn(c)*sgn(d)<0;
}

__attribute__((noinline,cold)) bool macroClosedOriented(const vector<Vec3>&vv,const vector<Face>&ff)const{
    if(vv.empty()||ff.empty())return false;
    struct EC{int count=0,dir=0;};unordered_map<unsigned long long,EC> em;em.reserve(ff.size()*4);
    vector<vector<pair<int,int>>> links(vv.size());
    auto key=[](int a,int b){if(a>b)swap(a,b);return (unsigned long long)(unsigned int)a<<32|(unsigned int)b;};
    for(const Face&f:ff){int a=f.v[0],b=f.v[1],c=f.v[2];if(a<0||b<0||c<0||a>=(int)vv.size()||b>=(int)vv.size()||c>=(int)vv.size()||a==b||b==c||a==c)return false;
        if(norm2(cross(vv[b]-vv[a],vv[c]-vv[a]))<1e-24*max(1.0,diag*diag*diag*diag))return false;
        int x[3]={a,b,c};for(int k=0;k<3;++k){int u=x[k],v=x[(k+1)%3],lo=min(u,v);auto &e=em[key(u,v)];e.count++;e.dir+=(u==lo?1:-1);}links[a].push_back({b,c});links[b].push_back({c,a});links[c].push_back({a,b});
    }
    for(auto&kv:em)if(kv.second.count!=2||kv.second.dir!=0)return false;
    // A closed edge-manifold can still pinch at a vertex.  Require one circular link.
    for(int v=0;v<(int)links.size();++v){if(links[v].empty())continue;unordered_map<int,vector<int>> g;g.reserve(links[v].size()*2);for(auto p:links[v]){g[p.first].push_back(p.second);g[p.second].push_back(p.first);}for(auto&kv:g)if(kv.second.size()!=2)return false;unordered_set<int>seen;vector<int>st{g.begin()->first};while(!st.empty()){int x=st.back();st.pop_back();if(!seen.insert(x).second)continue;for(int y:g[x])st.push_back(y);}if(seen.size()!=g.size())return false;}
    return true;
}

__attribute__((noinline,cold)) void macroBuildTopology(const vector<Face>&ff,int nv,vector<vector<int>>&vf,vector<vector<int>>&fa)const{
    vf.assign(nv,{});fa.assign(ff.size(),{});
    unordered_map<unsigned long long,pair<int,int>> first;first.reserve(ff.size()*4);
    auto key=[](int a,int b){if(a>b)swap(a,b);return (unsigned long long)(unsigned int)a<<32|(unsigned int)b;};
    for(int fi=0;fi<(int)ff.size();++fi){const Face&f=ff[fi];for(int k=0;k<3;++k)vf[f.v[k]].push_back(fi);for(int k=0;k<3;++k){int a=f.v[k],b=f.v[(k+1)%3];auto K=key(a,b);auto it=first.find(K);if(it==first.end())first[K]={fi,k};else{int fj=it->second.first;fa[fi].push_back(fj);fa[fj].push_back(fi);}}}
}

__attribute__((noinline,cold)) bool macroBoundaryCycle(const vector<Face>&ff,const vector<int>&patch,vector<int>&ring,vector<int>&inside)const{
    ring.clear();inside.clear();unordered_map<unsigned long long,pair<int,pair<int,int>>> e; e.reserve(patch.size()*4);
    unordered_set<int>pv;pv.reserve(patch.size()*2);
    auto key=[](int a,int b){if(a>b)swap(a,b);return (unsigned long long)(unsigned int)a<<32|(unsigned int)b;};
    for(int fi:patch){const Face&f=ff[fi];for(int k=0;k<3;++k)pv.insert(f.v[k]);for(int k=0;k<3;++k){int a=f.v[k],b=f.v[(k+1)%3];auto K=key(a,b);auto it=e.find(K);if(it==e.end())e[K]={1,{a,b}};else it->second.first++;}}
    unordered_map<int,int>nxt,indeg;unordered_set<int>bv;
    for(auto&kv:e)if(kv.second.first==1){int a=kv.second.second.first,b=kv.second.second.second;if(nxt.count(a)||indeg[b])return false;nxt[a]=b;indeg[b]=1;bv.insert(a);bv.insert(b);}else if(kv.second.first!=2)return false;
    if(bv.size()<4||bv.size()>24||nxt.size()!=bv.size())return false;
    int start=*bv.begin(),cur=start;bool closed=false;for(size_t s=0;s<bv.size();++s){if(!nxt.count(cur))return false;ring.push_back(cur);cur=nxt[cur];if(cur==start){closed=(s+1==bv.size());break;}}
    if(!closed||cur!=start)return false;
    for(int v:pv)if(!bv.count(v))inside.push_back(v);
    // Disk Euler characteristic.
    int V=(int)pv.size(),E=(int)e.size(),F=(int)patch.size();if(V-E+F!=1)return false;
    return true;
}

__attribute__((noinline,cold)) double macroTriSetDistance2(const vector<array<Vec3,3>>&A,const vector<array<Vec3,3>>&B)const{
    if(A.empty()||B.empty())return CParam_Inf;double worst=0;
    for(const auto&t:A){Vec3 s[13]={t[0],t[1],t[2],(t[0]+t[1])*.5,(t[1]+t[2])*.5,(t[2]+t[0])*.5,(t[0]+t[1]+t[2])/3.0,(t[0]*2+t[1]+t[2])/4.0,(t[1]*2+t[2]+t[0])/4.0,(t[2]*2+t[0]+t[1])/4.0,(t[0]*3+t[1])/4.0,(t[1]*3+t[2])/4.0,(t[2]*3+t[0])/4.0};for(Vec3&p:s){double d=CParam_Inf;for(const auto&q:B)d=min(d,pointTriangleDistance2(p,q[0],q[1],q[2]));worst=max(worst,d);}}
    return worst;
}

__attribute__((noinline,cold)) bool macroCandidate(const vector<Vec3>&vv,const vector<Face>&ff,const vector<vector<int>>&vf,const vector<vector<int>>&fa,int seed,int depth,MacroRegionCandidate&out)const{
    out=MacroRegionCandidate();if(seed<0||seed>=(int)vf.size()||vf[seed].size()<4||vf[seed].size()>12)return false;
    vector<char>inF(ff.size(),0);vector<int>front=vf[seed],patch;for(int f:front)if(!inF[f]){inF[f]=1;patch.push_back(f);}for(int d=0;d<depth;++d){vector<int>nf;for(int f:front)for(int g:fa[f])if(!inF[g]){inF[g]=1;patch.push_back(g);nf.push_back(g);}front.swap(nf);if(front.empty())break;}
    if(patch.size()<10||patch.size()>72)return false;
    vector<int>ring,inside;if(!macroBoundaryCycle(ff,patch,ring,inside))return false;if(inside.size()<3||inside.size()>28)return false;
    vector<char>patchFlag(ff.size(),0);for(int fi:patch)patchFlag[fi]=1;for(int v:inside)for(int fi:vf[v])if(!patchFlag[fi])return false;
    bool seedInside=find(inside.begin(),inside.end(),seed)!=inside.end();if(!seedInside)return false;
    Vec3 avgN,cent;double area=0,oldArea=0;vector<Vec3>oldN;vector<Vec3>oldC;vector<array<Vec3,3>>oldT;oldT.reserve(patch.size());Quadric qsum;
    for(int fi:patch){const Face&f=ff[fi];Vec3 a=vv[f.v[0]],b=vv[f.v[1]],c=vv[f.v[2]],n=cross(b-a,c-a);double nl=norm(n);if(nl<CParam_MinNormalNorm)return false;avgN=avgN+n;cent=cent+(a+b+c)*(nl/3.0);area+=nl;oldArea+=0.5*nl;oldN.push_back(n/nl);oldC.push_back((a+b+c)/3.0);oldT.push_back({a,b,c});qsum+=Quadric::fromTriangle(a,b,c);}
    double al=norm(avgN);if(al<CParam_MinNormalNorm||area<=0)return false;avgN=avgN/al;cent=cent/area;
    double oldDev=0;for(Vec3 n:oldN)oldDev=max(oldDev,1.0-dot(n,avgN));if(oldDev>0.045)return false;
    // Plane basis and polygon validity.
    Vec3 axis=fabs(avgN.x)<0.8?Vec3(1,0,0):Vec3(0,1,0);Vec3 u=cross(axis,avgN);u=u/norm(u);Vec3 v=cross(avgN,u);
    vector<pair<double,double>>p2;for(int x:ring){Vec3 d=vv[x]-cent;p2.push_back({dot(d,u),dot(d,v)});}double ar2=0;for(int i=0;i<(int)p2.size();++i){auto a=p2[i],b=p2[(i+1)%p2.size()];ar2+=a.first*b.second-a.second*b.first;}if(fabs(ar2)<1e-15*max(1.0,diag*diag))return false;
    for(int i=0;i<(int)p2.size();++i)for(int j=i+1;j<(int)p2.size();++j){int i2=(i+1)%p2.size(),j2=(j+1)%p2.size();if(i==j||i2==j||j2==i)continue;if(macroSegCross(p2[i].first,p2[i].second,p2[i2].first,p2[i2].second,p2[j].first,p2[j].second,p2[j2].first,p2[j2].second))return false;}
    Vec3 qp=cent;Vec3 solved;if(solveQem3x3(qsum,solved)&&finiteVec(solved))qp=solved;
    // Project the representative back onto the old patch surface.
    Vec3 center=cent;double bd=CParam_Inf;for(auto&t:oldT){Vec3 z=macroClosestPointTri(qp,t[0],t[1],t[2]);double d=norm2(z-qp);if(d<bd){bd=d;center=z;}}
    double cx=dot(center-cent,u),cy=dot(center-cent,v);bool pin=false;for(int i=0,j=(int)p2.size()-1;i<(int)p2.size();j=i++){double xi=p2[i].first,yi=p2[i].second,xj=p2[j].first,yj=p2[j].second;if(((yi>cy)!=(yj>cy))&&(cx<(xj-xi)*(cy-yi)/(yj-yi+1e-300)+xi))pin=!pin;}if(!pin)return false;
    vector<array<Vec3,3>>newT;newT.reserve(ring.size());double newArea=0,maxND=0;
    for(int i=0;i<(int)ring.size();++i){Vec3 a=vv[ring[i]],b=vv[ring[(i+1)%ring.size()]],c=center;Vec3 n=cross(b-a,c-a);double nl=norm(n);if(nl<1e-14*max(1.0,diag*diag)||dot(n,avgN)<=0)return false;Vec3 un=n/nl;Vec3 cc=(a+b+c)/3.0;double best=CParam_Inf,md=1;for(int k=0;k<(int)oldT.size();++k){double d=norm2(cc-oldC[k]);if(d<best){best=d;md=1.0-dot(un,oldN[k]);}}maxND=max(maxND,md);newArea+=.5*nl;newT.push_back({a,b,c});}
    if(maxND>0.018)return false;double areaRel=fabs(newArea-oldArea)/(oldArea+1e-30);if(areaRel>0.025)return false;
    double dev2=max(macroTriSetDistance2(oldT,newT),macroTriSetDistance2(newT,oldT));if(!isfinite(dev2))return false;double dev=sqrt(max(0.0,dev2));if(dev>0.055*hausd)return false;
    out.patchFaces=patch;out.boundary=ring;out.interior=inside;out.center=center;out.gain=(int)inside.size()-1;out.geom=dev;out.normalDamage=maxND;
    double damage=dev/(hausd+1e-30)+18.0*maxND+2.0*areaRel;out.score=damage/max(1,out.gain)+0.0002*ring.size();return out.valid();
}

__attribute__((noinline,cold)) bool macroApplyBatch(const vector<Vec3>&baseV,const vector<Face>&baseF,const vector<MacroRegionCandidate>&sel,vector<Vec3>&outV,vector<Face>&outF)const{
    vector<char>rmF(baseF.size(),0),rmV(baseV.size(),0);outV=baseV;outF.clear();outF.reserve(baseF.size());
    for(auto&c:sel){for(int f:c.patchFaces){if(f<0||f>=(int)rmF.size()||rmF[f])return false;rmF[f]=1;}for(int v:c.interior){if(v<0||v>=(int)rmV.size()||rmV[v])return false;rmV[v]=1;}}
    for(int fi=0;fi<(int)baseF.size();++fi)if(!rmF[fi])outF.push_back(baseF[fi]);
    for(auto&c:sel){int z=(int)outV.size();outV.push_back(c.center);for(int i=0;i<(int)c.boundary.size();++i){Face f;f.v[0]=c.boundary[i];f.v[1]=c.boundary[(i+1)%c.boundary.size()];f.v[2]=z;outF.push_back(f);}}
    vector<char>used(outV.size(),0);for(auto&f:outF)for(int k=0;k<3;++k)used[f.v[k]]=1;vector<int>mp(outV.size(),-1);vector<Vec3>nv;for(int i=0;i<(int)outV.size();++i)if(used[i]&&(i>=(int)rmV.size()||!rmV[i])){mp[i]=(int)nv.size();nv.push_back(outV[i]);}
    vector<Face>nf;nf.reserve(outF.size());set<array<int,3>>keys;for(auto f:outF){for(int k=0;k<3;++k){if(f.v[k]<0||f.v[k]>=(int)mp.size()||mp[f.v[k]]<0)return false;f.v[k]=mp[f.v[k]];}if(f.v[0]==f.v[1]||f.v[1]==f.v[2]||f.v[0]==f.v[2])return false;array<int,3>q={f.v[0],f.v[1],f.v[2]};sort(q.begin(),q.end());if(!keys.insert(q).second)return false;nf.push_back(f);}outV.swap(nv);outF.swap(nf);return macroClosedOriented(outV,outF);
}

__attribute__((noinline,cold)) bool runMacroRegionAlternatives(const vector<Vec3>&baseV,const vector<Face>&baseF,
                                vector<Vec3>&manyV,vector<Face>&manyF,
                                vector<Vec3>&oneV,vector<Face>&oneF)const{
    if(baseV.size()<100||elapsed()>HParam_TimeBudgetSeconds-4.0)return false;
    vector<vector<int>>vf,fa;
    macroBuildTopology(baseF,(int)baseV.size(),vf,fa);
    vector<MacroRegionCandidate>cs;cs.reserve(512);
    int stride=max(1,(int)baseV.size()/5000);
    for(int seed=0;seed<(int)baseV.size();seed+=stride){
        if(elapsed()>HParam_TimeBudgetSeconds-3.55)break;
        MacroRegionCandidate c,best;
        for(int d=1;d<=2;++d)
            if(macroCandidate(baseV,baseF,vf,fa,seed,d,c)&&
               (!best.valid()||c.score<best.score))best=c;
        if(best.valid())cs.push_back(move(best));
    }
    if(cs.empty())return false;
    sort(cs.begin(),cs.end());

    // The geometric test finds viable macro operations.  A cheap six-view pass
    // then ranks whole regions by the actual normal/depth damage they cause.
    constexpr int RR=256;
    vector<RefImage>refLow(6);
    for(int v=0;v<6;++v)refLow[v]=renderReference(baseV,baseF,v,RR);
    struct Ranked{int idx=-1;double key=CParam_Inf;};
    vector<Ranked>ranked;ranked.reserve(40);
    int lim=min(36,(int)cs.size());
    for(int i=0;i<lim;++i){
        if(elapsed()>HParam_TimeBudgetSeconds-2.15)break;
        vector<Vec3>tv;vector<Face>tf;
        vector<MacroRegionCandidate>single{cs[i]};
        if(!macroApplyBatch(baseV,baseF,single,tv,tf))continue;
        RefScore sc=compareToReference(refLow,tv,tf,RR);
        double key=(1.0-sc.finalScore+1.5*(1.0-sc.minNormal)+
                    2.5*(1.0-sc.minDepth))/max(1,cs[i].gain);
        ranked.push_back({i,key});
    }
    if(ranked.empty())return false;
    sort(ranked.begin(),ranked.end(),[](const Ranked&a,const Ranked&b){return a.key<b.key;});

    vector<MacroRegionCandidate>one{cs[ranked[0].idx]};
    if(!macroApplyBatch(baseV,baseF,one,oneV,oneF)||oneV.size()>=baseV.size())return false;

    vector<char>usedV(baseV.size(),0),usedF(baseF.size(),0);
    vector<MacroRegionCandidate>many;
    for(const Ranked&r:ranked){
        const auto&c=cs[r.idx];bool clash=false;
        for(int f:c.patchFaces)if(usedF[f]){clash=true;break;}
        if(!clash)for(int v:c.boundary)if(usedV[v]){clash=true;break;}
        if(!clash)for(int v:c.interior)if(usedV[v]){clash=true;break;}
        if(clash)continue;
        many.push_back(c);
        for(int f:c.patchFaces)usedF[f]=1;
        for(int v:c.boundary)usedV[v]=1;
        for(int v:c.interior)usedV[v]=1;
        if(many.size()>=3)break;
    }
    if(many.empty())return false;
    if(!macroApplyBatch(baseV,baseF,many,manyV,manyF)||manyV.size()>=baseV.size()){
        manyV=oneV;manyF=oneF;
    }
    return true;
}

__attribute__((noinline,cold)) void runHugeAtomicRegionTxPlain();
...
else kr=hugeInputMode?0.028:HParam_KeepRatio_Huge;
...
bool tailMode()const{if(nV<=HParam_TailOriginalVertexThreshold)return false;if(hugeInputMode)return accepted*100LL>=collapseLimit*95LL;double e=elapsed();return e>HParam_TailBatchElapsedStart&&e<HParam_TailBatchStopElapsed;}
...
int scanCap=hugeInputMode?131072:HParam_TailBatchScanEdges;int acceptCap=hugeInputMode?4096:HParam_TailBatchTargetAccepts;double tailStop=hugeInputMode?19.8:HParam_TailBatchStopElapsed;
vector<EBC> cands;cands.reserve(scanCap/4);

int scanned=0,visited=0;

for(;visited<(int)verts.size()&&scanned<scanCap;++visited){

int a=(tailCursor+visited)%(int)verts.size();if(vdead[a])continue;

for(int b:vneigh[a]){if(scanned>=scanCap)break;if(b<=a||vdead[b])continue;++scanned;
...
vector<EBC> sel;sel.reserve(acceptCap);

for(auto&e:cands){if((int)sel.size()>=acceptCap)break;if(!edgeExists(e.a,e.b))continue;if(!batchFree(e.a,e.b))continue;lockBatch(e.a,e.b);sel.push_back(e);}

int acc=0;

for(auto&e:sel){

if(accepted>=collapseLimit||(!hugeInputMode&&elapsed()>tailStop))break;
...
__attribute__((noinline,cold,section(".text.unlikely.atomic_t7"))) void QemSimplifier::runHugeAtomicRegionTxPlain(){
    if(nV<100)return;
    const vector<Vec3>baseV=verts;const vector<Face>baseF=faces;
    vector<vector<int>>vf,fa;macroBuildTopology(baseF,nV,vf,fa);
    MacroRegionCandidate best;const int samples=128,stride=max(1,nV/samples);int tested=0;
    for(int seed=0;seed<nV&&tested<samples;seed+=stride,++tested){
        MacroRegionCandidate c,c2;bool ok=macroCandidate(baseV,baseF,vf,fa,seed,1,c);
        if(macroCandidate(baseV,baseF,vf,fa,seed,2,c2)&&(!ok||c2.score<c.score)){c=move(c2);ok=true;}
        if(!ok||c.geom>0.024*hausd||c.normalDamage>0.0075||c.boundary.size()>20||c.interior.size()>22||c.gain<2)continue;
        if(!best.valid()||c.score<best.score||(c.score==best.score&&c.gain>best.gain))best=move(c);
    }
    if(!best.valid())return;
    vector<MacroRegionCandidate>one{best};vector<Vec3>candV;vector<Face>candF;
    if(!macroApplyBatch(baseV,baseF,one,candV,candF)||candV.size()>=baseV.size())return;
    constexpr int R=96;vector<RefImage>ref(6);for(int v=0;v<6;++v)ref[v]=renderReference(baseV,baseF,v,R);
    RefScore sc=compareToReference(ref,candV,candF,R);
    if(sc.finalScore<0.99996||sc.minView<0.99990||sc.minNormal<0.99982||sc.minDepth<0.99998)return;
    verts.swap(candV);faces.swap(candF);nV=(int)verts.size();nF=(int)faces.size();
}
