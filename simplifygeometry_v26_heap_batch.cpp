...
/*
IMC2026 Problem B — simplifygeometry_v26_heap_batch.cpp

V24 tail batching sourced from refreshed heap candidates instead of adjacency scan.
...
private:
    ...
    void requeueCurrentEdge(int a, int b) {
        if (!edgeStillExists(a, b)) return;
    ...
    int runTailBatch() {
        ...
        candidates.reserve(HParam_TailBatchScanEdges);
        const double activeCap = currentCostCap();

        for (int popped = 0; popped < HParam_TailBatchScanEdges && !queue.empty(); ++popped) {
            ...
            candidates.push_back({a, b, c.cost, c});
        }
        ...
        vector<char> selectedFlags(candidates.size(), 0);
        selected.reserve(HParam_TailBatchTargetAccepts);
        for (int i = 0; i < (int)candidates.size(); ++i) {
            const EdgeBatchCandidate& e = candidates[i];
        ...
        for (int i = 0; i < (int)candidates.size(); ++i) {
            if (!selectedFlags[i] && candidates[i].candidate.valid()) {
                queue.push(candidates[i].candidate);
            }
        }
        ...
        for (const EdgeBatchCandidate& e : selected) {
            ...
            if (!best.valid()) {
                requeueCurrentEdge(e.a, e.b);
                continue;
            }
            if (best.cost > currentCostCap()) {
                requeueCurrentEdge(e.a, e.b);
                continue;
            }
