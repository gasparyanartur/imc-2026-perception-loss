...
/*
IMC2026 Problem B — simplifygeometry_v18_stats.cpp

V15 baseline with optional LOCAL_STATS instrumentation.
...
#ifdef LOCAL_STATS
    struct LocalStats {
        long long deadEdgePops = 0;
        long long stalePops = 0;
        long long topologyRejects = 0;
        long long invalidCandidateRejects = 0;
        long long envelopeRejects = 0;
        long long neighborRejects = 0;
        long long candidateCostCapRejects = 0;
        long long stopCostCapBreaks = 0;
        int stopReason = 0; // 1=time, 2=target, 3=cost cap, 4=queue empty
    };

    mutable LocalStats stats;
#endif
...
private:
    ...
    bool passesEnvelopeGuard(int a, int b, const Vec3& p, double& mergedRadius) const {
        ...
        bool ok = mergedRadius <= hausdorffLimit;
#ifdef LOCAL_STATS
        if (!ok) ++stats.envelopeRejects;
#endif
        return ok;
    }
    ...
    bool passesNeighborDistanceGuard(int absorbedVertex, int keptVertex, const Vec3& p) const {
        ...
        for (int nb : vertexNeighbors[absorbedVertex]) {
            ...
            if (norm(vertices[nb] - p) > neighborDistanceLimit) {
#ifdef LOCAL_STATS
                ++stats.neighborRejects;
#endif
                return false;
            }
        }
    ...
    void collapseLoop() {
        ...
        while (acceptedCollapses < collapseLimit && !queue.empty()) {
            ...
            if (!best.valid()) {
#ifdef LOCAL_STATS
                ++stats.invalidCandidateRejects;
#endif
                continue;
            }
            if (best.cost > costCap) {
#ifdef LOCAL_STATS
                ++stats.candidateCostCapRejects;
#endif
                continue;
            }
...
#ifdef LOCAL_STATS
        if (stats.stopReason == 0) {
            if (acceptedCollapses >= collapseLimit) stats.stopReason = 2;
            else if (queue.empty()) stats.stopReason = 4;
        }
#endif
    }
    ...
    void printStats() const {
        const int alive = (int)vertices.size() - acceptedCollapses;
        const char* reason = "unknown";
        if (stats.stopReason == 1) reason = "time";
        else if (stats.stopReason == 2) reason = "target";
        else if (stats.stopReason == 3) reason = "cost_cap";
        else if (stats.stopReason == 4) reason = "queue_empty";
        fprintf(stderr,
                "LOCAL_STATS stop=%s elapsed=%.6f original=%d alive=%d accepted=%d "
                "queue=%zu dead_edge_pops=%lld stale_pops=%lld topology_rejects=%lld "
                "invalid_candidate_rejects=%lld envelope_rejects=%lld neighbor_rejects=%lld "
                "candidate_costcap_rejects=%lld stop_costcap_breaks=%lld\n",
                reason, elapsedSeconds(), (int)vertices.size(), alive, acceptedCollapses,
                queue.size(), stats.deadEdgePops, stats.stalePops, stats.topologyRejects,
                stats.invalidCandidateRejects, stats.envelopeRejects, stats.neighborRejects,
                stats.candidateCostCapRejects, stats.stopCostCapBreaks);
    }
#endif
