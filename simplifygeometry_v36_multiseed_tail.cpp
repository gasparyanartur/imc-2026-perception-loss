...
/*
IMC2026 Problem B — simplifygeometry_v36_multiseed_tail.cpp

V25-style deterministic multi-profile stochastic tail batching.
...
static constexpr int    HParam_TailBatchConflictMode = 1; // fallback only; 0=one-ring, 1=common-neighbor, 2=endpoints
static constexpr double HParam_TailBatchJitterAmplitude = 0.010;
static constexpr unsigned long long HParam_TailBatchJitterSeed = 11ULL;
...
private:
    ...
    double meshRiskScore = 0.0;
    unsigned long long meshSeed = HParam_TailBatchJitterSeed;
...
private:
    ...
    void buildInitialConnectivityAndQuadrics() {
        ...
        for (int fi = 0; fi < nF; ++fi) {
            ...
            faceNormals[fi] = normalizedOrZero(cross(p1 - p0, p2 - p0));
        ...
        for (int v = 0; v < nV; ++v) {
            Vec3 sumNormal;
            for (int fi : vertexFaces[v]) sumNormal += faceNormals[fi];
            Vec3 avgNormal = normalizedOrZero(sumNormal);
            double curvatureRisk = 0.0;
            for (int fi : vertexFaces[v]) {
                const Vec3& n = faceNormals[fi];
                if (norm2(n) > 1e-30 && norm2(avgNormal) > 1e-30) {
                    curvatureRisk = max(curvatureRisk, 1.0 - fabs(dot(n, avgNormal)));
                }
            }
            double valence = (double)vertexNeighbors[v].size();
            double valenceRisk = valence <= 8.0 ? 0.0 : min(1.0, (valence - 8.0) / 8.0);
            riskSum += curvatureRisk + 0.2 * valenceRisk;
            ++riskCount;
            meshSeed ^= mix64((unsigned long long)(v + 1) * 0x9e3779b97f4a7c15ULL
                              ^ (unsigned long long)vertexNeighbors[v].size());
        }
        meshRiskScore = riskCount ? riskSum / (double)riskCount : 0.0;
    }
    ...
    struct EdgeBatchCandidate {
        ...
        int b = -1;
        double cost = CParam_Inf;
        double sortCost = CParam_Inf;
        bool operator<(const EdgeBatchCandidate& o) const {
            return sortCost < o.sortCost;
        }
    };

    int profileConflictMode() const {
        if ((int)vertices.size() > 900000 && meshRiskScore < 0.10) return 2;
        if (meshRiskScore < 0.18) return 1;
        return 0;
    }

    double profileJitterAmplitude() const {
        if ((int)vertices.size() > 900000 && meshRiskScore < 0.10) return 0.020;
        if (meshRiskScore < 0.18) return 0.010;
        return 0.005;
    }

    bool batchNeighborhoodFree(int a, int b) const {
        if (tailLocks[a] == tailLockStamp || tailLocks[b] == tailLockStamp) return false;
        int conflictMode = profileConflictMode();
        if (conflictMode == 2) return true;
        if (conflictMode == 1) {
            const auto& na = vertexNeighbors[a];
            const auto& nb = vertexNeighbors[b];
            if (na.size() < nb.size()) {
    ...
    double jitteredBatchCost(int a, int b, double cost) const {
        double amplitude = profileJitterAmplitude();
        if (amplitude <= 0.0) return cost;
        unsigned long long lo = (unsigned long long)min(a, b);
        unsigned long long hi = (unsigned long long)max(a, b);
        unsigned long long h = meshSeed;
        h ^= mix64(lo + 0x123456789abcdefULL);
        h ^= mix64((hi << 1) + 0xfedcba987654321ULL);
        h ^= mix64((unsigned long long)acceptedCollapses + 0x517cc1b727220a95ULL);
        double u = (double)(h >> 11) * (1.0 / 9007199254740992.0);
        double jitter = (2.0 * u - 1.0) * amplitude;
        return cost * max(0.0, 1.0 + jitter);
    }
    ...
    int runTailBatch() {
        ...
        for (; visitedVertices < nV && scannedEdges < HParam_TailBatchScanEdges; ++visitedVertices) {
            ...
            for (int b : vertexNeighbors[a]) {
                ...
                candidates.push_back({a, b, c.cost, jitteredBatchCost(a, b, c.cost)});
            }
        }
