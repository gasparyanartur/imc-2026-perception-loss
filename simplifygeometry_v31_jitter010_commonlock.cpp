...
/*
IMC2026 Problem B — simplifygeometry_v31_jitter010_commonlock.cpp

V27 deterministic stochastic tail batching, jitter 0.010 seed 5, common-neighbor lock.
...
static constexpr int    HParam_TailBatchConflictMode = 1; // 0=one-ring, 1=common-neighbor, 2=endpoints
static constexpr double HParam_TailBatchJitterAmplitude = 0.010;
static constexpr unsigned long long HParam_TailBatchJitterSeed = 5ULL;
...
private:
    ...
    bool batchNeighborhoodFree(int a, int b) const {
        ...
        if (HParam_TailBatchConflictMode == 2) return true;
        if (HParam_TailBatchConflictMode == 1) {
            const auto& na = vertexNeighbors[a];
            const auto& nb = vertexNeighbors[b];
            if (na.size() < nb.size()) {
                for (int x : na) {
    ...
    void lockBatchNeighborhood(int a, int b) {
        ...
        if (HParam_TailBatchConflictMode == 2) return;
        if (HParam_TailBatchConflictMode == 1) {
            const auto& na = vertexNeighbors[a];
            const auto& nb = vertexNeighbors[b];
            if (na.size() < nb.size()) {
    ...
    double jitteredBatchCost(int a, int b, double cost) const {
        if (HParam_TailBatchJitterAmplitude <= 0.0) return cost;
        unsigned long long lo = (unsigned long long)min(a, b);
        unsigned long long hi = (unsigned long long)max(a, b);
        unsigned long long h = HParam_TailBatchJitterSeed;
        ...
        double jitter = (2.0 * u - 1.0) * HParam_TailBatchJitterAmplitude;
