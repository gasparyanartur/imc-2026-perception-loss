...
static constexpr double CParam_ViewWeightK = 850.0;
static constexpr double CParam_MaxFaceWeight = 2.4;
...
static constexpr bool   HParam_EnableEdgePatchGuard = true;
static constexpr int    HParam_EdgePatchMaxPixels = 18000;
...
private:
    ...
    double faceWeightFor(const Vec3& p0,const Vec3& p1,const Vec3& p2,const Vec3& unitNormal,double area) const {
        Vec3 c=(p0+p1+p2)/3.0;
        double visual=0.0;
        for(int k=0;k<6;++k){
            Vec3 eye=cameraDirs[k]*2.5;
            Vec3 fwd=cameraDirs[k]*(-1.0);
            Vec3 rel=c-eye;
            double z=dot(rel,fwd);
            if(z<=1e-8)continue;
            double nd=fabs(dot(unitNormal,fwd));
            double invz2=1.0/(z*z);
            double proj=area*nd*invz2;
            double sil=area*sqrt(max(0.0,1.0-nd*nd))*invz2;
            visual+=proj+0.22*sil;
        }
        double w = 1.0 + CParam_ViewWeightK * visual;
    ...
    void initFaceWeights(){
        ...
        for(int fi=0;fi<nF;++fi){
            ...
            if(area >= 1e-30){
                n = n / (2.0*area);
                double w = faceWeightFor(verts[f.v[0]],verts[f.v[1]],verts[f.v[2]],n,area);
    ...
    CollapseCandidate computeBestValid(int a,int b)const{
        ...
        for(int i=0;i<np;++i){
            for(int dir=0;dir<2;++dir){
                ...
                c.mergedRadius=mr;
                if(edgePatchShouldCheck(c.cost)&&!edgePatchAccepts(ab,kp,pos[i],c.cost))continue;
    ...
    void applyCollapse(int ab,int kp,const Vec3&np,double nr){
        ...
        if(MEMLESS){
            ...
            for(int fi:vfaces[kp]){if(fdead[fi])continue;const Face&f=faces[fi];
                ...
                if(area>=1e-30){
                    n=n/(2.0*area);
                    double w=faceWeightFor(verts[f.v[0]],verts[f.v[1]],verts[f.v[2]],n,area)*visibilityFaceWeight(fi);
                    if(w!=1.0)q.scale(w);
                }
                fresh+=q;}
            vquad[kp]=fresh;
    ...
    bool edgePatchShouldCheck(double cost)const{
        if(!HParam_EnableEdgePatchGuard)return false;
        int tier=originalTier();
        if(tier<2||tier>4)return false;
        if(elapsed()>HParam_TimeBudgetSeconds-1.25)return false;
        if(collapseLimit<=0)return false;
        return accepted>int(0.82*collapseLimit)||cost>0.72*costCap;
    }
    double edgePatchMinSsim()const{
        int tier=originalTier();
        if(tier==2)return 0.974;
        if(tier==3)return 0.962;
        if(tier==4)return 0.958;
        return 1.01;
    }
    double edgePatchGeomFrac()const{
        int tier=originalTier();
        if(tier==2)return 0.55;
        if(tier==3)return 0.50;
        if(tier==4)return 0.48;
        return 0.0;
    }
    bool buildEdgePatchTris(int ab,int kp,const Vec3& p,vector<VegaTri>& oldTris,vector<VegaTri>& newTris)const{
        oldTris.clear();newTris.clear();
        vector<int> inc;
        inc.reserve(vfaces[ab].size()+vfaces[kp].size());
        auto addFace=[&](int fi){
            if(fi<0||fi>=(int)faces.size()||fdead[fi])return;
            for(int x:inc)if(x==fi)return;
            inc.push_back(fi);
        };
        for(int fi:vfaces[ab])addFace(fi);
        for(int fi:vfaces[kp])addFace(fi);
        if(inc.empty())return false;
        for(int fi:inc){
            const Face& f=faces[fi];
            VegaTri ot;
            for(int k=0;k<3;++k)ot.p[k]=verts[f.v[k]];
            oldTris.push_back(ot);
            int ids[3]={f.v[0],f.v[1],f.v[2]};
            for(int k=0;k<3;++k)if(ids[k]==ab)ids[k]=kp;
            if(ids[0]==ids[1]||ids[1]==ids[2]||ids[0]==ids[2])continue;
            VegaTri nt;
            for(int k=0;k<3;++k)nt.p[k]=(ids[k]==kp)?p:verts[ids[k]];
            Vec3 nr=cross(nt.p[1]-nt.p[0],nt.p[2]-nt.p[0]);
            if(norm2(nr)<CParam_MinNormalNorm*CParam_MinNormalNorm)continue;
            newTris.push_back(nt);
        }
        return !oldTris.empty()&&!newTris.empty();
    }
    double vegaWindowedSsimChannel(const vector<VegaPixel>& a,const vector<VegaPixel>& b,int w,int h,int ch)const{
        double total=0.0;int centers=0;
        for(int y=0;y<h;++y)for(int x=0;x<w;++x){
            int ci=y*w+x;
            if(!a[ci].fg&&!b[ci].fg)continue;
            double sx=0,sy=0,sxx=0,syy=0,sxy=0;int cnt=0;
            int y0=max(0,y-5),y1=min(h-1,y+5);
            int x0=max(0,x-5),x1=min(w-1,x+5);
            for(int yy=y0;yy<=y1;++yy)for(int xx=x0;xx<=x1;++xx){
                int idx=yy*w+xx;
                double vx=vegaPixelChannel(a[idx],ch),vy=vegaPixelChannel(b[idx],ch);
                sx+=vx;sy+=vy;sxx+=vx*vx;syy+=vy*vy;sxy+=vx*vy;++cnt;
            }
            if(cnt<4)continue;
            double inv=1.0/double(cnt);
            double mx=sx*inv,my=sy*inv;
            double varx=max(0.0,sxx*inv-mx*mx),vary=max(0.0,syy*inv-my*my);
            double cov=sxy*inv-mx*my;
            double num=(2.0*mx*my+HParam_VegaC1)*(2.0*cov+HParam_VegaC2);
            double den=(mx*mx+my*my+HParam_VegaC1)*(varx+vary+HParam_VegaC2);
            total+=(den>0.0)?clampDouble(num/den,-1.0,1.0):1.0;
            ++centers;
        }
        if(centers==0)return 1.0;
        return total/double(centers);
    }
    double vegaBuffersWindowedSsim(const vector<VegaPixel>& a,const vector<VegaPixel>& b,int w,int h)const{
        double sn=(vegaWindowedSsimChannel(a,b,w,h,0)+
                   vegaWindowedSsimChannel(a,b,w,h,1)+
                   vegaWindowedSsimChannel(a,b,w,h,2))/3.0;
        double sd=vegaWindowedSsimChannel(a,b,w,h,3);
        return 0.5*sn+0.5*sd;
    }
    double localEdgePatchSsim(int ab,int kp,const Vec3& p)const{
        vector<VegaTri> oldTris,newTris;
        if(!buildEdgePatchTris(ab,kp,p,oldTris,newTris))return -1.0;
        double dev=vegaPatchDeviation(oldTris,newTris);
        if(!(dev<=hausd*edgePatchGeomFrac()))return -1.0;
        const int R=HParam_VegaPatchResolution;
        ...
        for(int view=0;view<6;++view){
            ...
            auto includeTri=[&](const VegaTri& t){
                for(int k=0;k<3;++k){
                    VegaProj p2=vegaProjectPoint(t.p[k],view,R);
                    if(!p2.ok)continue;
                    any=true;
                    mnU=min(mnU,p2.u);mxU=max(mxU,p2.u);
                    mnV=min(mnV,p2.v);mxV=max(mxV,p2.v);
                }
            };
            for(const VegaTri& t:oldTris)includeTri(t);
            for(const VegaTri& t:newTris)includeTri(t);
            if(!any)continue;
            int pad=HParam_VegaPatchPaddingPixels+5;
            ...
            if(w*h>HParam_EdgePatchMaxPixels)return -1.0;
            renderVegaPatch(oldTris,view,x0,y0,w,h,a);
            renderVegaPatch(newTris,view,x0,y0,w,h,b);
            total+=vegaBuffersWindowedSsim(a,b,w,h);
            ++usedViews;
        }
        if(usedViews==0)return 1.0;
        return total/double(usedViews);
    }
    bool edgePatchAccepts(int ab,int kp,const Vec3& p,double)const{
        double s=localEdgePatchSsim(ab,kp,p);
        return s>=edgePatchMinSsim();
    }
