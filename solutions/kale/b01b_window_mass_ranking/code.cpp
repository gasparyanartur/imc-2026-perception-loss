...
long long viewForegroundWindows=0;
for(int y=5;y<side-5;++y)for(int x=5;x<side-5;++x){
int center=y*side+x;
viewForegroundWindows+=bf[center]||af[center];
}
totalLoss+=(1.0-0.5*(normalSsim+depthSsim))*viewForegroundWindows;
foregroundMass+=viewForegroundWindows;
}
return foregroundMass?totalLoss/foregroundMass:_k;
}
