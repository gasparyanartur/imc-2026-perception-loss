...
if(tier==3){
int cap=fc?fc:_H1*breadth;
VC<int>natural(evals.size());
iota(natural.begin(),natural.end(),0);
VC<VC<int>>foot(evals.size());
for(int i=0;i<(int)evals.size();++i){
const auto&e=evals[i];
foot[i]={e.c._o,e.c.kept};
for(int x:ZN[e.c._o])foot[i].push_back(x);
for(int x:ZN[e.c.kept])foot[i].push_back(x);
sort(foot[i].begin(),foot[i].end());
foot[i].erase(unique(foot[i].begin(),foot[i].end()),foot[i].end());
}
auto conflict=[&](int i,int j){
size_t a=0,b=0;
while(a<foot[i].size()&&b<foot[j].size()){
if(foot[i][a]==foot[j][b])return true;
if(foot[i][a]<foot[j][b])++a;
else++b;
}
int hit=0;
for(int v=0;v<6;++v)if(_W3(evals[i].centers[v],evals[j].centers[v]))++hit;
return hit>ov;
};
auto choose=[&](const VC<int>&order,_l&score){
VC<char>lock(ZV.size(),0);
VC<array<_m,6>>rects;
DB sums[6][4];
long long counts[6];
...
VC<int>selected;
for(int id:order){
if((int)selected.size()>=cap||ET()>=stop)break;
const auto&e=evals[id];
int a=e.c._o,b=e.c.kept;
if(!_n1(a,b)||lock[a]||lock[b])continue;
bool bad=false;
for(int x:ZN[a])if(lock[x]){bad=true;break;}
for(int x:ZN[b])if(lock[x]){bad=true;break;}
if(bad)continue;
for(const auto&r:rects){
int hit=0;
for(int v=0;v<6;++v)if(_W3(e.centers[v],r[v]))++hit;
if(hit>ov){bad=true;break;}
}
if(bad)continue;
DB ns[6][4];
long long nc[6];
for(int v=0;v<6;++v){
nc[v]=counts[v]+e._J3[v];
for(int ch=0;ch<4;++ch)ns[v][ch]=sums[v][ch]+e._y4[v][ch];
}
_l trial=_b2(ns,nc);
...
for(int v=0;v<6;++v){
counts[v]=nc[v];
for(int ch=0;ch<4;++ch)sums[v][ch]=ns[v][ch];
}
lock[a]=lock[b]=1;
for(int x:ZN[b])if(x>=0&&x<(int)lock.size())lock[x]=1;
rects.push_back(e.centers);
score=trial;
}
return selected;
};
_l naturalScore=base.score;
VC<int>selected=choose(natural,naturalScore);
if(ET()<stop){
VC<int>degree(evals.size(),0),opportunity=natural;
for(int i=0;i<(int)evals.size();++i)for(int j=i+1;j<(int)evals.size();++j)if(conflict(i,j))++degree[i],++degree[j];
sort(opportunity.begin(),opportunity.end(),[&](int i,int j){
DB a=max(0.,evals[i].damage)+budget*degree[i]/max<size_t>(1,evals.size());
DB b=max(0.,evals[j].damage)+budget*degree[j]/max<size_t>(1,evals.size());
if(a!=b)return a<b;
if(evals[i].damage!=evals[j].damage)return evals[i].damage<evals[j].damage;
return evals[i].c.cost<evals[j].c.cost;
});
_l opportunityScore=base.score;
VC<int>alternative=choose(opportunity,opportunityScore);
if(alternative.size()>selected.size()||(alternative.size()==selected.size()&&(opportunityScore._f>naturalScore._f||(opportunityScore._f==naturalScore._f&&opportunityScore.minView>naturalScore.minView))))selected.swap(alternative);
}
if(selected.size()<size_t(cap)&&ET()<stop){
VC<char>in(evals.size(),0);
for(int id:selected)in[id]=1;
VC<VC<int>>only(selected.size());
for(int id=0;id<(int)evals.size();++id)if(!in[id]){
int blocker=-1,count=0;
for(int s=0;s<(int)selected.size();++s)if(conflict(id,selected[s])){
blocker=s;
if(++count>1)break;
}
if(count==1&&only[blocker].size()<64)only[blocker].push_back(id);
}
DB sums[6][4];
long long counts[6];
for(int v=0;v<6;++v){
...
for(int id:selected)for(int v=0;v<6;++v){
counts[v]+=evals[id]._J3[v];
for(int ch=0;ch<4;++ch)sums[v][ch]+=evals[id]._y4[v][ch];
}
int bs=-1,ba=-1,bb=-1;
_l best;
for(int s=0;s<(int)selected.size()&&ET()<stop;++s){
const auto&pool=only[s];
for(int x=0;x<(int)pool.size();++x)for(int y=x+1;y<(int)pool.size();++y){
int a=pool[x],b=pool[y];
if(conflict(a,b))continue;
DB ns[6][4];
long long nc[6];
for(int v=0;v<6;++v){
nc[v]=counts[v]-evals[selected[s]]._J3[v]+evals[a]._J3[v]+evals[b]._J3[v];
for(int ch=0;ch<4;++ch)ns[v][ch]=sums[v][ch]-evals[selected[s]]._y4[v][ch]+evals[a]._y4[v][ch]+evals[b]._y4[v][ch];
}
_l trial=_b2(ns,nc);
if(trial._f<base.score._f-budget||trial.minView<base.score.minView-_q)continue;
if(bs<0||trial._f>best._f||(trial._f==best._f&&trial.minView>best.minView))bs=s,ba=a,bb=b,best=trial;
}
}
if(bs>=0){
selected[bs]=ba;
...
for(int id:selected){
const auto&e=evals[id];
if(!_n1(e.c._o,e.c.kept))continue;
_D1(e.c._o,e.c.kept,e.c._t,e.c._E);
++_j;
++done;
}
if(!done){_i(safe);return 0;}
A3(activeV,activeF);
VC<VC<_d>>audit(6);
for(int v=0;v<6;++v)_Q1(activeV,activeF,v,R,audit[v]);
_l audited=_K3(ref,audit,R);
if(audited._f<base.score._f-budget||audited.minView<base.score.minView-_q){_i(safe);return 0;}
return done;
}
VC<char>lock(ZV.size(),0);
VC<array<_m,6>>_a4;
...
bool cov=b>=0;
DB b2=b*b;
if(cov)for(int y:kept)if(norm2(ZV[x]-ZV[y])<=b2){
cov=true;
goto covered;
}
cov=false;
