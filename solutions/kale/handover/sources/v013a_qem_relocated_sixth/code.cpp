...
DB _P6(int kept,int removed,const V3&position,const VC<VC<int>>&vertexFaces)const{
...
for(int k=0;
k<3;
++k)triangle.p[k]=(rewritten.v[k]==kept?position:ZV[rewritten.v[k]]);
...
int vertex=face.v[k];
V3 point=ZV[vertex];
if(after&&(vertex==removed||vertex==kept))point=position;
auto projected=A6(point,view,resolution);
...
bool _G7(int kept,int removed,const V3&position,const VC<VC<int>>&vf)const{
VC<int>fs=vf[kept];
fs.insert(fs.end(),vf[removed].begin(),vf[removed].end());
sort(fs.begin(),fs.end());
fs.erase(unique(fs.begin(),fs.end()),fs.end());
for(int fi:fs){
FC f=ZF[fi];
V3 p[3];
for(int k=0;k<3;++k)p[k]=(f.v[k]==kept||f.v[k]==removed)?position:ZV[f.v[k]];
for(int k=0;k<3;++k)if(f.v[k]==removed)f.v[k]=kept;
if(f.v[0]==f.v[1]||f.v[1]==f.v[2]||f.v[2]==f.v[0])continue;
V3 n0=cross(ZV[ZF[fi].v[1]]-ZV[ZF[fi].v[0]],ZV[ZF[fi].v[2]]-ZV[ZF[fi].v[0]]);
V3 n1=cross(p[1]-p[0],p[2]-p[0]);
if(norm2(n1)<1e-24*max(1.,diag*diag*diag*diag)||dot(n0,n1)<=0)return false;
}
return true;
}
V3 _H7(int kept,int removed,const VC<VC<int>>&vf)const{
VC<int>fs=vf[kept];
fs.insert(fs.end(),vf[removed].begin(),vf[removed].end());
sort(fs.begin(),fs.end());
fs.erase(unique(fs.begin(),fs.end()),fs.end());
...
for(int fi:fs){
const FC&f=ZF[fi];
q+=Quadric::_T1(ZV[f.v[0]],ZV[f.v[1]],ZV[f.v[2]]);
}
V3 a=ZV[kept],d=ZV[removed]-a,m=a+d*.5;
DB f0=q._b5(a),fh=q._b5(m),f1=q._b5(a+d);
DB A=2.*(f1+f0-2.*fh),B=f1-f0-A,t=0;
if(A>1e-30*max(1.,diag*diag))t=max(0.,min(1.,-B/(2.*A)));
return a+d*t;
}
__attribute__((noinline,cold,section(".text.unlikely.hiddenedge")))int _P5(const VC<V3>&ov,int maxCandidates=0,DB scoreCap=hparam_T3EndpointWeldScoreCap,bool strategicStrike=false,int strategicProbeCount=16,int placementMode=0){
if(nV<100)return 0;
VC<V3>sv=ZV;
FD.assign(nF,0);
ZD.assign(nV,0);
...
if(placementMode==1){
ts.push_back(.5);
V3 qp=_H7(kept,removed,vf),d=ZV[removed]-ZV[kept];
DB dd=norm2(d),t=dd>0?dot(qp-ZV[kept],d)/dd:0.;
if(t>1e-9&&t<1.-1e-9)ts.push_back(t);
}
else ts={.25,.5,.75};
DB edgeBest=_k,bestT=0;
for(DB t:ts){
V3 p=ZV[kept]+(ZV[removed]-ZV[kept])*t;
if(!_G7(kept,removed,p,vf))continue;
DB loss=_P6(kept,removed,p,vf);
if(loss<edgeBest){edgeBest=loss;bestT=t;}
}
if(placementMode==2&&edgeBest<_k){
for(DB t:{max(0.,bestT-.125),min(1.,bestT+.125)}){
V3 p=ZV[kept]+(ZV[removed]-ZV[kept])*t;
if(!_G7(kept,removed,p,vf))continue;
DB loss=_P6(kept,removed,p,vf);
if(loss<edgeBest){edgeBest=loss;bestT=t;}
...
if(edgeBest<bestRank){
bestRank=edgeBest;
bestIndex=i;
moveKept=kept;
moveRemoved=removed;
movePosition=ZV[kept]+(ZV[removed]-ZV[kept])*bestT;
}
continue;
}
...
DB localSsimLoss=_P6(kept,removed,ZV[kept],vf);
...
++i)if(us[i]){
nm[i]=nv.size();
nv.push_back(i==moveKept?movePosition:ZV[i]);
}
for(auto&f:nf)for(int k=0;
k<3;
++k)f.v[k]=nm[f.v[k]];
