...
for(int view=0;
view<6;
++view)_or[view]=_x(_U,_T,view,1024);
_R0=&_or;
_dw=1e-5;
for(int strike=0;
strike<4;
++strike){
A7(_or,ZV,ZF,1024,true);
...
DB refSsim(const VC<float>&a,const VC<float>&b,const VC<unsigned char>&fa,const VC<unsigned char>&fb,int R,long long*N=nullptr)const{
...
long long nn;
DB sx=refSsim(ref[v].nx,b.nx,ref[v].fg,b.fg,R,debt?&nn:nullptr),sy=refSsim(ref[v].ny,b.ny,ref[v].fg,b.fg,R),sz=refSsim(ref[v].nz,b.nz,ref[v].fg,b.fg,R),sd=refSsim(ref[v].d,b.d,ref[v].fg,b.fg,R),sn=(sx+sy+sz)/3.,cv=.5*(sn+sd);
if(debt){
_gn[v]=nn;
_gs[v][0]=sx;
_gs[v][1]=sy;
...
DB _P6(int kept,int removed,const VC<VC<int>>&_vf0){
VC<int>_afc=_vf0[kept];
_afc.insert(_afc.end(),_vf0[removed].begin(),_vf0[removed].end());
sort(_afc.begin(),_afc.end());
_afc.erase(unique(_afc.begin(),_afc.end()),_afc.end());
VC<VegaTri>_bt;
VC<VegaTri>_at;
_bt.reserve(ZF.size());
_at.reserve(ZF.size());
...
k<3;
++k)triangle.p[k]=ZV[rewritten.v[k]];
_at.push_back(triangle);
}
constexpr int _rs=1024;
DB _tl=0.0;
int _ev=0;
if(_G0)_gm=0;
VC<_d>_bb,_ab;
...
view<6;
++view){
DB minX=_rs,minY=_rs,maxX=0,maxY=0;
bool _ap=false;
for(int faceIndex:_afc){
...
auto _pr=A6(ZV[vertex],view,_rs);
if(!_pr.ok)continue;
_ap=true;
minX=min(minX,_pr.u);
minY=min(minY,_pr.v);
...
if(!_ap){
if(_G0){
_tl+=.5*((_gs[view][0]+_gs[view][1]+_gs[view][2])/3.+_gs[view][3]);
++_ev;
}
continue;
}
...
_l1(_bt,view,x0,y0,side,side,_bb,_rs);
_l1(_at,view,x0,y0,side,side,_ab,_rs);
int _pc=side*side;
VC<float>bnx(_pc),anx(_pc),bny(_pc),any(_pc);
VC<float>bnz(_pc),anz(_pc),bd(_pc),ad(_pc);
VC<unsigned char>bf(_pc),af(_pc);
for(int i=0;
i<_pc;
++i){
if(_G0){
int q=(y0+i/side)*_rs+x0+i%side;
bnx[i]=(*_R0)[view].nx[q];
anx[i]=_bb[i].n[0];
bny[i]=(*_R0)[view].ny[q];
any[i]=_bb[i].n[1];
...
if(_G0){
long long nb,na;
DB bs[4]={refSsim(bnx,anx,bf,af,side,&nb),refSsim(bny,any,bf,af,side),refSsim(bnz,anz,bf,af,side),refSsim(bd,ad,bf,af,side)};
for(int i=0;
i<_pc;
++i){
anx[i]=_ab[i].n[0];
...
DB as[4]={refSsim(bnx,anx,bf,af,side,&na),refSsim(bny,any,bf,af,side),refSsim(bnz,anz,bf,af,side),refSsim(bd,ad,bf,af,side)},p[4];
long long pn=max(1LL,_gn[view]-nb+na);
for(int c=0;
c<4;
++c)p[c]=(_gs[view][c]*_gn[view]-bs[c]*nb+as[c]*na)/pn;
_tl+=.5*((p[0]+p[1]+p[2])/3.+p[3]);
_gm+=DB(nb)/max(1LL,_gn[view]);
++_ev;
continue;
}
DB _ns=(refSsim(bnx,anx,bf,af,side)+refSsim(bny,any,bf,af,side)+refSsim(bnz,anz,bf,af,side))/3.0;
DB _ds=refSsim(bd,ad,bf,af,side);
_tl+=(1.0-0.5*(_ns+_ds))*side*side;
_ev+=side*side;
}
if(_G0){
_gm/=max(1,_ev);
return _ev?1-_tl/_ev:_k;
}
return _ev?_tl/_ev:_k;
}
...
if(strategicStrike){
int _pc0=min(strategicProbeCount,(int)ca.size());
int _bi=-1;
DB _br=_k;
for(int i=0;
i<_pc0&&get<0>(ca[i])<=scoreCap;
++i){
int kept=get<1>(ca[i]);
...
k<3;
++k){
_ad0+=_fd[ZF[faceIndex].v[k]];
++_dc;
}
}
DB _ll=_P6(kept,removed,vf);
DB _avd=_ad0/max(1,_dc);
DB rank=_ll-_dw*_avd*(_G0?_gm:1);
if(rank<_br){
_br=rank;
_bi=i;
}
}
if(_bi>0)swap(ca[0],ca[_bi]);
}
VC<char>lk(nV,0);
int g=0;
