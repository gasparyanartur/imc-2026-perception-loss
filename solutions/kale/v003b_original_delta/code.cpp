...
i<pixelCount;
++i){
int q=(y0+i/side)*resolution+x0+i%side;
bnx[i]=(*_R0)[view].nx[q];
anx[i]=beforeBuffer[i].n[0];
bny[i]=(*_R0)[view].ny[q];
any[i]=beforeBuffer[i].n[1];
bnz[i]=(*_R0)[view].nz[q];
anz[i]=beforeBuffer[i].n[2];
bd[i]=(*_R0)[view].d[q];
ad[i]=beforeBuffer[i].d;
bf[i]=(*_R0)[view].fg[q];
af[i]=beforeBuffer[i].fg;
}
DB beforeSsim=(refSsim(bnx,anx,bf,af,side)+refSsim(bny,any,bf,af,side)+refSsim(bnz,anz,bf,af,side))/3.0+refSsim(bd,ad,bf,af,side);
for(int i=0;
i<pixelCount;
++i){
...
DB afterSsim=(refSsim(bnx,anx,bf,af,side)+refSsim(bny,any,bf,af,side)+refSsim(bnz,anz,bf,af,side))/3.0+refSsim(bd,ad,bf,af,side);
totalLoss+=0.5*(beforeSsim-afterSsim)*side*side;
