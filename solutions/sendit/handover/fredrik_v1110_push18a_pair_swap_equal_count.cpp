...
#define FC Face
/*
Fredrik v1110 Push 14A: exact nine-round boundary probe.

Eight sequential post-baseline single-weld rounds passed at 90.5437.
Ten, eleven, twelve, and twenty rounds failed official test 4. This version
performs exactly nine fully rebuilt and reranked single-weld rounds.

The added rounds still have no elapsed-time gate, SSIM gate, or perceptual
rollback. A round can make no change only when no structurally valid candidate
survives duplicate-face, manifold, and coverage reconstruction checks.
*/
using namespace std;
CX DB _f2=0.055;
CX DB A2=1e-12;
...
CX DB _Z1=0.00018;
CX DB _q=0.00080;
CX int hparam_T3SecondWeldMaxCandidates=1;
CX int hparam_T3SequentialSingleWeldRounds=7;
CX DB hparam_T3EndpointWeldScoreCap=.20;
CX int hparam_T3PairPatchVertexLimit=2;
struct V3 {
    DB x=0,y=0,z=0;
    CX V3()=default;
...
class _v3 {
    ...
    public:void run() {
        ...
        if(nV>5000&&nV<=50000) {
            ...
            if(inputV>25000&&inputV<=45000) {
                ...
                _e4();
                _M1();
                _U1(hparam_T3PairPatchVertexLimit,true);
                compact();
                _P5(_ov,0,hparam_T3EndpointWeldScoreCap);
                for(int round=0;round<hparam_T3SequentialSingleWeldRounds;++round)
                    _P5(_ov,hparam_T3SecondWeldMaxCandidates,hparam_T3EndpointWeldScoreCap);
            }
            _v2();
            return;
        }
    ...
    bool _G4()const {
        if(inputV<=_A1)return false;
        DB e=elapsed();
        return e>_h2&&e<min(_f1,_S);
    }
    ...
    bool _o5(int a,int b)const {
        if(_s[a]==_H||_s[b]==_H)return false;
        for(int nb:vneigh[a])if(!vdead[nb]&&_s[nb]==_H)return false;
        for(int nb:vneigh[b])if(!vdead[nb]&&_s[nb]==_H)return false;
        return true;
    }
    void _r5(int a,int b) {
        _s[a]=_s[b]=_H;
        for(int nb:vneigh[a])if(!vdead[nb])_s[nb]=_H;
        for(int nb:vneigh[b])if(!vdead[nb])_s[nb]=_H;
    }
    struct EBC {
        int a,b;
        DB cost;
        bool operator<(const EBC&o)const {
            return cost<o.cost;
        }
    };
    int _Q4() {
        ...
        for(; visited<(int)verts.size()&&scanned<_I; ++visited) {
            int a=(_y1+visited)%(int)verts.size();
            if(vdead[a])continue;
            for(int b:vneigh[a]) {
                if(scanned>=_I)break;
        ...
        _y1=(_y1+max(1,visited))%(int)verts.size();
        if(cands.empty())return 0;
        sort(cands.begin(),cands.end());
        VC<EBC>sel;
        sel.reserve(_W);
        for(auto&e:cands) {
            if((int)sel.size()>=_W)break;
            if(!_n1(e.a,e.b))continue;
            if(!_o5(e.a,e.b))continue;
            _r5(e.a,e.b);
            sel.push_back(e);
        }
        int acc=0;
        for(auto&e:sel) {
            if(_j>=B1||elapsed()>min(_f1,_S))break;
            if(!_n1(e.a,e.b))continue;
            if(_A(e.a,e.b)!=2)continue;
            if(B7(e.a,e.b)!=2)continue;
            auto best=_X(e.a,e.b);
            if(!best.valid()||(!_frag&&best.cost>costCap))continue;
            if(_s1(best)<0.99992)continue;
    ...
    void _U1(int lm=0,bool ignoreDeadline=false) {
        ...
        DB _G=ignoreDeadline?1e100:elapsed()+min(sp._K2,_c1*sp._F3);
        int _y=min(sp.hardCap,max(0,(int)floor(nV*sp._g3)));
        if(lm)_y=min(_y,lm);
        if(_y<=0)return;
        VC<B0>cands;
    ...
    bool _l1(const VC<VegaTri>&tris,int view,int x0,int y0,int w,int h,VC<_d>&buf)const {
        _d bg=_w1();
        buf.assign(w*h,bg);
    ...
    __attribute__((noinline,cold,section(".text.unlikely.hiddenedge")))int _P5(
        const VC<V3>&ov,
        int maxCandidates=0,
        DB scoreCap=hparam_T3EndpointWeldScoreCap
    ) {
        if(nV<100)return 0;
        VC<V3>sv=verts;
        fdead.assign(nF,0);
        ...
        for(int b=0; b<nV; ++b)for(int a:nb[b]) {
            int cm=0;
            ...
            for(int fi:vf[b]) {
                FC f=faces[fi];
                bool ha=false;
                for(int k=0; k<3; ++k)if(f.v[k]==a)ha=true;
                if(ha)continue;
                V3 n0=cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]);
                for(int k=0; k<3; ++k)if(f.v[k]==b)f.v[k]=a;
                V3 n1=cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]);
            ...
            if(ok) {
                ...
                for(int vv=0; vv<6; ++vv) {
                    auto pa=_n(verts[a],vv,512),pb=_n(verts[b],vv,512);
                    if(pa.ok&&pb.ok)mv=max(mv,hypot(pa.u-pb.u,pa.v-pb.v));
                }
                ca.push_back( {
                    co+.0020*mv*mv+1e-4*norm2(verts[a]-verts[b])/(diag*diag),a,b
                });
            }
        }
        ...
        VC<char>us(nV,0);
        for(auto f:nf)for(int k=0; k<3; ++k)us[f.v[k]]=1;
        VC<int>nm(nV,-1);
        VC<V3>nv;
        for(int i=0; i<nV; ++i)if(us[i])nm[i]=nv.size(),nv.push_back(verts[i]);
        for(auto&f:nf)for(int k=0; k<3; ++k)f.v[k]=nm[f.v[k]];
        ...
        for(int i=0; i<(int)nv.size(); ++i)Q[ce(nv[i])].push_back(i);
        for(auto p:ov) {
            auto c=ce(p);
            bool ok=false;
            for(int x=-1; x<=1&&!ok; ++x)for(int y=-1; y<=1&&!ok; ++y)for(int z=-1; z<=1&&!ok; ++z) {
                auto it=Q.find( {
                    c[0]+x,c[1]+y,c[2]+z
                });
