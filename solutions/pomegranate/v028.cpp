// Pomegranate v028: accepted four-pass star candidate with robust T7 margin
// geometric star-retriangulation endgame instead of edge collapse.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_Huge = 0.024;
...
void continueAbsoluteStar(int passes){screenTier=4;accepted=0;for(int i=0;i<passes&&elapsed()<HParam_TimeBudgetSeconds-1.35;++i)largeStarPass(inputV,5,1.0);compact();}
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.075:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);if(inputV>45000)continueAbsoluteStar(4);else continueAbsoluteQem(ratio);
