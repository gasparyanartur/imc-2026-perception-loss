// Pomegranate v022: whole-trajectory render selection with 4% deterministic
// QEM priority diversification in the strict 45-50k endgame.
#include <bits/stdc++.h>
...
static constexpr double HParam_PomegranateTrajectoryJitter = 0.04;
...
if(strictContinuation&&screenTier==4){uint64_t h=(uint64_t)(uint32_t)min(ab,kp)*0x9e3779b97f4a7c15ULL^(uint64_t)(uint32_t)max(ab,kp)*0xbf58476d1ce4e5b9ULL;h^=h>>30;h*=0xbf58476d1ce4e5b9ULL;h^=h>>27;double u=(double)(h&0xffffu)/65535.0-0.5;c.cost*=1.0+HParam_PomegranateTrajectoryJitter*u;} 
...
void absoluteQemEndgame(){
    ...
    double oldDiag=diag,oldHausd=hausd;int R=inputV>45000?512:1024;
