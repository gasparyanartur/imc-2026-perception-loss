// Pomegranate v012: 75% render-supported regional density at 7.5%
...
static constexpr int HParam_PomegranateDensityProfile = 4;
...
double wr=0.10,wd=0.15,wp=0.75;
...
if(screenTier==4)for(int i=0;i<nV;++i)if(!vdead[i]&&i<(int)vertexCurv.size()&&vertexCurv[i]>0){double w=0.0000060*diag*diag*sqrt(vertexCurv[i]);vquad[i]+=anchorPointQuadric(verts[i],w);}
...
if(screenTier==4&&vertexCurv.size()==(size_t)nV&&vertexArea.size()==(size_t)nV){double feature=sqrt(max(vertexCurv[ab],vertexCurv[kp])),load=0.5*(vertexArea[ab]+vertexArea[kp]);double quota=load*(1.0+0.70*feature);c.cost*=1.0+0.55*min(12.0,quota);}return c;
