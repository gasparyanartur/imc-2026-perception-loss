// Pomegranate v038: mid-tier star roots with future-valence and strong min-quality ranking
// native-1024 whole-trajectory endgame validation.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_Huge = 0.100;
...
static constexpr int HParam_PomegranateFlipProfile = 3;
static constexpr double HParam_PomegranateStarValenceWeight = 0.002;
static constexpr double HParam_PomegranateStarQualityWeight = 0.10;
...
void buildCurvatureDensity(){vertexCurv.assign(nV,0.0);vertexArea.assign(nV,0.0);vector<Vec3>fn(nF);for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi])){const Face&f=faces[fi];Vec3 n=cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]);double l=norm(n);if(l>1e-15)fn[fi]=n/l;double ar=0.5*l;for(int k=0;k<3;++k)vertexArea[f.v[k]]+=ar/3.0;}for(int a=0;a<nV;++a)if(!vdead[a])for(int b:vneigh[a])if(a<b&&!vdead[b]){int f0=-1,f1=-1;for(int fi:vfaces[a])if(!fdead[fi]&&faceHasVertex(fi,b)){if(f0<0)f0=fi;else{f1=fi;break;}}if(f1>=0){double k=1.0-fabs(dot(fn[f0],fn[f1])),w=norm(verts[a]-verts[b])*k;vertexCurv[a]+=w;vertexCurv[b]+=w;}}double m=0;int c=0;for(int i=0;i<nV;++i)if(!vdead[i]){m+=vertexCurv[i];++c;}m=m/max(1,c)+1e-15;double am=0;for(int i=0;i<nV;++i)if(!vdead[i])am+=vertexArea[i];am=am/max(1,c)+1e-15;for(double&x:vertexCurv)x=min(12.0,x/m);for(double&x:vertexArea)x=min(12.0,x/am);}
...
double maxNewDev=0.0; double minDist2=CParam_Inf; double minStarQ=1.0;
...
minDist2=min(minDist2,pointTriangleDistance2(verts[v],verts[a],verts[b],verts[c]));minStarQ=min(minStarQ,triangleQuality(verts[a],verts[b],verts[c]));
...
if(inputV>25000&&inputV<=45000){double vd=0;for(int j=0;j<m;++j){int post=vneigh[rr[j]].size()-1;if(j==0)post+=max(0,m-3);else if(j>=2&&j<=m-2)post+=1;double z=post-6;vd+=z*z;}out.score+=HParam_PomegranateStarValenceWeight*vd/max(1,m)+HParam_PomegranateStarQualityWeight*(1.0-minStarQ);}
...
void continueAbsoluteQem(double ratio){
    ...
    if(inputV>25000&&inputV<=45000)largeStarPass(inputV,5,8.0);
    compact();
}
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.075:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
