// Pomegranate v013: carried cluster-area budget with moderate saturation
...
if(screenTier==4&&vertexCurv.size()==(size_t)nV&&vertexArea.size()==(size_t)nV){double feature=sqrt(max(vertexCurv[ab],vertexCurv[kp])),load=vertexArea[ab]+vertexArea[kp];double quota=load*(1.0+0.40*feature);c.cost*=1.0+0.12*min(18.0,quota);}return c;
