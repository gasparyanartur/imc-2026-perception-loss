// Pomegranate v009: v008-layout patch density at 7.0% boundary continuation
...
static constexpr int HParam_PomegranateDensityProfile = 2;
...
double wr=HParam_PomegranateDensityProfile==1?0.55:0.35,wd=HParam_PomegranateDensityProfile==1?0.35:0.40,wp=1.0-wr-wd;
...
if(screenTier==4)for(int i=0;i<nV;++i)if(!vdead[i]&&i<(int)vertexCurv.size()&&vertexCurv[i]>0){double w=0.0000020*diag*diag*sqrt(vertexCurv[i]);vquad[i]+=anchorPointQuadric(verts[i],w);}
...
if(screenTier==4&&vertexCurv.size()==(size_t)nV&&vertexArea.size()==(size_t)nV){double feature=sqrt(max(vertexCurv[ab],vertexCurv[kp])),load=0.5*(vertexArea[ab]+vertexArea[kp]);double quota=load*(1.0+0.70*feature);c.cost*=1.0+0.22*min(12.0,quota);}return c;
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.070:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);if(inputV>45000)preconditionEdgeFlips();continueAbsoluteQem(ratio);
