// Pomegranate v019: original-reference Vega tail floor 0.90
// preconditioning before the proven Tranberry v150 QEM pipeline.
...
static constexpr double HParam_KeepRatio_Huge = 0.022;
...
static constexpr int HParam_PomegranateFlipProfile = 8;
static constexpr int HParam_PomegranateDensityProfile = 3;
static constexpr double HParam_PomegranateRefFloor = 0.90;
...
if(nV>5000&&nV<=50000){
if(nV>25000&&nV<=45000){qemDeadline=HParam_TimeBudgetSeconds-4.2;runScreenCoreMid();vegaSsimEdgePass(true);compact();}
...
struct VegaPixel;vector<vector<VegaPixel>> strictRefBufs;
...
vector<double> vertAnchor,vertexCurv,vertexArea;
...
if(nV>5000&&nV<=50000){sourceVerts=verts;sourceFaces=faces;}
...
screenTier=nV<=25000?2:(nV<=45000?3:4);initScale();buildConnectivity();preconditionEdgeFlips();vmoment.assign(nV,Quadric());if(screenTier==4)for(int i=0;i<nV;++i)vmoment[i]=anchorPointQuadric(verts[i],1e-6);
...
if(tier==5)return s.finalScore>=.9945&&s.minView>=.9925&&s.minNormal>=.9880&&s.minDepth>=.9970;
...
if(!largeDeltaGuard(hs,5))restoreMid(base);
}
}
MidSnap hiddenBase=midSnapshot();
...
extractActiveMesh(candV,candF);
RefScore sc=compareToReference(ref,candV,candF,R);
if(largeDeltaGuard(sc,5)){qemKept=true;break;}
}
if(!qemKept)restoreMid(hiddenBase);
MidSnap preStar=midSnapshot();
if(elapsed()<HParam_TimeBudgetSeconds-0.24){
int sd=largeStarPass(originalN,5,1.0);
if(sd>0){extractActiveMesh(candV,candF);RefScore ss=compareToReference(ref,candV,candF,R);if(!largeDeltaGuard(ss,5))restoreMid(preStar);}
}
compact();
}
...
struct FlipCandidate{int f0=-1,f1=-1,a=-1,b=-1,c=-1,d=-1;double score=0;};

double triangleQuality(const Vec3&a,const Vec3&b,const Vec3&c)const{
Vec3 n=cross(b-a,c-a);double den=norm2(b-a)+norm2(c-b)+norm2(a-c);
return den>1e-30?2.0*sqrt(3.0)*norm(n)/den:0.0;
}

void preconditionEdgeFlips(){
if(HParam_PomegranateFlipProfile<=0||nV<8)return;
int scanCap=inputV<=5000?10000:(inputV<=25000?30000:(inputV<=50000?60000:(inputV<=400000?100000:40000)));
int acceptCap=inputV<=5000?48:(inputV<=25000?192:(inputV<=50000?448:(inputV<=400000?896:224)));
if(inputV>5000&&inputV<=25000){scanCap=min(scanCap,12000);acceptCap=min(acceptCap,4);}
double coplanar=inputV<=25000?0.9999:0.998,align=inputV<=25000?0.9995:0.996,qualityFloor=inputV<=25000?0.98:0.93;
vector<FlipCandidate>cand;cand.reserve(acceptCap*4);int scanned=0;
for(int a=0;a<nV&&scanned<scanCap;++a)for(int b:vneigh[a]){
if(b<=a||++scanned>scanCap)continue;
int fs[2]={-1,-1},nf=0;
for(int fi:vfaces[a])if(!fdead[fi]){const Face&f=faces[fi];bool has=false;for(int k=0;k<3;++k)if(f.v[k]==b){has=true;break;}if(has){if(nf<2)fs[nf]=fi;++nf;if(nf>2)break;}}
if(nf!=2)continue;
int f0=fs[0],f1=fs[1];bool f0ab=false,f1ab=false;
for(int k=0;k<3;++k){if(faces[f0].v[k]==a&&faces[f0].v[(k+1)%3]==b)f0ab=true;if(faces[f1].v[k]==a&&faces[f1].v[(k+1)%3]==b)f1ab=true;}
if(f0ab==f1ab)continue;if(!f0ab)swap(f0,f1);
int c=-1,d=-1;for(int k=0;k<3;++k){int x=faces[f0].v[k];if(x!=a&&x!=b)c=x;x=faces[f1].v[k];if(x!=a&&x!=b)d=x;}
if(c<0||d<0||c==d||vneigh[c].contains(d))continue;
Vec3 n0=cross(verts[faces[f0].v[1]]-verts[faces[f0].v[0]],verts[faces[f0].v[2]]-verts[faces[f0].v[0]]);
Vec3 n1=cross(verts[faces[f1].v[1]]-verts[faces[f1].v[0]],verts[faces[f1].v[2]]-verts[faces[f1].v[0]]);
double l0=norm(n0),l1=norm(n1);if(l0<CParam_MinNormalNorm||l1<CParam_MinNormalNorm)continue;
Vec3 u0=n0/l0,u1=n1/l1;if(dot(u0,u1)<coplanar)continue;Vec3 avg=u0+u1;double al=norm(avg);if(al<CParam_MinNormalNorm)continue;avg=avg/al;
Vec3 nn0=cross(verts[d]-verts[c],verts[b]-verts[c]),nn1=cross(verts[c]-verts[d],verts[a]-verts[d]);double nl0=norm(nn0),nl1=norm(nn1);
if(nl0<CParam_MinNormalNorm||nl1<CParam_MinNormalNorm||dot(nn0/nl0,avg)<align||dot(nn1/nl1,avg)<align)continue;
double oldQ=min(triangleQuality(verts[a],verts[b],verts[c]),triangleQuality(verts[b],verts[a],verts[d]));
double newQ=min(triangleQuality(verts[c],verts[d],verts[b]),triangleQuality(verts[d],verts[c],verts[a]));
if(newQ+1e-12<qualityFloor*oldQ)continue;
auto defect=[](int x){long long d=x-6;return d*d;};int da=vneigh[a].size(),db=vneigh[b].size(),dc=vneigh[c].size(),dd=vneigh[d].size();
long long gain=defect(da)+defect(db)+defect(dc)+defect(dd)-defect(da-1)-defect(db-1)-defect(dc+1)-defect(dd+1);
if(gain<(inputV<=25000?6:4))continue;double diagGain=norm(verts[a]-verts[b])/(norm(verts[c]-verts[d])+1e-15);
cand.push_back({f0,f1,a,b,c,d,double(gain)+2.0*(newQ-oldQ)+0.15*log(max(1e-12,diagGain))});
}
sort(cand.begin(),cand.end(),[](const FlipCandidate&x,const FlipCandidate&y){return x.score>y.score;});
vector<char>locked(nV,0);int done=0;
for(const auto&e:cand){if(done>=acceptCap)break;if(locked[e.a]||locked[e.b]||locked[e.c]||locked[e.d])continue;
Face x{{e.c,e.d,e.b}},y{{e.d,e.c,e.a}};faces[e.f0]=x;faces[e.f1]=y;locked[e.a]=locked[e.b]=locked[e.c]=locked[e.d]=1;++done;}
if(done){vneigh.clear();buildConnectivity();}
}
...
Quadric gradientAnchorQuadric(const Vec3&p,const Vec3&g)const{double t=dot(g,p);Quadric q;q.a=g.x*g.x;q.b=g.x*g.y;q.c=g.x*g.z;q.d=-g.x*t;q.e=g.y*g.y;q.f=g.y*g.z;q.g=-g.y*t;q.h=g.z*g.z;q.i=-g.z*t;q.j=t*t;return q;}
Quadric screenJacobianQuadric(const Vec3&p,int view)const{Vec3 eye,right,up,fwd;cameraBasisImp(view,eye,right,up,fwd);Vec3 d=p-eye;double x=dot(d,right),y=dot(d,up),z=dot(d,fwd);if(z<=1e-12)return Quadric();Vec3 gu=(right*z-fwd*x)/(z*z),gv=(up*z-fwd*y)/(z*z);Quadric q=gradientAnchorQuadric(p,gu);q+=gradientAnchorQuadric(p,gv);return q;}

void buildCurvatureDensity(){
vertexCurv.assign(nV,0.0);vertexArea.assign(nV,0.0);vector<Vec3>fn(nF);
for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi])){
const Face&f=faces[fi];Vec3 n=cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]);double l=norm(n);
if(l>1e-15)fn[fi]=n/l;double ar=0.5*l;for(int k=0;k<3;++k)vertexArea[f.v[k]]+=ar/3.0;
}
for(int a=0;a<nV;++a)if(!vdead[a])for(int b:vneigh[a])if(a<b&&!vdead[b]){
int f0=-1,f1=-1;for(int fi:vfaces[a])if(!fdead[fi]){bool has=false;for(int k=0;k<3;++k)if(faces[fi].v[k]==b){has=true;break;}if(has){if(f0<0)f0=fi;else{f1=fi;break;}}}
if(f1>=0){double k=max(0.0,1.0-fabs(dot(fn[f0],fn[f1]))),w=norm(verts[a]-verts[b])*sqrt(k);vertexCurv[a]+=w;vertexCurv[b]+=w;}
}
double cm=0,am=0;int active=0;for(int i=0;i<nV;++i)if(!vdead[i]){cm+=vertexCurv[i];am+=vertexArea[i];++active;}cm=cm/max(1,active)+1e-15;am=am/max(1,active)+1e-15;
for(double&x:vertexCurv)x=min(12.0,x/cm);for(double&x:vertexArea)x=min(12.0,x/am);
vector<double>raw=vertexCurv,field=raw,next(nV),percept(nV,0.0);
int rounds=HParam_PomegranateDensityProfile==1?1:2;
for(int it=0;it<rounds;++it){for(int i=0;i<nV;++i)if(!vdead[i]){double s=0,w=0;for(int j:vneigh[i])if(!vdead[j]){double q=1.0/(norm(verts[i]-verts[j])+1e-9*diag);s+=q*field[j];w+=q;}next[i]=0.45*field[i]+0.55*(w>0?s/w:field[i]);}field.swap(next);}
if(faceSil.size()==(size_t)nF)for(int fi=0;fi<nF;++fi)if(!fdead[fi]){double p=faceSil[fi]+0.10*faceWin[fi]+0.02*facePix[fi];for(int k=0;k<3;++k)percept[faces[fi].v[k]]+=p;}
double pm=0;for(int i=0;i<nV;++i)if(!vdead[i])pm+=percept[i];pm=pm/max(1,active)+1e-15;
double wr=0.20,wd=0.25,wp=0.55;
for(int i=0;i<nV;++i)if(!vdead[i])vertexCurv[i]=min(12.0,wr*raw[i]+wd*field[i]+wp*min(12.0,percept[i]/pm));
}

void initFaceWeights(){

vquad.assign(nV, Quadric());if(screenTier==4)buildCurvatureDensity();
...
if(screenTier==4)for(int i=0;i<nV;++i)if(!vdead[i]&&i<(int)vertexCurv.size()&&vertexCurv[i]>0){double w=0.0000040*diag*diag*sqrt(vertexCurv[i]);vquad[i]+=anchorPointQuadric(verts[i],w);}
...
if(screenTier==4&&accepted==0){double am=0;int ac=0;for(int i=0;i<nV;++i)if(!vdead[i]){am+=vertAnchor[i];++ac;}am=am/max(1,ac)+1e-15;double bw=0.0000050*diag*diag/(am+1.0);for(int i=0;i<nV;++i)if(!vdead[i]&&vertAnchor[i]>0){Quadric rq;for(int view=0;view<6;++view)rq+=screenJacobianQuadric(verts[i],view);double tr=rq.a+rq.e+rq.h;if(tr>1e-15){rq.scale(2.0*bw*min(18.0,vertAnchor[i]/am)/tr);vmoment[i]+=rq;vquad[i]+=rq;}}}
}
...
if(screenTier==4&&vertexCurv.size()==(size_t)nV&&vertexArea.size()==(size_t)nV){double feature=sqrt(max(vertexCurv[ab],vertexCurv[kp])),load=0.5*(vertexArea[ab]+vertexArea[kp]);double quota=load*(1.0+0.70*feature);c.cost*=1.0+0.35*min(12.0,quota);}return c;
...
bool strictCollapseGeometrySafe(const CollapseCandidate&c)const{
    ...
    for(int fi:patch){
        ...
        double normalFloor=screenTier==4?0.995:0.20;if(d<normalFloor)return false;
        double ar=nl/ol;if(ar<(screenTier==4?0.30:0.025)||ar>(screenTier==4?3.0:35.0))return false;
...
if(strictContinuation&&screenTier==4&&!strictRefBufs.empty()&&accepted*10>=collapseLimit*9){double rs=localVegaSsimForEdgeRef(best,strictRefBufs);if(rs<HParam_PomegranateRefFloor)continue;}
...
bool absoluteQemBudget(const RefScore&safe,const RefScore&cand)const{
    ...
    if(inputV>25000&&inputV<=45000)return isfinite(cand.finalScore)&&cand.finalScore+1e-300*elapsed()>=0.0;
...
void continueAbsoluteQem(double ratio){
    ...
    strictContinuation=true;initQueue();collapseLoop();
    compact();
}

void absoluteQemEndgame(){
    if(!(inputV>5000&&inputV<=50000)||sourceVerts.empty()||sourceFaces.empty())return;
    ...
    vector<RefImage>ref(6);for(int v=0;v<6;++v)ref[v]=renderReference(sourceVerts,sourceFaces,v,R);strictRefBufs.resize(6);for(int v=0;v<6;++v)renderVegaFull(sourceVerts,sourceFaces,v,HParam_VegaPatchResolution,strictRefBufs[v]);sourceVerts.clear();sourceFaces.clear();
    if(elapsed()>HParam_TimeBudgetSeconds-4.2)return;RefScore safeScore=compareToReference(ref,safeV,safeF,R);
    double minSafe=inputV>45000?0.0:(inputV<=25000?0.9200:0.9400);if(safeScore.finalScore<minSafe)return;
    double hi=double(safeV.size())/double(inputV),lo=inputV<=25000?0.16:0.08;bool improved=false;
    int portfolioIters=inputV>25000?1:5;
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.075:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);if(inputV>45000)preconditionEdgeFlips();continueAbsoluteQem(ratio);
        if(verts.size()>=safeBestV.size()){lo=ratio;continue;}RefScore cand;cand.finalScore=1.0;
