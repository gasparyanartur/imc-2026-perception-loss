// Pomegranate v044: four weak-valence mid-tier star rounds
...
if(inputV>25000&&inputV<=45000){double vd=0;for(int j=0;j<m;++j){int post=vneigh[rr[j]].size()-1;if(j==0)post+=max(0,m-3);else if(j>=2&&j<=m-2)post+=1;double z=post-6;vd+=z*z;}out.score+=HParam_PomegranateStarValenceWeight*vd/max(1,m);}
...
void continueAbsoluteQem(double ratio){
    ...
    if(inputV>25000&&inputV<=45000){largeStarPass(inputV,5,8.0);largeStarPass(inputV,5,8.0);largeStarPass(inputV,5,8.0);largeStarPass(inputV,5,8.0);}
    compact();
}
