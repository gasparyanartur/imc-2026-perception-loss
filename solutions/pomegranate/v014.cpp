// Pomegranate v014: carried cluster-area budget with strong feature saturation
...
if(screenTier==4&&vertexCurv.size()==(size_t)nV&&vertexArea.size()==(size_t)nV){double feature=sqrt(max(vertexCurv[ab],vertexCurv[kp])),load=vertexArea[ab]+vertexArea[kp];double quota=load*(1.0+0.70*feature);c.cost*=1.0+0.22*min(18.0,quota);}return c;
...
if(screenTier==4&&vertexArea.size()==(size_t)nV&&vertexCurv.size()==(size_t)nV){double aa=vertexArea[ab],ak=vertexArea[kp],at=aa+ak;if(at>1e-15)vertexCurv[kp]=(aa*vertexCurv[ab]+ak*vertexCurv[kp])/at;vertexArea[kp]=at;vertexArea[ab]=0.0;vertexCurv[ab]=0.0;}
