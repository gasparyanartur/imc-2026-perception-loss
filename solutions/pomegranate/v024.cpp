// Pomegranate v024: canonical all-pass v003 scaffold with a 7.0%
// native-1024 whole-trajectory endgame and a stricter render-loss budget.
#include <bits/stdc++.h>
...
bool absoluteQemBudget(const RefScore&safe,const RefScore&cand)const{
    ...
    double dropF=inputV<=25000?0.0085:(inputV<=45000?0.0085:0.0025);
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.070:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
