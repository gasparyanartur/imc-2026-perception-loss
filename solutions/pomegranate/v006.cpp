// Pomegranate v006: broad safe-snapshot flips plus 4% boundary continuation.
...
static constexpr int HParam_PomegranateFlipProfile = 6;
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.040:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);if(inputV>45000)preconditionEdgeFlips();continueAbsoluteQem(ratio);
