// Kumquat v14 aggressive
// profile: growth=20deg, turn=20deg, keep=0, minInterior=20, selection=exact-max-removal
#include <algorithm>
#include <bits/stdc++.h>
#include <numeric>
using namespace std;
...
void run() {
 ...
 case 4:
  initScaleTargets();
  runPatchFirst();
#ifdef KUMQUAT_PATCH_ONLY
  break;
#endif
  setStopReserve(hp_boot_t3_reserve);
  boot_qem();
  compact();
  tx_final();
  break;
...
void runPatchFirst() {
 ...
 int totalRemoved = 0;
 const int rn = (int)replacements.size();
 vector<vector<int>> touched(rn);
 for (int i = 0; i < rn; ++i) {
  const vector<int> &cf = replacements[i].conflictFaces.empty()
       ? replacements[i].oldFaces : replacements[i].conflictFaces;
  for (int fi : cf) if (fi >= 0 && fi < (int)baseF.size())
   for (int k = 0; k < 3; ++k) touched[i].push_back(baseF[fi].v[k]);
  sort(touched[i].begin(), touched[i].end());
  touched[i].erase(unique(touched[i].begin(), touched[i].end()), touched[i].end());
 }
 vector<unsigned int> conflict(rn, 0u);
 for (int i = 0; i < rn; ++i) for (int j = i + 1; j < rn; ++j) {
  size_t a = 0, b = 0; bool hit = false;
  while (a < touched[i].size() && b < touched[j].size()) {
   if (touched[i][a] == touched[j][b]) { hit = true; break; }
   if (touched[i][a] < touched[j][b]) ++a; else ++b;
  }
  if (hit) { conflict[i] |= 1u << j; conflict[j] |= 1u << i; }
 }
 unsigned int bestMask = 0u;
 int bestRemoved = -1, bestCount = INT_MAX;
 const unsigned int limit = 1u << rn;
 for (unsigned int mask = 1u; mask < limit; ++mask) {
  int count = __builtin_popcount(mask);
  if (count > v7.maxBatch) continue;
  bool ok = true; int sum = 0;
  for (int i = 0; i < rn && ok; ++i) if (mask & (1u << i)) {
   if (conflict[i] & mask) ok = false;
   else sum += replacements[i].removed;
  }
  if (ok && (sum > bestRemoved || (sum == bestRemoved && count < bestCount))) {
   bestRemoved = sum; bestCount = count; bestMask = mask;
  }
 }
 for (int i = 0; i < rn; ++i) if (bestMask & (1u << i)) {
  selected.push_back(replacements[i]);
  totalRemoved += replacements[i].removed;
 }
