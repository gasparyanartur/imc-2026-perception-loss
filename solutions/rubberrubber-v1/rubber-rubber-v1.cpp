#include <bits/stdc++.h>
using namespace std;
constexpr double _f2=0.055;
constexpr double A2=1e-12;
constexpr double _X1=1e-12;
constexpr double _k=1e100;
constexpr double A0=20.2;
constexpr int B3=10;
constexpr double _N2=0.00;
constexpr double _F2=0.00;
constexpr double _G2=0.00;
constexpr double _H2=0.00;
constexpr double _B2=0.0237;
constexpr double _c3=0.019912109375;
constexpr double _J1=0.0330;
constexpr bool _Y2=true;
constexpr int _P=1;
constexpr double _u3=3.;
constexpr double _j3=3.;
constexpr bool A9=true;
constexpr int B8=512;
constexpr int _a2=4;
constexpr int _C2=52000;
constexpr int _g1=28000;
constexpr double _a1=0.55;
constexpr double _w2=0.0018;
constexpr double _i2=(0.01*255.)*(0.01*255.);
constexpr double _j2=(0.03*255.)*(0.03*255.);
constexpr bool _L1=true;
constexpr int B5=512;
constexpr int _q2=176;
constexpr int _g2=96;
constexpr int _t1=112;
constexpr int B2=320;
constexpr int _G1=48;
constexpr int _H1=72;
constexpr int _I1=76000;
constexpr double _Y1=0.00024;
constexpr double _Z1=0.00018;
constexpr double _q=0.00080;
constexpr double hparam_T3EndpointWeldScoreCap=.20;
struct Vec3{
double x=0,y=0,z=0;
...
static Quadric _T1(const Vec3&p0,const Vec3&p1,const Vec3&p2){
...
bool _X4(const Quadric&q,Vec3&out){
double D=det3(q.a,q.b,q.c,q.b,q.e,q.f,q.c,q.f,q.h);
if(fabs(D)<_X1)return false;
out=Vec3(det3(-q.d,q.b,q.c,-q.g,q.e,q.f,-q.i,q.f,q.h)/D,det3(q.a,-q.d,q.c,q.b,-q.g,q.f,q.c,-q.i,q.h)/D,det3(q.a,q.b,-q.d,q.b,q.e,-q.g,q.c,q.f,-q.i)/D);
return _N4(out);
}
struct A1{
...
_P5(_ov,0,.20);
_P5(_ov,1,.20);
vector<_u1>originalReference(6);
for(int view=0;
view<6;
...
double strategicDebtWeight=1e-5;
int IV=0,inputF=0;
vector<Vec3>ZV;
vector<Face>ZF;
vector<Vec3>_U;
...
vector<double>_b,_fd;
vector<vector<vector<int>>>_d3;
int _b4=0;
static constexpr int _V=32;
int _a=0,_frag=0;
double B6=0,_M4=0;
double _S=A0;
...
int _o1=0,_O1=0;
double diag=0,hausd=0,costCap=_k;
double _D4=0.;
...
void _q4(int view,Vec3&eye,Vec3&right,Vec3&up,Vec3&fwd)const{
Vec3 dirs[6]={
{
1,0,0}
,{
...
double fl=norm(fwd);
fwd=fwd/fl;
Vec3 wu=_oc?((fabs(dot(fwd,Vec3(0,1,0)))>.9)?Vec3(0,0,1):Vec3(0,1,0)):((fabs(dot(fwd,Vec3(0,0,1)))>.9)?Vec3(0,1,0):Vec3(0,0,1));
right=cross(wu,fwd);
...
;
ImpProj _n(const Vec3&p,int view,int R)const{
Vec3 e,r,u,f;
_q4(view,e,r,u,f);
Vec3 q=p-e;
...
auto&f=ZF[fi];
Vec3 n=cross(ZV[f.v[1]]-ZV[f.v[0]],ZV[f.v[2]]-ZV[f.v[0]]);
double l=norm(n);
...
vector<double>st=_a==2?vector<double>{
0.36,0.33,0.30}
:vector<double>{
0.14,0.10,0.08}
;
...
;
_u1 _x(const vector<Vec3>&vv,const vector<Face>&ff,int view,int R,vector<int>*oi=nullptr)const{
_u1 im(R);
...
x<=x1;
++x){
double X=x+.5,Y=y+.5,w0=((p1 .v-p2 .v)*(X-p2 .u)+(p2 .u-p1 .u)*(Y-p2 .v))/den,w1=((p2 .v-p0 .v)*(X-p2 .u)+(p0 .u-p2 .u)*(Y-p2 .v))/den,w2=1-w0-w1;
if(w0<-1e-9||w1<-1e-9||w2<-1e-9)continue;
...
double refSsim(const vector<float>&a,const vector<float>&b,const vector<unsigned char>&fa,const vector<unsigned char>&fb,int R)const{
vector<double>x(R),y(R),xx(R),yy(R),xy(R);
auto row=[&](int r,double s){
int q=r*R;
for(int i=0;
i<R;
++i){
double A=a[q+i],B=b[q+i];
x[i]+=s*A;
y[i]+=s*B;
xx[i]+=s*A*A;
yy[i]+=s*B*B;
...
r<11;
++r)row(r,1);
double T=0,iv=1./121.,C1=6.5025,C2=58.5225;
long long n=0;
for(int v=5;
v<R-5;
++v){
...
int q=v*R+u;
if(fa[q]||fb[q]){
double A=X*iv,B=Y*iv,V=max(0.,XX*iv-A*A),W=max(0.,YY*iv-B*B),K=XY*iv-A*B,D=(A*A+B*B+C1)*(V+W+C2);
...
struct _l{
double _f=0,minView=1,_w=1,_R=1;
}
;
_l A7(const vector<_u1>&ref,const vector<Vec3>&vv,const vector<Face>&ff,int R,bool debt=false){
_l o;
...
double sn=(refSsim(ref[v].nx,b.nx,ref[v].fg,b.fg,R)+refSsim(ref[v].ny,b.ny,ref[v].fg,b.fg,R)+refSsim(ref[v].nz,b.nz,ref[v].fg,b.fg,R))/3.,sd=refSsim(ref[v].d,b.d,ref[v].fg,b.fg,R),cv=.5*(sn+sd);
o._f+=cv;
...
if(debt){
double m=0;
for(double x:_fd)m+=x;
m=m/max(1,(int)_fd.size())+1e-12;
for(double&x:_fd)x=max(0.,x/m-.65);
}
return o;
}
void A3(vector<Vec3>&vv,vector<Face>&ff)const{
vector<int>m(ZV.size(),-1);
vv.clear();
ff.clear();
vv.reserve(ZV.size());
for(int i=0;
...
void _A2(vector<double>&rr)const{
rr.clear();
rr.reserve(ZV.size());
for(int i=0;
i<(int)ZV.size();
...
void _U2(int tier,int _r,double ratio,int _N1){
if(tier==3)B9(512);
...
MidSnap safe=_K();
vector<Vec3>safeV,candV;
vector<Face>safeF,candF;
A3(safeV,safeF);
int R=1024;
if(IV>25000&&ET()>A0-4.){
compact();
...
_g();
MidSnap _s5=_K();
vector<Vec3>baseV,safeV,candV;
vector<Face>baseF,safeF,candF;
A3(baseV,baseF);
int _N1=0,R=1024;
if(ET()<A0-4.8){
_N1=_O();
...
v<6;
++v)ref[v]=_x(safeV,safeF,v,R);
vector<double>tries=tier==3?vector<double>{
...
int _B3(int _r,int tier,double scale=1.){
_c sp=tier==5?_c{
...
double _G=ET()+min(sp._K2,_c1*sp._F3);
...
i<(int)ZD.size();
++i)if(!ZD[i])++_J4;
double _j4=double(_J4)/double(_r);
double _L3=0.0155,cut=0.18;
vector<double>tries={
max(_L3,_j4*(1.-cut)),max(_L3,_j4*(1.-0.55*cut)),max(_L3,_j4*0.97)}
;
sort(tries.begin(),tries.end());
tries.erase(unique(tries.begin(),tries.end(),[](double a,double b){
...
double _A3(const Vec3&_R3,double area)const{
double absSum=fabs(_R3 .x)+fabs(_R3 .y)+fabs(_R3 .z);
double _r4=area*_D4;
double w=1.+_u3*_r4*absSum;
return min(w,_j3);
}
Quadric _x2(const Vec3&p,double w)const{
Quadric q;
q.a=q.e=q.h=w;
q.d=-w*p.x;
q.g=-w*p.y;
...
Quadric q=Quadric::_T1(ZV[f.v[0]],ZV[f.v[1]],ZV[f.v[2]]);
if(_a){
double imp=_a==3?(facePix[fi]+10.*faceSil[fi]):(facePix[fi]+12.*faceSil[fi]+2.*faceWin[fi]);
double r=_a==3?(imp+0.20*B6)/(B6+1e-12):(imp+0.12*B6)/(B6+1e-12);
double w;
...
_M4=am;
double bw=(_a==2?0.0000035:0.0000050)*diag*diag/(am+1.);
for(int i=0;
i<nV;
...
double imp=facePix[fi]+12.*faceSil[fi]+2.*faceWin[fi];
double r=(imp+0.12*B6)/(B6+1e-12);
double w=_a==2?min(18.,max(0.06,0.08+0.92*pow(max(0.,r),0.90))):min(16.,max(0.06,0.08+0.92*pow(max(0.,r),0.72)));
if(w!=1.)q.scale(w);
}
else if(area>=1e-30){
n=n/(2.*area);
...
void _k1(int a,int b,const Quadric&q,Vec3 pos[9],int&np)const{
np=0;
Vec3 qp;
...
double el=norm(ed);
if(el>1e-30){
double t=max(0.,min(1.,0.5*(1.+(crad[b]-crad[a])/el)));
pos[np++]=ZV[a]+ed*t;
}
}
if(_frag&&_a==2){
...
double x=1+min(12.,_b[a]/(_M4+1e-12)),y=1+min(12.,_b[b]/(_M4+1e-12));
...
A1 _n2(int ab,int kp,const Vec3&p,const Quadric&q)const{
A1 c;
c._o=ab;
c.kept=kp;
c._i3=vver[ab];
...
double a=min(12.,_b[ab]/(_M4+1e-12)),b=min(12.,_b[kp]/(_M4+1e-12)),d=max(0.,a-b),mv=norm2(p-ZV[kp])*_D4;
uint32_t h=ab*73856093u^kp*19349663u^targetV*83492791u;
double j=(h&1023)/1023.-.5;
c.cost=r*(1+.62*d+.035*j)/(1+.16*max(0.,1.-a))+costCap*(.0011*a+.075*b*mv);
return c;
}
double _O2(int a,int b)const{
Quadric q=vquad[a];
q+=vquad[b];
Vec3 mid=(ZV[a]+ZV[b])*0.5;
return q._b5(mid);
}
...
bool _l3(int a,int b,const Vec3&p,double&mr)const{
...
pq=priority_queue<A1>(less<A1>(),std::move(h));
}
void _s2(){
for(int a=0;
a<nV;
...
double ET()const{
return chrono::duration<double>(chrono::steady_clock::now()-_T4).count();
}
...
static void _B4(vector<int>&v,int x){
...
void _D1(int ab,int kp,const Vec3&np,double nr){
if(_frag&&_a==2){
...
static double _v1(double x,double lo,double hi){
return x<lo?lo:(x>hi?hi:x);
}
static double _z(const Vec3&p,const Vec3&a,const Vec3&b,const Vec3&c){
...
bool _R2(const A1&c,vector<int>&patch,vector<array<Vec3,3>>&oldTris,vector<array<Vec3,3>>&newTris)const{
int a=c._o,b=c.kept;
patch.clear();
for(int fi:ZI[a])if(!FD[fi])patch.push_back(fi);
for(int fi:ZI[b])if(!FD[fi])patch.push_back(fi);
...
auto worst=[&](const vector<array<Vec3,3>>&A,const vector<array<Vec3,3>>&B){
double w=0;
for(auto&t:A){
Vec3 s[7]={
...
;
double w2=max(worst(oldTris,newTris),worst(newTris,oldTris));
return isfinite(w2)&&w2<=(0.72*hausd)*(0.72*hausd);
}
bool _O3(const _m3&h,const A1&c,const vector<int>&patch)const{
...
double s=(_O2(a,b)+1e-5*norm2(ZV[a]-ZV[b]))*0.7;
cand.push_back({
a,b,s}
);
}
sort(cand.begin(),cand.end());
double frac=tier<=4?0.12:(tier==5?0.035:0.018);
...
bool A8(int v,const _c&sp,vector<int>&ring,vector<int>&inc)const{
ring.clear();
inc.clear();
...
bool _S1(int a,int b,int c,const vector<int>&skip)const{
array<int,3>key={
a,b,c}
;
sort(key.begin(),key.end());
...
;
bool _p1(int v,const vector<int>&ring,const vector<int>&inc,int root,double oldDev,const Vec3&avgN,const _c&sp,A4&out)const{
int m=(int)ring.size();
vector<int>rr;
rr.reserve(m);
for(int i=0;
...
Vec3 n=cross(ZV[b]-ZV[a],ZV[c]-ZV[a]);
double nl=norm(n);
if(nl<A2)return false;
Vec3 un=n/nl;
double d=_v1(dot(un,avgN),-1.,1.);
if(d<=0.)return false;
_N=max(_N,1.-d);
if(_N>sp._N)return false;
_E4=min(_E4,_z(ZV[v],ZV[a],ZV[b],ZV[c]));
}
double dist=sqrt(max(0.,_E4));
...
struct _C1{
double frac=0.,_W4=0.,_j5=0.,minDot=1.01;
int _V4=0;
}
;
_C1 _g4()const{
...
bool _G3(int root,const Vec3&target,const _C1&rp){
...
if(FD[fi])continue;
const Face&f=ZF[fi];
Vec3 oldN=cross(ZV[f.v[1]]-ZV[f.v[0]],ZV[f.v[2]]-ZV[f.v[0]]);
...
double d=_v1(dot(oldN/oldLen,newN/newLen),-1.,1.);
...
bool _D3(const vector<pair<int,int>>&edges,vector<int>&ring)const{
ring.clear();
...
double _W1(const vector<array<Vec3,3>>&src,const vector<array<Vec3,3>>&dst)const{
...
Vec3 n=cross(ZV[y]-ZV[x],ZV[z]-ZV[x]);
double nl=norm(n);
if(nl<A2){
ok=false;
break;
...
double worst2=max(_W1(oldTris,newTris),_W1(newTris,oldTris));
if(!isfinite(worst2))continue;
double worst=sqrt(max(0.,worst2));
double _U3=max(crad[a],crad[b])+worst;
...
vector<int>na=ZN[fresh.a].data,nb=ZN[fresh.b].data;
...
double _G=ignoreDeadline?1e100:ET()+min(sp._K2,_c1*sp._F3);
int _y=min(sp.hardCap,max(0,(int)floor(nV*sp._g3)));
if(lm)_y=min(_y,lm);
if(_y<=0)return;
vector<B0>cands;
...
void _n4(int view,Vec3&eye,Vec3&right,Vec3&up,Vec3&fwd)const{
switch(view){
case 0:eye=Vec3(2.5,0,0);
break;
case 1:eye=Vec3(-2.5,0,0);
break;
case 2:eye=Vec3(0,2.5,0);
break;
case 3:eye=Vec3(0,-2.5,0);
break;
case 4:eye=Vec3(0,0,2.5);
break;
default:eye=Vec3(0,0,-2.5);
break;
}
fwd=eye*(-1.);
double fl=norm(fwd);
if(fl<A2)fwd=Vec3(0,0,-1);
else fwd=fwd/fl;
Vec3 worldUp=(fabs(dot(fwd,Vec3(0,0,1)))>0.9)?Vec3(0,1,0):Vec3(0,0,1);
right=cross(worldUp,fwd);
double rl=norm(right);
if(rl<A2)right=Vec3(1,0,0);
else right=right/rl;
up=cross(fwd,right);
double ul=norm(up);
if(ul<A2)up=Vec3(0,1,0);
else up=up/ul;
}
_P1 A6(const Vec3&p,int view,int R)const{
...
bool _Z2(int v,int root,const _c&sp,vector<VegaTri>&oldTris,vector<VegaTri>&newTris)const{
oldTris.clear();
newTris.clear();
vector<int>ring,inc;
if(!A8(v,sp,ring,inc))return false;
...
double _e1(const vector<VegaTri>&src,const vector<VegaTri>&dst)const{
if(src.empty()||dst.empty())return _k;
double worst=0.;
...
double _d1(const vector<VegaTri>&oldTris,const vector<VegaTri>&newTris)const{
double d2=max(_e1(oldTris,newTris),_e1(newTris,oldTris));
if(!isfinite(d2))return _k;
return sqrt(max(0.,d2));
}
static double _C(const _d&p,int ch){
if(ch<3)return p.n[ch];
return p.d;
}
double _R1(const vector<_d>&a,const vector<_d>&b,int ch)const{
...
double x=_C(a[i],ch),y=_C(b[i],ch);
sx+=x;
sy+=y;
sxx+=x*x;
syy+=y*y;
...
if(cnt<4)return 1.;
double inv=1./double(cnt);
double mx=sx*inv,my=sy*inv;
double vx=max(0.,sxx*inv-mx*mx),vy=max(0.,syy*inv-my*my);
double cov=sxy*inv-mx*my;
double num=(2.*mx*my+_i2)*(2.*cov+_j2);
double den=(mx*mx+my*my+_i2)*(vx+vy+_j2);
if(den<=0.)return 1.;
return _v1(num/den,-1.,1.);
}
double _u2(const vector<_d>&a,const vector<_d>&b)const{
int cnt=0;
for(int i=0;
i<(int)a.size();
++i)if(a[i].fg||b[i].fg)++cnt;
if(cnt==0)return 1.;
double sn=(_R1(a,b,0)+_R1(a,b,1)+_R1(a,b,2))/3.;
double sd=_R1(a,b,3);
return _a1*sn+(1.-_a1)*sd;
}
bool _l1(const vector<VegaTri>&tris,int view,int x0,int y0,int w,int h,vector<_d>&buf,int R=B8)const{
_d bg=_w1();
buf.assign(w*h,bg);
...
void _Q1(const vector<Vec3>&vv,const vector<Face>&ff,int view,int R,vector<_d>&buf)const{
_d bg=_w1();
buf.assign(R*R,bg);
...
double _E1(const A1&c,const vector<vector<_d>>&refBufs)const{
vector<int>patch=ZI[c._o];
for(int fi:ZI[c.kept])if(find(patch.begin(),patch.end(),fi)==patch.end())patch.push_back(fi);
vector<VegaTri>ot,nt;
ot.reserve(patch.size());
...
a.clear();
a.reserve(w*h);
const vector<_d>&ref=refBufs[view];
for(int py=y0;
py<=y1;
++py)for(int px=x0;
px<=x1;
...
double stop=ET()+min(2.,left*0.42);
bool _N3=!_U.empty()&&!_T.empty();
vector<vector<_d>>refBufs;
if(_N3){
refBufs.resize(6);
for(int view=0;
view<6;
...
double s=_N3?_E1(c,refBufs):_s1(c);
...
static double _r2(const vector<double>&p,int S,const _m&r){
if(!r.valid())return 0.;
int x0=r.x0,y0=r.y0,x1=r.x1+1,y1=r.y1+1;
return p[(size_t)y1*S+x1]-p[(size_t)y0*S+x1]-p[(size_t)y1*S+x0]+p[(size_t)y0*S+x0];
}
static long long _r2(const vector<int>&p,int S,const _m&r){
if(!r.valid())return 0;
int x0=r.x0,y0=r.y0,x1=r.x1+1,y1=r.y1+1;
return(long long)p[(size_t)y1*S+x1]-p[(size_t)y0*S+x1]-p[(size_t)y1*S+x0]+p[(size_t)y0*S+x0];
}
static double _q1(double sx,double sy,double sxx,double syy,double sxy){
constexpr double inv=1./121.;
double mx=sx*inv,my=sy*inv;
double vx=max(0.,sxx*inv-mx*mx),vy=max(0.,syy*inv-my*my);
double cov=sxy*inv-mx*my;
double num=(2.*mx*my+_i2)*(2.*cov+_j2);
...
void _l2(const vector<_d>&ref,const vector<_d>&cur,int R,_b3&out)const{
int S=R+1;
size_t N=(size_t)S*S;
out._P2 .assign(N,0);
for(int ch=0;
...
x<R;
++x){
double a=_C(ref[y*R+x],ch),b=_C(cur[y*R+x],ch);
sx+=a;
sy+=b;
sxx+=a*a;
syy+=b*b;
...
_l _b2(const double sums[6][4],const long long counts[6])const{
_l sc;
sc._f=0.;
sc.minView=1.;
sc._w=1.;
...
v<6;
++v){
double den=counts[v]>0?double(counts[v]):1.;
double sn=(sums[v][0]+sums[v][1]+sums[v][2])/(3.*den);
...
_p2 _V2(const vector<vector<_d>>&ref,const vector<vector<_d>>&cur,int R)const{
_p2 L;
double sums[6][4]={
{
0}
...
_b4=R;
int T=(R+_V-1)/_V;
_d3 .assign(6,vector<vector<int>>(T*T));
for(int view=0;
view<6;
++view)for(int fi=0;
fi<(int)ZF.size();
...
void _V1(const A1&c,int view,const _m&r,vector<_d>&buf)const{
...
double den=(p1 .v-p2 .v)*(p0 .u-p2 .u)+(p2 .u-p1 .u)*(p0 .v-p2 .v);
if(fabs(den)<1e-18)continue;
Vec3 nr=cross(pp[1]-pp[0],pp[2]-pp[0]);
double nl=norm(nr);
if(nl<A2)continue;
nr=nr/nl;
for(int py=by0;
...
double w0=((p1 .v-p2 .v)*(sx-p2 .u)+(p2 .u-p1 .u)*(sy-p2 .v))/den,w1=((p2 .v-p0 .v)*(sx-p2 .u)+(p0 .u-p2 .u)*(sy-p2 .v))/den,w2=1-w0-w1;
if(w0<-1e-9||w1<-1e-9||w2<-1e-9)continue;
double iz=w0/p0 .z+w1/p1 .z+w2/p2 .z;
if(iz<=0)continue;
float z=(float)(1./iz);
...
void _I2(const vector<_d>&refFull,const vector<_d>&_k3,const _m&sample,const _m&center,int R,double sums[4],long long&count)const{
...
void _e3(int a,int b,vector<A1>&out)const{
out.clear();
Quadric q=vquad[a];
q+=vquad[b];
Vec3 pos[5];
...
ch<4;
++ch){
double _v5=_r2(base.view[v]._b1[ch],S,centers[v]);
...
_l _K3(const vector<vector<_d>>&ref,const vector<vector<_d>>&cur,int R)const{
_p2 L=_V2(ref,cur,R);
return L.score;
}
...
double stop=ET()+min(tier==2?2.20:5.00,left*(tier==2?0.40:0.70));
MidSnap safe=_K();
const int R=rr?rr:B5;
vector<vector<_d>>ref(6),cur(6);
vector<Vec3>activeV;
vector<Face>activeF;
A3(activeV,activeF);
if(tier>=3)_o3(R);
...
auto _Z4=[&](const vector<Seed>&src,int cap){
for(int i=0;
...
double _Y(int v,int root,const _c&sp)const{
vector<VegaTri>oldTris,newTris;
if(!_Z2(v,root,sp,oldTris,newTris))return-1.;
double dev=_d1(oldTris,newTris);
...
double s=_Y(geom.v,geom.root,sp);
if(s<0.)continue;
double damage=1.-s;
...
return cand._f>=0.9250&&cand.minView>=0.9000&&cand._w>=0.8000&&cand._R>=0.9700;
}
void _S2(const vector<Vec3>&safeV,const vector<Face>&safeF,const vector<double>&safeR,double oldDiag,double _F4){
ZV=safeV;
ZF=safeF;
nV=(int)ZV.size();
nF=(int)ZF.size();
...
vector<Vec3>preV=ZV,bestV=ZV,rootV;
vector<Face>preF=ZF,bestF=ZF,rootF;
vector<double>preR=_e,bestR=_e,rootR;
if(preR.size()!=preV.size())preR.assign(preV.size(),0);
bestR=preR;
double od=diag,hd=hausd;
vector<_u1>r(6),r5;
for(int v=0;
v<6;
++v)r[v]=_x(_U,_T,v,1024);
_l pre=A7(r,preV,preF,1024),root;
...
_U.clear();
_T.clear();
double hi=double(preV.size())/IV,lo=.16;
bool got=0,deep=0;
...
if(ex){
p5=A7(r5,ZV,ZF,512);
if(p5._f<0.925||p5.minView<0.900||p5._w<0.800||p5._R<0.970){
lo=x;
continue;
}
}
_l sc=A7(r,ZV,ZF,1024,ex&&!deep&&pass==1);
bool ok=_I3(deep?root:pre,sc);
if(ok){
bestV=ZV;
bestF=ZF;
bestR=_e;
...
void _R6(int t,const vector<Vec3>&ov,const vector<Face>&of){
int R=t==4?512:384;
vector<Vec3>rv=ov,cv;
vector<Face>rf=of,cf;
vector<_u1>ref(6);
for(int v=0;
v<6;
...
double od=diag,oh=hausd;
auto qe=[&](int rr,const vector<_u1>&rf0,_l&b0,int da,int db,double ef,int dg,double af,double vf,double nf,double df,double rs){
vector<Vec3>sv=ZV;
vector<Face>sf=ZF;
vector<double>sr=_e;
int nv=nV;
...
;
auto pp=[&](double af,double vf,double nf,double df){
vector<Vec3>sv=ZV;
vector<Face>sf=ZF;
vector<double>sr=_e;
_S2(sv,sf,sr,od,oh);
...
A3(cv,cf);
_l q=A7(ref,cv,cf,R);
double af=gi?.00024:.00016,vf=gi?.00034:.00024,nf=gi?.00046:.00032,df=gi?.00010:.00007;
...
bool _r3(const vector<Vec3>&vv,const vector<Face>&ff)const{
if(vv.empty()||ff.empty())return false;
struct EC{
int count=0,dir=0;
}
;
unordered_map<unsigned long long,EC>em;
...
__attribute__((noinline,cold))void _C3(const vector<Face>&ff,int nv,vector<vector<int>>&vf,vector<vector<int>>&fa)const{
vf.assign(nv,{
}
);
fa.assign(ff.size(),{
...
unordered_map<unsigned long long,pair<int,int>>first;
...
struct _H3{
vector<int>fs,ring,anchors;
vector<Face>tris;
}
;
bool _L(int a,int b,int c,const Vec3&nn,Face&f)const{
if(a==b||b==c||a==c)return false;
f.v[0]=a;
f.v[1]=b;
f.v[2]=c;
...
double _D7(const Vec3&p,const Face&f)const{
const Vec3&a=ZV[f.v[0]],&b=ZV[f.v[1]],&c=ZV[f.v[2]];
Vec3 ab=b-a,ac=c-a,ap=p-a;
double d1=dot(ab,ap),d2=dot(ac,ap);
if(d1<=0&&d2<=0)return norm2(ap);
Vec3 bp=p-b;
...
double z=1./(va+vb+vc),v=vb*z,w=vc*z;
return norm2(p-(a+ab*v+ac*w));
}
bool _L2(const vector<int>&ring,const vector<int>&anchors,const Vec3&nn,vector<Face>&out)const{
out.clear();
int m=(int)ring.size();
if(m<3)return false;
Vec3 ax=fabs(nn.x)<.8?Vec3(1,0,0):Vec3(0,1,0),u=cross(ax,nn);
double ul=norm(u);
...
double sg=ar>0?1.:-1.,eps=1e-10*max(1.,diag*diag);
vector<int>w=ring;
int guard=0;
...
double A=sg*ori(a,b,q),B=sg*ori(b,c,q),C=sg*ori(c,a,q);
...
Vec3 tn=cross(ZV[f.v[1]]-ZV[f.v[0]],ZV[f.v[2]]-ZV[f.v[0]]);
double tl=norm(tn);
if(tl<=0)continue;
double sc=1.-dot(tn,nn)/tl+1e-6*norm2(ZV[a]-ZV[c])/(diag*diag+1e-30);
if(sc<bs){
bs=sc;
bj=j;
bf=f;
...
++i){
Face f=out[i];
double T=ori(f.v[0],f.v[1],f.v[2]);
double s=T>=0?1.:-1.,A=s*ori(f.v[0],f.v[1],x),B=s*ori(f.v[1],f.v[2],x),C=s*ori(f.v[2],f.v[0],x);
if(A>=-eps&&B>=-eps&&C>=-eps){
ti=i;
if(fabs(A)<=eps){
ea=f.v[0];
...
int _Q2(int _t5,int _U4,double _p5){
if(nV<20||nF<20)return 0;
vector<Vec3>oldV=ZV;
vector<Face>oldF=ZF;
vector<double>oldR=_e;
vector<vector<int>>vf,fa;
_C3(ZF,nV,vf,fa);
vector<Vec3>fn(nF);
vector<double>pd(nF),wa(nF);
vector<char>good(nF,0);
for(int fi=0;
fi<nF;
++fi){
Face&f=ZF[fi];
Vec3 r=cross(ZV[f.v[1]]-ZV[f.v[0]],ZV[f.v[2]]-ZV[f.v[0]]);
double l=norm(r);
...
double nt=IV<=25000?3.75e-4:(IV<=45000?7.5e-5:1e-10),pt=IV<=25000?.10*hausd:(IV<=45000?.03*hausd:max(1e-13*max(1.,diag),1e-10*hausd));
vector<int>lab(nF,-1);
...
vector<_H3>reps;
vector<char>rmF(nF,0);
vector<double>rad(nV,0.);
...
unordered_map<unsigned long long,pair<int,pair<int,int>>>em;
...
double d2=_k;
for(const Face&f:tr)d2=min(d2,_D7(ZV[x],f));
double err=rad[x]+sqrt(max(0.,d2));
if(err>cap)miss.push_back({
err-cap,x}
);
}
if(miss.empty()){
...
sort(miss.begin(),miss.end(),[](const pair<double,int>&a,const pair<double,int>&b){
...
double _P6(int kept,int removed,const vector<vector<int>>&vertexFaces)const{
vector<int>affectedFaces=vertexFaces[kept];
...
vector<VegaTri>beforeTriangles;
vector<VegaTri>afterTriangles;
...
vector<_d>beforeBuffer,afterBuffer;
for(int view=0;
view<6;
++view){
double minX=resolution,minY=resolution,maxX=0,maxY=0;
bool anyProjection=false;
for(int faceIndex:affectedFaces){
Face face=ZF[faceIndex];
for(int after=0;
after<2;
++after){
for(int k=0;
...
vector<float>bnx(pixelCount),anx(pixelCount),bny(pixelCount),any(pixelCount);
vector<float>bnz(pixelCount),anz(pixelCount),bd(pixelCount),ad(pixelCount);
vector<unsigned char>bf(pixelCount),af(pixelCount);
for(int i=0;
i<pixelCount;
++i){
...
double normalSsim=(refSsim(bnx,anx,bf,af,side)+refSsim(bny,any,bf,af,side)+refSsim(bnz,anz,bf,af,side))/3.0;
double depthSsim=refSsim(bd,ad,bf,af,side);
...
__attribute__((noinline,cold,section(".text.unlikely.hiddenedge")))int _P5(const vector<Vec3>&ov,int maxCandidates=0,double scoreCap=hparam_T3EndpointWeldScoreCap,bool strategicStrike=false,int strategicProbeCount=16){
if(nV<100)return 0;
vector<Vec3>sv=ZV;
FD.assign(nF,0);
ZD.assign(nV,0);
...
vector<tuple<double,int,int>>ca;
for(int b=0;
b<nV;
++b)for(int a:nb[b]){
int cm=0;
...
if(ha)continue;
Vec3 n0=cross(ZV[f.v[1]]-ZV[f.v[0]],ZV[f.v[2]]-ZV[f.v[0]]);
for(int k=0;
k<3;
++k)if(f.v[k]==b)f.v[k]=a;
Vec3 n1=cross(ZV[f.v[1]]-ZV[f.v[0]],ZV[f.v[2]]-ZV[f.v[0]]);
...
double localSsimLoss=_P6(kept,removed,vf);
double averageDebt=accumulatedDebt/max(1,debtSamples);
double rank=localSsimLoss-strategicDebtWeight*averageDebt;
if(rank<bestRank){
bestRank=rank;
bestIndex=i;
}
