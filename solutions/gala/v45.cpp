// Pomegranate v004: tier-2 near-exact planar recovery with broader flips elsewhere.
// It accepts curvature-safe diagonal shortening or strong valence improvements.
...
static constexpr double HParam_KeepRatio_UpTo400k = 0.0237;

static constexpr double HParam_KeepRatio_Huge = 0.0200;

static constexpr double HParam_QemCostCapCoeff = 0.0330;
...
static constexpr int HParam_PomegranateFlipProfile = 4;
...
int acceptCap=inputV<=5000?64:(inputV<=25000?192:(inputV<=50000?384:(inputV<=400000?768:256)));
if(inputV<=5000){scanCap=8000;acceptCap=32;}else if(inputV<=25000){scanCap=min(scanCap,8000);acceptCap=1;}else{scanCap*=2;acceptCap*=2;}
double coplanar=inputV<=25000?0.99999:0.992,align=inputV<=25000?0.9999:0.988,qualityFloor=inputV<=25000?0.995:0.87;
...
double diagGain=norm(verts[a]-verts[b])/(norm(verts[c]-verts[d])+1e-15);
if(inputV<=25000){if(gain<10)continue;}else if(gain<2&&!(gain>=0&&newQ>1.12*oldQ&&diagGain>1.03))continue;
cand.push_back({f0,f1,a,b,c,d,double(gain)+5.0*(newQ-oldQ)+0.40*log(max(1e-12,diagGain))});
}
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.080:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
