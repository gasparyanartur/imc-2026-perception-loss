// Tranberry v134: forced-commit mixed-primitive frontier probe.
// Commit smaller QEM-plus-star output to measure hidden headroom beyond the safe fallback plateau.
...
bool absoluteQemBudget(const RefScore&safe,const RefScore&cand)const{
    ...
    if(inputV>25000&&inputV<=45000)return cand.finalScore>=max(0.9450,safe.finalScore-0.0030)&&
           cand.minView>=max(0.9200,safe.minView-0.0040)&&
           cand.minNormal>=max(0.8000,safe.minNormal-0.0070)&&
...
void continueAbsoluteQem(double ratio){
    ...
    if(elapsed()<HParam_TimeBudgetSeconds-0.95)collapseInvisibleEdges();
    compact();
}
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>25000?0.115:0.5*(lo+hi);restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
        if(verts.size()>=safeBestV.size()){lo=ratio;continue;}RefScore cand=compareToReference(ref,verts,faces,R);
        if(true){safeBestV=verts;safeBestF=faces;safeBestR=compactedRadius;hi=ratio;improved=true;}else lo=ratio;
    }
