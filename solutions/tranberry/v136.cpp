// Tranberry v136: judge-aligned mixed-primitive guard on the all-pass v131 layout.
// Preserve the runtime branch while admitting QEM-plus-star meshes safely above 0.9.
...
bool absoluteQemBudget(const RefScore&safe,const RefScore&cand)const{
    ...
    if(inputV>25000&&inputV<=45000)return cand.finalScore>=max(0.9150,safe.finalScore-0.0500)&&
           cand.minView>=max(0.8750,safe.minView-0.0600)&&
           cand.minNormal>=max(0.6200,safe.minNormal-0.0800)&&
           cand.minDepth>=max(0.9550,safe.minDepth-0.0080);
