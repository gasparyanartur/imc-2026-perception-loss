// Tranberry v127: one-shot original-reference portfolio for the skipped 25k-45k tier.
// Spend the saved render passes on the completed core, then verify one aggressive mesh.
...
if(nV>5000&&nV<=50000){
if(nV>25000&&nV<=45000){qemDeadline=HParam_TimeBudgetSeconds-4.6;runScreenCoreMid();vegaSsimEdgePass(true);compact();}
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<1&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>25000?0.105:0.5*(lo+hi);restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
