...
private:
    ...
    CollapseCandidate makeCandidate(int ab, int kp, const Vec3 &p, const Quadric &q) const
    {
        ...
        double future = 0.55 / (1.0 + 0.08 * vneigh[kp].size()) + 0.45 * max(0.0, 1.0 - used / max(hausd, 1e-30));
        c.choice = c.cost * (1.01 - 0.01 * future);
        return c;
    }
    ...
    CollapseCandidate computeQueueCandidate(int a, int b) const
    {
        ...
        for (int i = 0; i < np; ++i)
        {
            ...
            if (c1.choice < best.choice)
                best = c1;

            CollapseCandidate c2 = makeCandidate(b, a, pos[i], q);
            if (c2.choice < best.choice)
                best = c2;
        }
