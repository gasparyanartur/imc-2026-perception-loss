// Tranberry v142: deeper T7-shielded render-once QEM transaction.
// Push the one-shot medium frontier below v140 while retaining the same safe giant path.
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>25000?0.095:0.5*(lo+hi);restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
