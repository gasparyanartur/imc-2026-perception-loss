...
bool txOriginalGuard(const RefScore&s)const{return s.finalScore>=.9180 && s.minView>=.8800 && s.minNormal>=.6200 && s.minDepth>=.9550;}
...
if((tier==3?txOriginalGuard(sc):txGuard(sc,tier))){extractActiveRadii(compactedRadius);verts=candV;faces=candF;nV=(int)verts.size();nF=(int)faces.size();kept=true;break;}
}
...
if(elapsed()>HParam_TimeBudgetSeconds-4.0){compact();return;}
vector<RefImage>ref(6);for(int v=0;v<6;++v)ref[v]=tier==3?renderReference(sourceVerts,sourceFaces,v,R):renderReference(safeV,safeF,v,R);
vector<double>tries=tier==3?vector<double>{.1425,.14,.1375,.135,.13,.125}:vector<double>{.079,.0775,.075,.07,.065};
bool kept=false;vector<Vec3>bestV;vector<Face>bestF;vector<double>bestR;
for(double ratio:tries){
if(elapsed()>HParam_TimeBudgetSeconds-1.8)break;
restoreMid(safe);continueScreenTo(tier,originalN,ratio,freeRemoved);
extractActiveMesh(candV,candF);RefScore sc=compareToReference(ref,candV,candF,R);
if((tier==3?txOriginalGuard(sc):txGuard(sc,tier))){bestV=candV;bestF=candF;extractActiveRadii(bestR);kept=true;continue;}
if(kept)break;
}
if(kept){verts.swap(bestV);faces.swap(bestF);compactedRadius.swap(bestR);nV=(int)verts.size();nF=(int)faces.size();}
else{restoreMid(safe);compact();}
}
