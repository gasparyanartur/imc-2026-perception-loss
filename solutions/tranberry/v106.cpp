// Tranberry v106: v104 corrected to decouple cost only on screen tiers 2 and 3.
...
vquad.assign(nV, Quadric());if(screenTier==2||screenTier==3)vmoment.assign(nV,Quadric());vector<Quadric> rayAnchor(nV);
...
if((screenTier==2||screenTier==3||screenTier==4)&&kp<(int)vmoment.size())fresh+=vmoment[kp];vquad[kp]=fresh;
