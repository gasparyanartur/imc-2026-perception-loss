...
static constexpr double HParam_Pineapple_KeepRatio_Huge = 0.040;
...
struct CollapseCandidate
{
    ...
    double cost = CParam_Inf, rank = CParam_Inf, mergedRadius = 0.0;

    Vec3 position;

    bool operator<(const CollapseCandidate &o) const { return rank > o.rank; }
...
private:
    ...
    double futureCollapsibility(int ab, int kp, const Vec3 &p) const
    {
        if (nV > 400000) {
            double degree = vneigh[ab].size() + vneigh[kp].size();
            double radial = crad[ab] + crad[kp];
            return 0.75 / (1.0 + 0.06 * degree) + 0.25 * (1.0 - min(1.0, radial / max(hausd, 1e-30)));
        }
        double viable = 0.0, bottleneck = 1.0, total = 0.0;
        double used = max(crad[ab] + norm(verts[ab] - p), crad[kp] + norm(verts[kp] - p));
        auto scan = [&](int v) {
            for (int nb : vneigh[v]) if (nb != ab && nb != kp && !vdead[nb]) {
                ++total;
                double slack = 1.0 - (used + norm(p - verts[nb]) + crad[nb]) / max(hausd, 1e-30);
                if (slack > 0.0) { viable += sqrt(slack); bottleneck = min(bottleneck, slack); }
            }
        };
        scan(ab); scan(kp);
        if (total == 0.0) return 0.0;
        return 0.75 * viable / total + 0.25 * max(0.0, bottleneck);
    }
    ...
    CollapseCandidate makeCandidate(int ab, int kp, const Vec3 &p, const Quadric &q) const
    {
        ...
        c.position = p;
        c.cost = q.evaluate(p);
        c.rank = c.cost * (1.55 - 0.55 * futureCollapsibility(ab, kp, p));
        return c;
    }
