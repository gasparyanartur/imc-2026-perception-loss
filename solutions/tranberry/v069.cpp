...
static constexpr double HParam_KeepRatio_Huge = 0.028;
...
if(nV>5000&&nV<=50000){
if(nV>25000&&nV<=45000){runScreenCoreMid();vegaSsimEdgePass(false);compact();}
...
if(tier==2){
runScreenCoreMid();vegaSsimEdgePass(false);
...
for(int fi:patch){if(fi<0||fi>=(int)faces.size()||fdead[fi])continue;const Face&f=faces[fi];VegaTri a;a.p[0]=verts[f.v[0]];a.p[1]=verts[f.v[1]];a.p[2]=verts[f.v[2]];ot.push_back(a);int id[3]={f.v[0],f.v[1],f.v[2]};for(int k=0;k<3;++k)if(id[k]==c.absorbed)id[k]=c.kept;if(id[0]==id[1]||id[1]==id[2]||id[0]==id[2])continue;VegaTri b;for(int k=0;k<3;++k)b.p[k]=id[k]==c.kept?c.position:verts[id[k]];nt.push_back(b);}
if(ot.empty()||nt.empty())return -1.0;if(vegaPatchDeviation(ot,nt)>hausd)return -1.0;const int R=512;double total=0;int used=0;vector<VegaPixel>a,b;
for(int view=0;view<6;++view){double mnU=1e100,mnV=1e100,mxU=-1e100,mxV=-1e100;bool any=false;auto inc=[&](const VegaTri&t){for(int k=0;k<3;++k){VegaProj p=vegaProjectPoint(t.p[k],view,R);if(!p.ok)continue;any=true;mnU=min(mnU,p.u);mxU=max(mxU,p.u);mnV=min(mnV,p.v);mxV=max(mxV,p.v);}};for(const auto&t:ot)inc(t);for(const auto&t:nt)inc(t);if(!any)continue;int x0=max(0,(int)floor(mnU)-5),y0=max(0,(int)floor(mnV)-5),x1=min(R-1,(int)ceil(mxU)+5),y1=min(R-1,(int)ceil(mxV)+5);if(x0>x1||y0>y1)continue;int w=x1-x0+1,h=y1-y0+1;if(w*h>52000)return -1.0;renderVegaPatch(ot,view,x0,y0,w,h,a);renderVegaPatch(nt,view,x0,y0,w,h,b);total+=vegaBuffersSsim(a,b);++used;}return used?total/used:1.0;}
...
void vegaSsimEdgePass(bool independent){double left=HParam_TimeBudgetSeconds-elapsed();if(left<1.2)return;double stop=elapsed()+min(1.7,left*0.38);vector<EdgeVisualCandidate>cs;cs.reserve(512);int scanned=0;
for(int a=0;a<(int)verts.size()&&scanned<2400&&elapsed()<stop;++a)if(!vdead[a])for(int b:vneigh[a]){if(b<=a||vdead[b]||scanned>=2400||elapsed()>=stop)continue;++scanned;if(countCommonFaces(a,b)!=2||countCommonNeighbors(a,b)!=2)continue;auto c=computeBestValid(a,b);if(!c.valid()||c.cost>costCap)continue;double s=localVegaSsimForEdge(c);if(s>=0.99992)cs.push_back({a,b,1.0-s,c.cost});}
sort(cs.begin(),cs.end());vector<char>lock(verts.size(),0);int cap=max(8,min(180,(int)floor(inputV*0.0025))),done=0;for(const auto&e:cs){if(done>=cap||elapsed()>=stop)break;if(!edgeExists(e.a,e.b)||countCommonFaces(e.a,e.b)!=2||countCommonNeighbors(e.a,e.b)!=2)continue;if(independent&&(lock[e.a]||lock[e.b]))continue;auto c=computeBestValid(e.a,e.b);if(!c.valid()||c.cost>costCap||localVegaSsimForEdge(c)<0.99992)continue;vector<int>near;if(independent){near=vneigh[e.a].data;for(int x:vneigh[e.b])near.push_back(x);lock[e.a]=lock[e.b]=1;for(int x:near)if(x>=0&&x<(int)lock.size())lock[x]=1;}applyCollapse(c.absorbed,c.kept,c.position,c.mergedRadius);++accepted;++done;}}
