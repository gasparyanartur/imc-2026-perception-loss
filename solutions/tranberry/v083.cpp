...
c.position=p;c.cost=q.evaluate(p);c.choice=c.cost;double sw=sampleW[ab]+sampleW[kp];if(sw>1e-30){Vec3 m=(sampleSum[ab]+sampleSum[kp])/sw;double pe=norm2(p-m)/(hausd*hausd+1e-30);c.choice+=0.002*costCap*min(25.0,pe);}return c;
