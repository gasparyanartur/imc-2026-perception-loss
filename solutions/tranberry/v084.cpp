...
static constexpr double HParam_KeepRatio_UpTo400k = 0.023;
...
double cost=CParam_Inf,choice=CParam_Inf,mergedRadius=0.0;
...
vdead.assign(nV,0);fdead.assign(nF,0);vver.assign(nV,0);vquad.assign(nV,Quadric());vmoment.assign(nV,Quadric());sampleSum.assign(nV,Vec3());sampleW.assign(nV,0.0);vfaces.assign(nV,{});vneigh.assign(nV,SmallSet());tailLocks.assign(nV,0);facePix.clear();faceSil.clear();faceWin.clear();vertAnchor.clear();screenTier=0;faceImpMean=0;faceAreaMean=0;
for(int fi=0;fi<nF;++fi){Face&f=faces[fi];Quadric q=Quadric::fromTriangle(verts[f.v[0]],verts[f.v[1]],verts[f.v[2]]);Vec3 sm=(verts[f.v[0]]+verts[f.v[1]]+verts[f.v[2]])/3.0;double sw=norm(cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]));for(int k=0;k<3;++k){int u=f.v[k];sampleSum[u]=sampleSum[u]+sm*sw;sampleW[u]+=sw;}for(int k=0;k<3;++k){vfaces[f.v[k]].push_back(fi);vquad[f.v[k]]+=q;}for(int k=0;k<3;++k){int a=f.v[k],b=f.v[(k+1)%3];if(a!=b){vneigh[a].insert(b);vneigh[b].insert(a);}}}
...
vquad.assign(nV,Quadric());vmoment.assign(nV,Quadric());crad.assign(nV,0.0);sampleSum.assign(nV,Vec3());sampleW.assign(nV,0.0);
...
Vec3 sm=(verts[f.v[0]]+verts[f.v[1]]+verts[f.v[2]])/3.0;double sw=norm(cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]));for(int k=0;k<3;++k){int u=f.v[k];sampleSum[u]=sampleSum[u]+sm*sw;sampleW[u]+=sw;}
...
c.position=p;c.cost=q.evaluate(p);c.choice=c.cost;if(sampleW[ab]>1e-30&&sampleW[kp]>1e-30){Vec3 ma=sampleSum[ab]/sampleW[ab],mk=sampleSum[kp]/sampleW[kp];double pe=max(norm2(p-ma),norm2(p-mk))/(hausd*hausd+1e-30);c.choice+=0.003*costCap*min(25.0,pe);}return c;
...
CollapseCandidate c1=makeCandidate(a,b,pos[i],q);if(c1.choice<best.choice)best=c1;

CollapseCandidate c2=makeCandidate(b,a,pos[i],q);if(c2.choice<best.choice)best=c2;
...
if(c.choice<best.choice)best=c;
...
if(kp<(int)vmoment.size()&&ab<(int)vmoment.size())vmoment[kp]+=vmoment[ab];if(kp<(int)sampleSum.size()&&ab<(int)sampleSum.size()){sampleSum[kp]=sampleSum[kp]+sampleSum[ab];sampleW[kp]+=sampleW[ab];}
