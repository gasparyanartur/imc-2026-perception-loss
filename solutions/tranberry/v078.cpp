...
double cost=CParam_Inf,saliency=0.0,mergedRadius=0.0;
...
if(screenTier)faceImpMean/=max(1,activeF);if(!screenTier&&!facePix.empty()){for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi])){faceImpMean+=facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi];++activeF;}faceImpMean/=max(1,activeF);}
vertAnchor.assign(nV,0.0);if(!facePix.empty())for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi]))for(int k=0;k<3;++k)vertAnchor[faces[fi].v[k]]+=faceSil[fi]+0.05*facePix[fi]+0.10*faceWin[fi];
...
c.position=p;c.cost=q.evaluate(p);if(!screenTier&&!facePix.empty()){double vi=(vertAnchor[ab]+vertAnchor[kp])/(6.0*faceImpMean+1.0);c.saliency=min(8.0,max(0.0,vi));}return c;
...
if(!screenTier&&!facePix.empty()){vector<CollapseCandidate>pool;pool.push_back(c);int bw=inputV>400000?3:8;for(int k=1;k<bw&&!pq.empty();++k){pool.push_back(pq.top());pq.pop();}int pick=0;double lim=pool[0].cost*1.08+costCap*1e-7;for(int k=1;k<(int)pool.size();++k)if(pool[k].cost<=lim&&pool[k].saliency<pool[pick].saliency)pick=k;c=pool[pick];for(int k=0;k<(int)pool.size();++k)if(k!=pick)pq.push(pool[k]);}
