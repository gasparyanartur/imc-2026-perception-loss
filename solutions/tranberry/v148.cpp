// Tranberry v148: curvature-density collapse ranking on the strict 48k continuation.
// Prefer flat-region collapses and preserve high normal-variation graph samples.
...
c.position=p;c.cost=q.evaluate(p);if(screenTier==4&&ab<(int)vertexCurv.size()&&kp<(int)vertexCurv.size())c.cost*=1.0+3.0*max(vertexCurv[ab],vertexCurv[kp]);return c;
