...
bool largeRelativeGuard(const RefScore&b,const RefScore&s)const{auto ok=[](double x,double y){return 1.0-y<=1.05*(1.0-x)+1e-6;};return ok(b.finalScore,s.finalScore)&&ok(b.minView,s.minView)&&ok(b.minNormal,s.minNormal)&&ok(b.minDepth,s.minDepth);}
