...
static constexpr double HParam_Pineapple_KeepRatio_UpTo400k = 0.02475;
...
struct CollapseCandidate
{
    ...
    double cost = CParam_Inf, tie = -CParam_Inf, mergedRadius = 0.0;
...
public:
    void run()
    {
        ...
        if (nV > 400000) { compact(); writeMesh(); return; }
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.14355), safeTarget = (int)floor(nV * 0.1584);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.3564, 0.3267, 0.2970} : vector<double>{0.1386, 0.0990, 0.0792};
        for (double kr : st)
        {
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier)
        {
            for (int i = 0; i < nV; ++i) if (!vdead[i]) { Quadric h = vquad[i]; h.scale(0.20); vmoment[i] += h; }
            double am = 0;
            int ac = 0;
            for (int i = 0; i < nV; ++i)
                if (!vdead[i])
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            double imp = screenTier == 3 ? (facePix[fi] + 10.0 * faceSil[fi]) : (facePix[fi] + 12.0 * faceSil[fi] + 2.0 * faceWin[fi]);
            double r = screenTier == 3 ? (imp + 0.20 * faceImpMean) / (faceImpMean + 1e-12) : (imp + 0.12 * faceImpMean) / (faceImpMean + 1e-12);
            double w = screenTier == 2 ? min(18.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : (screenTier == 3 ? min(7.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r)))) : min(16.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72))));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
    ...
    CollapseCandidate makeCandidate(int ab, int kp, const Vec3 &p, const Quadric &q) const
    {
        ...
        c.position = p;
        c.cost = q.evaluate(p);
        c.tie = double(vneigh[kp].size()) - 4.0 * crad[kp] / max(hausd, 1e-30);
        return c;
    }
    ...
    CollapseCandidate computeQueueCandidate(int a, int b) const
    {
        ...
        for (int i = 0; i < np; ++i)
        {
            ...
            if (c1.cost < best.cost || (c1.cost == best.cost && c1.tie > best.tie))
                best = c1;

            CollapseCandidate c2 = makeCandidate(b, a, pos[i], q);
            if (c2.cost < best.cost || (c2.cost == best.cost && c2.tie > best.tie))
                best = c2;
        }
    ...
    CollapseCandidate computeBestValid(int a, int b) const
    {
        ...
        for (int i = 0; i < np; ++i)
        {
            ...
            for (int dir = 0; dir < 2; ++dir)
            {
                ...
                if (c.cost < best.cost || (c.cost == best.cost && c.tie > best.tie))
                    best = c;
            }
        }
    ...
    void applyCollapse(int ab, int kp, const Vec3 &np, double nr)
    {
        ...
        if (MEMLESS)
        {
            ...
            for (int fi : vfaces[kp])
            {
                ...
                fresh += ( screenTier ? weightedFaceQuadric(fi) : Quadric::fromTriangle(verts[f.v[0]], verts[f.v[1]], verts[f.v[2]]));
            }

            if (screenTier && kp < (int)vmoment.size())
                fresh += vmoment[kp];
            vquad[kp] = fresh;
        }
        else
