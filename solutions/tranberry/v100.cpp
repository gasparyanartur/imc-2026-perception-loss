// Tranberry v100: v096 + persistent analytic screen and normal Jacobians.
...
Quadric normalJacobianQuadric(const Face&fc,int k)const{Vec3 p=verts[fc.v[k]],e=verts[fc.v[(k+1)%3]]-verts[fc.v[(k+2)%3]],n=cross(verts[fc.v[1]]-verts[fc.v[0]],verts[fc.v[2]]-verts[fc.v[0]]);double nl=norm(n);if(nl<1e-15)return Quadric();n=n/nl;Vec3 ax[3]={Vec3(1,0,0),Vec3(0,1,0),Vec3(0,0,1)},col[3];for(int j=0;j<3;++j){col[j]=cross(ax[j],e);col[j]=(col[j]-n*dot(n,col[j]))/nl;}Quadric q;q.a=dot(col[0],col[0]);q.b=dot(col[0],col[1]);q.c=dot(col[0],col[2]);q.e=dot(col[1],col[1]);q.f=dot(col[1],col[2]);q.h=dot(col[2],col[2]);q.d=-(q.a*p.x+q.b*p.y+q.c*p.z);q.g=-(q.b*p.x+q.e*p.y+q.f*p.z);q.i=-(q.c*p.x+q.f*p.y+q.h*p.z);q.j=-(q.d*p.x+q.g*p.y+q.i*p.z);return q;}
...
if((screenTier==2||screenTier==3)&&faceSil[fi]>0){int pc=max(1,__builtin_popcount((unsigned)faceViews[fi]));for(int view=0;view<6;++view)if(faceViews[fi]&(1u<<view))for(int k=0;k<3;++k){Quadric rq=screenJacobianQuadric(verts[f.v[k]],view),nq=normalJacobianQuadric(f,k);double rt=rq.a+rq.e+rq.h,nt=nq.a+nq.e+nq.h;if(rt>1e-15&&nt>1e-15){nq.scale(rt/nt);rq+=nq;}rq.scale(1.0/pc);rayAnchor[f.v[k]]+=rq;}}
