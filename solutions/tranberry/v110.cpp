// Tranberry v110: v105 + render-ranked placement selection on tiers 2 and 3 with geometry caps.
...
CollapseCandidate makeCandidate(int ab,int kp,const Vec3&p,const Quadric&qg,const Quadric&qr)const{

CollapseCandidate c;c.absorbed=ab;c.kept=kp;

c.versionAbsorbed=vver[ab];c.versionKept=vver[kp];

c.position=p;c.cost=qg.evaluate(p);c.rankCost=(screenTier==2||screenTier==3)?qr.evaluate(p):c.cost;return c;
...
CollapseCandidate c1=makeCandidate(a,b,pos[i],qc,q);if(c1.rankCost<best.rankCost)best=c1;

CollapseCandidate c2=makeCandidate(b,a,pos[i],qc,q);if(c2.rankCost<best.rankCost)best=c2;
...
CollapseCandidate c=makeCandidate(ab,kp,pos[i],qc,q);

c.mergedRadius=mr;

if(c.rankCost<best.rankCost)best=c;
