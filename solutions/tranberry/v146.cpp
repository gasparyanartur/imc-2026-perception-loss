// Tranberry v146: topology-first full-band QEM on the new 48k boundary model.
// Remove strict continuation veto in the 45k-50k band and retain the T7 shield.
...
void continueAbsoluteQem(double ratio){
    ...
    strictContinuation=false;initQueue();collapseLoop();
    compact();
}
...
void absoluteQemEndgame(){
    ...
    for(int it=0;it<portfolioIters&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
        double ratio=inputV>45000?0.040:(inputV>25000?0.115:0.5*(lo+hi));restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);
