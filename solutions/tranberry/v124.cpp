// Tranberry v124: v105 + normalized-sum render-geometry Pareto placement on tiers 2 and 3.
...
Vec3 normalizedSumBridge(const Quadric&qr,const Quadric&qg,const Vec3&rp,const Vec3&gp)const{double g0=qg.evaluate(gp),g1=qg.evaluate(rp),r0=qr.evaluate(gp),r1=qr.evaluate(rp),gd=max(1e-18,g1-g0),rd=max(1e-18,r0-r1);auto score=[&](double t){Vec3 p=gp*(1-t)+rp*t;return (qg.evaluate(p)-g0)/gd+(qr.evaluate(p)-r1)/rd;};double lo=0,hi=1;for(int it=0;it<16;++it){double a=(2*lo+hi)/3,b=(lo+2*hi)/3;if(score(a)<score(b))hi=b;else lo=a;}double t=0.5*(lo+hi);return gp*(1-t)+rp*t;}
...
Vec3 pos[6];int np;getCandidatePositions(a,b,q,pos,np);if(screenTier==2||screenTier==3){Vec3 rp,gp;if(solveQem3x3(q,rp)&&solveQem3x3(qc,gp)&&np<6)pos[np++]=normalizedSumBridge(q,qc,rp,gp);}
