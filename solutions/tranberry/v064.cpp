...
bool absoluteQemBudget(const RefScore&safe,const RefScore&cand)const{
    return cand.finalScore>=safe.finalScore && cand.minView>=safe.minView-0.001 && cand.minNormal>=safe.minNormal-0.002 && cand.minDepth>=safe.minDepth-0.0004;
...
void absoluteQemEndgame(){
    ...
    double hi=double(safeV.size())/double(inputV),lo=inputV<=25000?0.10:0.05;bool improved=false;
