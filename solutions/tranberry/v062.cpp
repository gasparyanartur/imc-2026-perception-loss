...
bool absoluteQemBudget(const RefScore&safe,const RefScore&cand)const{
    if(inputV>25000 && inputV<=45000) return cand.finalScore>=safe.finalScore && cand.minView>=safe.minView-0.001 && cand.minNormal>=safe.minNormal-0.002 && cand.minDepth>=safe.minDepth-0.0004;
