// Tranberry v097: v095 + deeper tier-2 continuation using persistent screen memory.
// Projected position is anchored perpendicular to each visible camera ray while
// motion along the ray remains free, directly matching the rendered SSIM geometry.
...
vquad.assign(nV, Quadric());if(screenTier==2)vmoment.assign(nV,Quadric());vector<Quadric> rayAnchor(nV);
...
if(screenTier==2&&faceSil[fi]>0){int pc=max(1,__builtin_popcount((unsigned)faceViews[fi]));for(int view=0;view<6;++view)if(faceViews[fi]&(1u<<view))for(int k=0;k<3;++k){Quadric rq=screenJacobianQuadric(verts[f.v[k]],view);rq.scale(1.0/pc);rayAnchor[f.v[k]]+=rq;}}
for(int k=0;k<3;++k)vquad[f.v[k]]+=q;
}
if(screenTier==2||screenTier==4){double am=0;int ac=0;for(int i=0;i<nV;++i)if(!vdead[i]){am+=vertAnchor[i];++ac;}am/=max(1,ac);double bw=(screenTier==2?0.0000035:0.0000050)*diag*diag/(am+1.0);for(int i=0;i<nV;++i)if(!vdead[i]&&vertAnchor[i]>0){double w=bw*min(18.0,vertAnchor[i]/(am+1e-12));if(screenTier==2){double tr=rayAnchor[i].a+rayAnchor[i].e+rayAnchor[i].h;if(tr>1e-15){rayAnchor[i].scale(2.0*w/tr);vmoment[i]=rayAnchor[i];vquad[i]+=vmoment[i];}}else vquad[i]+=anchorPointQuadric(verts[i],w);}if(screenTier==4)for(int i=0;i<nV;++i)if(!vdead[i])vquad[i]+=vmoment[i];}
}
...
if((screenTier==2||screenTier==4)&&kp<(int)vmoment.size())fresh+=vmoment[kp];vquad[kp]=fresh;
