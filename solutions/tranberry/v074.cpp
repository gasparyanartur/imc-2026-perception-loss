// Durian v101: v097 + view-dependent face weights for large tiers (T5/T6/T7).
// Build raster importance once for inputV>50000 and use silhouette/visibility weights
// in initFaceWeights so the main QEM loop preserves visually important regions.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_Huge = 0.019;
...
if(inputV>50000){buildRasterImportance(inputV>400000?96:256);}
...
if(!screenTier&&!facePix.empty()){for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi])){faceImpMean+=facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi];++activeF;}faceImpMean/=max(1,activeF);}
...
if(w!=1.0)q.scale(w);
}else if(!facePix.empty()){
double imp=facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi];double r=(imp+0.12*faceImpMean)/(faceImpMean+1e-12);
double w=min(16.0,max(0.06,0.08+0.92*pow(max(0.0,r),0.72)));
if(w!=1.0)q.scale(w);
...
if(screenTier&&fi<(int)facePix.size()){double imp=facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi];double r=(imp+0.12*faceImpMean)/(faceImpMean+1e-12);double w=screenTier==2?min(18.0,max(0.06,0.08+0.92*pow(max(0.0,r),0.90))):min(16.0,max(0.06,0.08+0.92*pow(max(0.0,r),0.72)));if(w!=1.0)q.scale(w);}else if(!screenTier&&!facePix.empty()&&fi<(int)facePix.size()){double imp=facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi];double r=(imp+0.12*faceImpMean)/(faceImpMean+1e-12);double w=min(16.0,max(0.06,0.08+0.92*pow(max(0.0,r),0.72)));if(w!=1.0)q.scale(w);}else if(area>=1e-30){n=n/(2.0*area);double w=faceWeightFor(n,area);if(w!=1.0)q.scale(w);}return q;
}
...
fresh+=(!facePix.empty()?weightedFaceQuadric(fi):Quadric::fromTriangle(verts[f.v[0]],verts[f.v[1]],verts[f.v[2]]));}
