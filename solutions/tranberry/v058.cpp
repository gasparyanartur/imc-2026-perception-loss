...
static constexpr double HParam_Pineapple_KeepRatio_UpTo400k = 0.0245;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.1421), safeTarget = (int)floor(nV * 0.1568);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.3528, 0.3234, 0.2940} : vector<double>{0.1372, 0.0980, 0.0784};
        for (double kr : st)
        {
    ...
    {
        double e = elapsed();
        int quota = nV > 400000 ? collapseLimit / 20 : (collapseLimit * 2) / 5;
        return e < 6.0 && accepted < min(collapseLimit, max(1, quota));
    }
    ...
    bool batchFree(int a, int b) const
    {
        ...
        return tailLocks[a] != tailStamp && tailLocks[b] != tailStamp;
    }
