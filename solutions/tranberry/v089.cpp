// Tranberry v089: v072 + perspective view-ray silhouette quadrics on screen tier 2.
...
if(screenTier==2&&faceSil[fi]>0){int pc=max(1,__builtin_popcount((unsigned)faceViews[fi]));for(int view=0;view<6;++view)if(faceViews[fi]&(1u<<view))for(int k=0;k<3;++k){Quadric rq=rayAnchorQuadric(verts[f.v[k]],view);rq.scale(1.0/pc);rayAnchor[f.v[k]]+=rq;}}
