// Tranberry v149: curvature-aware valence regularization on the strict 48k continuation.
...
vector<double> vertAnchor,vertexCurv;
...
void buildCurvatureDensity(){vertexCurv.assign(nV,0.0);vector<Vec3>fn(nF);for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi])){const Face&f=faces[fi];Vec3 n=cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]);double l=norm(n);if(l>1e-15)fn[fi]=n/l;}for(int a=0;a<nV;++a)if(!vdead[a])for(int b:vneigh[a])if(a<b&&!vdead[b]){int f0=-1,f1=-1;for(int fi:vfaces[a])if(!fdead[fi]&&faceHasVertex(fi,b)){if(f0<0)f0=fi;else{f1=fi;break;}}if(f1>=0){double k=1.0-fabs(dot(fn[f0],fn[f1])),w=norm(verts[a]-verts[b])*k;vertexCurv[a]+=w;vertexCurv[b]+=w;}}double m=0;int c=0;for(int i=0;i<nV;++i)if(!vdead[i]){m+=vertexCurv[i];++c;}m=m/max(1,c)+1e-15;for(double&x:vertexCurv)x=min(12.0,x/m);}
...
if(screenTier==4&&vertexCurv.size()==(size_t)nV){int common=0;for(int x:vneigh[ab])if(x!=kp&&vneigh[kp].contains(x))++common;int merged=max(0,(int)vneigh[ab].size()+(int)vneigh[kp].size()-2-common);double feature=max(vertexCurv[ab],vertexCurv[kp]),flat=1.0/(1.0+feature),defect=fabs(double(merged-6));double loseFeature=max(0.0,vertexCurv[ab]-vertexCurv[kp]);c.cost*=1.0+0.11*flat*defect+0.24*loseFeature;}return c;
