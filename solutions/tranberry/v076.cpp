...
c.position=p;c.cost=q.evaluate(p);c.priority=c.cost;if(!screenTier&&!facePix.empty()){double vi=(vertAnchor[ab]+vertAnchor[kp])/(6.0*faceImpMean+1.0);vi=min(8.0,max(0.0,vi));c.priority=c.cost*(1.0+0.35*vi)+1e-9*costCap*vi;}return c;
