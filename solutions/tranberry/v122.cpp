// Tranberry v122: v118 + banded low-merged-valence collapsibility scheduling on tiers 2 and 3.
// Projected position is anchored perpendicular to each visible camera ray while
// motion along the ray remains free; tier 3 gains its first explicit silhouette anchor.
...
double cost=CParam_Inf,mergedRadius=0.0;int costBand=0,topoKey=-1;

Vec3 position;

bool operator<(const CollapseCandidate& o)const{if(topoKey>=0&&o.topoKey>=0){if(costBand!=o.costBand)return costBand>o.costBand;if(topoKey!=o.topoKey)return topoKey>o.topoKey;}return cost>o.cost;}
...
vector<int> facePix,faceSil,faceWin;vector<unsigned char> faceViews;
...
facePix.assign(nF,0);faceSil.assign(nF,0);faceWin.assign(nF,0);faceViews.assign(nF,0);vector<Vec3>fn(nF);
...
for(int q=0;q<R*R;++q)if(id[q]>=0){++facePix[id[q]];faceViews[id[q]]|=(unsigned char)(1u<<view);}
...
if(screenTier==3){int finalTarget=(int)floor(nV*0.135),safeTarget=(int)floor(nV*0.15);buildRasterImportanceTier3(384);initFaceWeights();targetV=safeTarget;collapseLimit=nV-targetV;initQueue();collapseLoop();if(accepted<nV-finalTarget&&elapsed()<HParam_TimeBudgetSeconds-0.4){buildRasterImportanceTier3(384);initFaceWeights();priority_queue<CollapseCandidate>e;pq.swap(e);targetV=finalTarget;collapseLimit=nV-targetV;initQueue();collapseLoop();}return;}
...
vdead.assign(nV,0);fdead.assign(nF,0);vver.assign(nV,0);vquad.assign(nV,Quadric());vmoment.assign(nV,Quadric());vfaces.assign(nV,{});vneigh.assign(nV,SmallSet());tailLocks.assign(nV,0);facePix.clear();faceSil.clear();faceWin.clear();faceViews.clear();vertAnchor.clear();screenTier=0;faceImpMean=0;faceAreaMean=0;
...
Quadric rayAnchorQuadric(const Vec3&p,int view)const{Vec3 eye,right,up,fwd;cameraBasisImp(view,eye,right,up,fwd);Vec3 r=p-eye;double rl=norm(r);if(rl<1e-15)return Quadric();r=r/rl;Quadric q;q.a=1-r.x*r.x;q.b=-r.x*r.y;q.c=-r.x*r.z;q.e=1-r.y*r.y;q.f=-r.y*r.z;q.h=1-r.z*r.z;q.d=-(q.a*p.x+q.b*p.y+q.c*p.z);q.g=-(q.b*p.x+q.e*p.y+q.f*p.z);q.i=-(q.c*p.x+q.f*p.y+q.h*p.z);q.j=-(q.d*p.x+q.g*p.y+q.i*p.z);return q;}
...
Quadric screenJacobianQuadric(const Vec3&p,int view)const{Vec3 eye,right,up,fwd;cameraBasisImp(view,eye,right,up,fwd);Vec3 d=p-eye;double x=dot(d,right),y=dot(d,up),z=dot(d,fwd);if(z<=1e-12)return Quadric();Vec3 gu=(right*z-fwd*x)/(z*z),gv=(up*z-fwd*y)/(z*z);Quadric q=gradientAnchorQuadric(p,gu);q+=gradientAnchorQuadric(p,gv);Quadric dq=gradientAnchorQuadric(p,fwd);double qt=q.a+q.e+q.h,dt=dq.a+dq.e+dq.h;if(qt>1e-15&&dt>1e-15)dq.scale(qt/dt);q+=dq;return q;}

void initFaceWeights(){

vquad.assign(nV, Quadric());vector<Quadric> rayAnchor(nV);
...
vertAnchor.assign(nV,0.0);if(screenTier==2||screenTier==3||screenTier==4)for(int fi=0;fi<nF;++fi)if(!(fi<(int)fdead.size()&&fdead[fi]))for(int k=0;k<3;++k)vertAnchor[faces[fi].v[k]]+=faceSil[fi]+0.05*facePix[fi]+0.10*faceWin[fi];
...
if((screenTier==2||screenTier==3)&&faceSil[fi]>0){int pc=max(1,__builtin_popcount((unsigned)faceViews[fi]));for(int view=0;view<6;++view)if(faceViews[fi]&(1u<<view))for(int k=0;k<3;++k){Quadric rq=screenJacobianQuadric(verts[f.v[k]],view);rq.scale(1.0/pc);rayAnchor[f.v[k]]+=rq;}}
for(int k=0;k<3;++k)vquad[f.v[k]]+=q;
}
if(screenTier==2||screenTier==3||screenTier==4){double am=0;int ac=0;for(int i=0;i<nV;++i)if(!vdead[i]){am+=vertAnchor[i];++ac;}am/=max(1,ac);double bw=(screenTier==2?0.0000035:(screenTier==3?0.0000030:0.0000050))*diag*diag/(am+1.0);for(int i=0;i<nV;++i)if(!vdead[i]&&vertAnchor[i]>0){double w=bw*min(18.0,vertAnchor[i]/(am+1e-12));if(screenTier==2||screenTier==3){double tr=rayAnchor[i].a+rayAnchor[i].e+rayAnchor[i].h;if(tr>1e-15){rayAnchor[i].scale(2.0*w/tr);vquad[i]+=rayAnchor[i];}}else vquad[i]+=anchorPointQuadric(verts[i],w);}if(screenTier==4)for(int i=0;i<nV;++i)if(!vdead[i])vquad[i]+=vmoment[i];}
}
...
c.position=p;c.cost=q.evaluate(p);if(screenTier==2||screenTier==3){c.costBand=ilogb(max(1e-300,fabs(c.cost)));int common=0;for(int x:vneigh[ab])if(x!=kp&&vneigh[kp].contains(x))++common;c.topoKey=vneigh[ab].size()+vneigh[kp].size()-common-2;}return c;

}

Quadric geometryFaceQuadric(int fi)const{const Face&f=faces[fi];Quadric q=Quadric::fromTriangle(verts[f.v[0]],verts[f.v[1]],verts[f.v[2]]);if(screenTier&&fi<(int)facePix.size()){double imp=screenTier==3?(facePix[fi]+10.0*faceSil[fi]):(facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi]);double r=screenTier==3?(imp+0.20*faceImpMean)/(faceImpMean+1e-12):(imp+0.12*faceImpMean)/(faceImpMean+1e-12);double w=screenTier==2?min(18.0,max(0.06,0.08+0.92*pow(max(0.0,r),0.90))):min(7.0,max(0.12,0.15+0.85*sqrt(max(0.0,r))));if(w!=1.0)q.scale(w);}return q;}
Quadric geometryEdgeQuadric(int a,int b)const{Quadric q;if(screenTier!=2&&screenTier!=3){q=vquad[a];q+=vquad[b];return q;}vector<int>fs=vfaces[a];for(int fi:vfaces[b])if(find(fs.begin(),fs.end(),fi)==fs.end())fs.push_back(fi);for(int fi:fs)if(!fdead[fi])q+=geometryFaceQuadric(fi);return q;}
...
Quadric q=geometryEdgeQuadric(a,b);
...
Quadric q=vquad[a];q+=vquad[b];Quadric qc=geometryEdgeQuadric(a,b);

Vec3 pos[6];int np;getCandidatePositions(a,b,q,pos,np);if(screenTier==2||screenTier==3){Vec3 rp,gp;if(solveQem3x3(q,rp)&&solveQem3x3(qc,gp)&&np<6)pos[np++]=(rp+gp)*0.5;}

CollapseCandidate best;

for(int i=0;i<np;++i){

CollapseCandidate c1=makeCandidate(a,b,pos[i],qc);if(c1.cost<best.cost)best=c1;

CollapseCandidate c2=makeCandidate(b,a,pos[i],qc);if(c2.cost<best.cost)best=c2;
...
CollapseCandidate c=makeCandidate(ab,kp,pos[i],qc);
