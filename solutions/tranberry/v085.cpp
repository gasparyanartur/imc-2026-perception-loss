...
if(nV>5000&&nV<=50000){
if(nV>25000&&nV<=45000){runScreenCoreMid();exactPlanarStarPass(false);vegaSsimEdgePass(true);compact();}
...
if(inputV<=400000)exactPlanarStarPass(false);
...
if(tier==2){
runScreenCoreMid();exactPlanarStarPass(false);vegaSsimEdgePass(true);
...
if(!HParam_EnableRootNudge||exactPlanarMode)return z;
...
void exactPlanarStarPass(bool independent){
double left=HParam_TimeBudgetSeconds-elapsed();if(left<0.35)return;double stop=elapsed()+min(1.8,left*0.35);StarParams sp={12,1e-9,2e-9,0.03,1.0,50000,min(nV,180000),2,0.5,1.8};vector<pair<int,int>>cand;cand.reserve(4096);exactPlanarMode=true;
for(int v=0;v<(int)verts.size()&&elapsed()<stop;++v){if(vdead[v])continue;StarCandidate c=computeStarCandidateWithParams(v,sp);if(c.valid())cand.push_back({c.v,c.root});}
vector<char>lock(verts.size(),0);int cap=min(50000,max(16,(int)floor(inputV*0.025))),done=0;for(auto pr:cand){if(done>=cap||elapsed()>=stop)break;int v=pr.first;if(v<0||v>=(int)vdead.size()||vdead[v]||lock[v])continue;vector<int>ring,inc;if(!orientedRingForVertexWithParams(v,sp,ring,inc))continue;if(independent){bool blocked=false;for(int x:ring)if(lock[x]){blocked=true;break;}if(blocked)continue;}if(applyStarDeleteWithParams(v,pr.second,sp)){++accepted;++done;if(independent){lock[v]=1;for(int x:ring)if(x>=0&&x<(int)lock.size()){lock[x]=1;for(int y:vneigh[x])if(y>=0&&y<(int)lock.size())lock[y]=1;}}}}exactPlanarMode=false;}
