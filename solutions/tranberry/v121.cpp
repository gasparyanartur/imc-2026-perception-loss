// Tranberry v121: v117 + banded low-merged-valence collapsibility scheduling on tier 2.
...
vector<double>st=screenTier==2?vector<double>{0.34,0.31,0.28}:vector<double>{0.14,0.10,0.08};
...
c.position=p;c.cost=q.evaluate(p);if(screenTier==2){c.costBand=ilogb(max(1e-300,fabs(c.cost)));int common=0;for(int x:vneigh[ab])if(x!=kp&&vneigh[kp].contains(x))++common;c.topoKey=vneigh[ab].size()+vneigh[kp].size()-common-2;}return c;
...
Vec3 pos[6];int np;getCandidatePositions(a,b,q,pos,np);if(screenTier==2){Vec3 rp,gp;if(solveQem3x3(q,rp)&&solveQem3x3(qc,gp)&&np<6)pos[np++]=(rp+gp)*0.5;}
