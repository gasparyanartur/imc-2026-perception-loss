// Tranberry v088: v072 + signed-volume-preserving placement on small/medium meshes.
// This stronger variant treats the local volume equation as a first-class error
// term across both rendered screen tiers while retaining the v072 large/T7 path.
...
Quadric edgePlacementQuadric(int a,int b)const{
Quadric q=vquad[a];q+=vquad[b];
if(screenTier!=2&&screenTier!=3)return q;
Vec3 coef;double old6=0;vector<int> fs=vfaces[a];
for(int fi:vfaces[b])if(find(fs.begin(),fs.end(),fi)==fs.end())fs.push_back(fi);
for(int fi:fs){if(fdead[fi])continue;const Face&f=faces[fi];
Vec3 p[3]={verts[f.v[0]],verts[f.v[1]],verts[f.v[2]]};
old6+=dot(p[0],cross(p[1],p[2]));int hit=-1,hits=0;
for(int k=0;k<3;++k)if(f.v[k]==a||f.v[k]==b){hit=k;++hits;}
if(hits!=1)continue;
if(hit==0)coef=coef+cross(p[1],p[2]);
else if(hit==1)coef=coef+cross(p[2],p[0]);
else coef=coef+cross(p[0],p[1]);
}
double len=norm(coef);if(len<1e-14*max(1.0,diag*diag))return q;
Vec3 n=coef/len;double pd=-old6/len;
Quadric v;v.a=n.x*n.x;v.b=n.x*n.y;v.c=n.x*n.z;v.d=n.x*pd;
v.e=n.y*n.y;v.f=n.y*n.z;v.g=n.y*pd;v.h=n.z*n.z;v.i=n.z*pd;v.j=pd*pd;
v.scale(max(1e-12,q.a+q.e+q.h));q+=v;return q;
}
...
Quadric q=edgePlacementQuadric(a,b);
