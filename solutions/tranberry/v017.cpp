...
static constexpr double HParam_KeepRatio_UpTo25k = 0.288;

static constexpr double HParam_KeepRatio_UpTo45k = 0.144;

static constexpr double HParam_KeepRatio_UpTo50k = 0.09;

static constexpr double HParam_KeepRatio_UpTo400k = 0.0225;

static constexpr double HParam_KeepRatio_Huge = 0.0288;
...
static constexpr int TParam_UnifiedPostOrder = 1;
...
public:
    void run()
    {
        ...
        if (HParam_EnableVegaSsimPass && elapsed() < HParam_TimeBudgetSeconds - 2.2) vegaSsimStarPass();
        if (elapsed() < HParam_TimeBudgetSeconds - 1.7) collapseInvisibleEdges();
        if (elapsed() < HParam_TimeBudgetSeconds - 1.25) valenceWeldPass();
        if (elapsed() < HParam_TimeBudgetSeconds - 0.90) pairDiskPass();
        if (elapsed() < HParam_TimeBudgetSeconds - 0.22) valenceWeldPass();
        compact(); writeMesh();
    }
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.1305), safeTarget = (int)floor(nV * 0.144);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.324, 0.297, 0.270} : vector<double>{0.126, 0.090, 0.072};
        for (double kr : st)
        {
    ...
    double perceptualRankCost(int ab, int kp, const Vec3 &p) const
    {
        ...
        return 0.02 * costCap*min(8.0,max(0.0,s));
    }
    ...
    CollapseCandidate computeBestValid(int a, int b) const
    {
        ...
        for (int i = 0; i < np; ++i)
        {
            ...
            for (int dir = 0; dir < 2; ++dir)
            {
                ...
                if (c.cost - c.geomCost > 0.0015 * costCap) continue;
