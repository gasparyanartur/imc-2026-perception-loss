// Tranberry v116: v105 + parameter-free minimax render-geometry placement on tiers 2 and 3.
...
Vec3 minimaxBridge(const Quadric&qr,const Quadric&qg,const Vec3&rp,const Vec3&gp)const{double g0=qg.evaluate(gp),g1=qg.evaluate(rp),r0=qr.evaluate(gp),r1=qr.evaluate(rp),gd=max(1e-18,g1-g0),rd=max(1e-18,r0-r1),lo=0,hi=1;for(int it=0;it<12;++it){double t=0.5*(lo+hi);Vec3 p=gp*(1-t)+rp*t;double gn=(qg.evaluate(p)-g0)/gd,rn=(qr.evaluate(p)-r1)/rd;if(gn<rn)lo=t;else hi=t;}double t=0.5*(lo+hi);return gp*(1-t)+rp*t;}
...
Vec3 pos[6];int np;getCandidatePositions(a,b,q,pos,np);if(screenTier==2||screenTier==3){Vec3 rp,gp;if(solveQem3x3(q,rp)&&solveQem3x3(qc,gp)&&np<6)pos[np++]=minimaxBridge(q,qc,rp,gp);}
