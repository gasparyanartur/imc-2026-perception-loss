...
void absoluteQemEndgame(){
    ...
    vector<Vec3>safeV=verts,bestV=verts;vector<Face>safeF=faces,bestF=faces;vector<double>safeR=compactedRadius,bestR=compactedRadius;
    if(safeR.size()!=safeV.size())safeR.assign(safeV.size(),0.0);bestR=safeR;double oldDiag=diag,oldHausd=hausd;int R=inputV<=25000?1024:512;
    ...
    if(elapsed()>HParam_TimeBudgetSeconds-4.2)return;RefScore safeScore=compareToReference(ref,safeV,safeF,R);double minSafe=inputV<=25000?0.9200:0.9180;if(safeScore.finalScore<minSafe)return;
    double sr=double(safeV.size())/double(inputV);vector<double>tries={max(0.04,sr*0.65),max(0.04,sr*0.75),max(0.04,sr*0.85),max(0.04,sr*0.92)};
    for(double ratio:tries){if(elapsed()>HParam_TimeBudgetSeconds-1.25)break;restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);continueAbsoluteQem(ratio);if(verts.size()>=bestV.size())continue;RefScore cand=compareToReference(ref,verts,faces,R);if(absoluteQemBudget(safeScore,cand)){bestV=verts;bestF=faces;bestR=compactedRadius;}}
    verts.swap(bestV);faces.swap(bestF);compactedRadius.swap(bestR);nV=(int)verts.size();nF=(int)faces.size();
}
