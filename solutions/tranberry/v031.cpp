...
double cost=CParam_Inf,priority=CParam_Inf,mergedRadius=0.0;

Vec3 position;

bool operator<(const CollapseCandidate& o)const{return priority>o.priority;}

bool valid()const{return absorbed>=0&&kept>=0&&cost<CParam_Inf&&priority<CParam_Inf;}
...
if(nV>5000&&nV<=50000){
if(nV>25000&&nV<=45000){runScreenCoreMid();compact();}
...
if(elapsed()<HParam_TimeBudgetSeconds-0.35)runLargeCameraTx(inputV);
else compact();
writeMesh();
return;
}
...
c.position=p;c.cost=q.evaluate(p);c.priority=c.cost;return c;
...
void absoluteQemEndgame(){
    if(!(inputV>5000&&inputV<=25000))return;
    if(sourceVerts.empty()||sourceFaces.empty()||inputV<=5000||inputV>50000)return;
    if(elapsed()>HParam_TimeBudgetSeconds-6.0){sourceVerts.clear();sourceFaces.clear();return;}
    vector<Vec3>safeV=verts;vector<Face>safeF=faces;vector<double>safeR=compactedRadius;
    if(safeR.size()!=safeV.size())safeR.assign(safeV.size(),0.0);
    double oldDiag=diag,oldHausd=hausd;
    const int R=1024;
    vector<RefImage>ref(6);for(int v=0;v<6;++v)ref[v]=renderReference(sourceVerts,sourceFaces,v,R);
    sourceVerts.clear();sourceFaces.clear();
    if(elapsed()>HParam_TimeBudgetSeconds-4.2)return;
    RefScore safeScore=compareToReference(ref,safeV,safeF,R);
    double minSafe=inputV<=25000?0.9200:(inputV<=45000?0.9180:0.9250);
    if(safeScore.finalScore<minSafe||elapsed()>HParam_TimeBudgetSeconds-3.2)return;

    vector<double>tries;
    if(inputV<=25000)tries={0.245,0.255,0.265,0.275,0.285};
    else if(inputV<=45000)tries={0.112,0.120,0.127,0.134,0.140};
    else tries={0.057,0.062,0.067,0.072};

    for(double ratio:tries){
        if(elapsed()>HParam_TimeBudgetSeconds-1.25)break;
        restoreSafeContinuation(safeV,safeF,safeR,oldDiag,oldHausd);
        continueAbsoluteQem(ratio);
        if(verts.size()>=safeV.size())continue;
        RefScore cand=compareToReference(ref,verts,faces,R);
        if(absoluteQemBudget(safeScore,cand))return;
    }
    verts.swap(safeV);faces.swap(safeF);compactedRadius.swap(safeR);nV=(int)verts.size();nF=(int)faces.size();
}
...
int solverMain(){QemSimplifier s;s.run();return 0;}
}

int main() {
    std::vector<char> input; char chunk[65536]; size_t n;
    while ((n = fread(chunk, 1, sizeof(chunk), stdin)) > 0) input.insert(input.end(), chunk, chunk + n);
    input.push_back(0); char *p = input.data(); long vertices = strtol(p, &p, 10);
    FILE *mem = fmemopen(input.data(), input.size() - 1, "r"); if (!mem) return 2; stdin = mem;
    int rc = vertices > 50000 ? pine_solver::solverMain() : mid_solver::solverMain(); fclose(mem); return rc;
}
