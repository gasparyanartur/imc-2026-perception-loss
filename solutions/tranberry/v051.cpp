...
private:
    ...
    double futureCollapsibility(int ab, int kp, const Vec3 &p) const
    {
        ...
        double viable = 0.0, total = 0.0;
        double used = max(crad[ab] + norm(verts[ab] - p), crad[kp] + norm(verts[kp] - p));
        auto scan = [&](int v) {
            for (int nb : vneigh[v]) if (nb != ab && nb != kp && !vdead[nb]) {
                ++total;
                double next = used + norm(p - verts[nb]) + crad[nb];
                if (next < hausd) viable += 1.0 - next / max(hausd, 1e-30);
            }
        };
        scan(ab); scan(kp);
        return total > 0.0 ? viable / total : 0.0;
    }
    ...
    CollapseCandidate makeCandidate(int ab, int kp, const Vec3 &p, const Quadric &q) const
    {
        ...
        c.position = p;
        c.cost = q.evaluate(p);
        c.rank = c.cost * (1.30 - 0.30 * futureCollapsibility(ab, kp, p));
        return c;
    }
