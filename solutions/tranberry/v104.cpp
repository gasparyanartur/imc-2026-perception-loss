// Tranberry v104: v102 + persistent perceptual placement with geometry-only collapse cost.
...
Quadric geometryEdgeQuadric(int a,int b)const{Quadric q;vector<int>fs=vfaces[a];for(int fi:vfaces[b])if(find(fs.begin(),fs.end(),fi)==fs.end())fs.push_back(fi);for(int fi:fs)if(!fdead[fi])q+=geometryFaceQuadric(fi);return q;}
