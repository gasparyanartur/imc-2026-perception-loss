// Tranberry v112: v105 + pixel-support-weighted projective Jacobians on every visible face.
...
if((screenTier==2||screenTier==3)&&faceViews[fi]){int pc=max(1,__builtin_popcount((unsigned)faceViews[fi]));for(int view=0;view<6;++view)if(faceViews[fi]&(1u<<view))for(int k=0;k<3;++k){Quadric rq=screenJacobianQuadric(verts[f.v[k]],view);rq.scale((facePix[fi]+10.0*faceSil[fi]+1.0)/((faceImpMean+1.0)*pc));rayAnchor[f.v[k]]+=rq;}}
