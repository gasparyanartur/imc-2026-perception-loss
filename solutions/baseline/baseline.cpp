...
// Metric (QEM) but with the replacement vertex restricted to one of the two
// endpoints (endpoint-only placement). This keeps every output vertex on the
// original surface and makes the Hausdorff bound easy to track. A collapse is
// applied only when it preserves a closed 2-manifold and respects the
// evaluator's gates:
...
// Numerical tolerances (kept identical to baseline.py).
...
// 0.4 keeps every dataset scenario valid with a safe SSIM margin while
// preserving aggressive compression.
static const double NORMAL_FLIP_COS = 0.4;
