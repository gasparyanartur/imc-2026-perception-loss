// Pineapple v179: v174 + T5/T6 weld maxOldDev/maxNewDev bumped. Apply lenient dev to all.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        int t = originalTier();
        if (t == 4)
            return {16, 0.025, 0.034, 0.66, 0.0110, 2800, 280000, 1, 0.52, 2.30};
        if (t == 5)
            return {10, 0.030, 0.040, 0.80, 0.0150, 1500, 280000, 1, 0.60, 2.80};
        if (t == 6)
            return {8, 0.034, 0.048, 0.95, 0.0180, 2500, 280000, 1, 0.68, 5.00};
