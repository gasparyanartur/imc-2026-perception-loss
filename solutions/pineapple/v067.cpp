// Pineapple v067: v061 keepRatios AND lower face weight floor
// (0.06 -> 0.04 for T2/T4, 0.12 -> 0.08 for T3) for stronger differentiation
// between important and unimportant faces.
// Hypothesis: lower floor makes the w=1.0 "no-change" value easier to
// exceed for marginal faces, sharpening the collapse ordering away from
// perceptually-important faces.
//   Keep ratios: same as v061
//   Caps: 25 uniform (same as v061)
//   Floor: T2/T4 0.06 -> 0.04, T3 0.12 -> 0.08
#include <bits/stdc++.h>
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(25.0, max(0.04, 0.06 + 0.94 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(25.0, max(0.08, 0.11 + 0.89 * sqrt(max(0.0, r))));
                else
                    w = min(25.0, max(0.04, 0.06 + 0.94 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
