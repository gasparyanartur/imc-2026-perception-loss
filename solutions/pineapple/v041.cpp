// Pineapple v041: Area-weighted vertex repositioning as post-pass.
// Hypothesis: After QEM collapses, vertex positions may be sub-optimal.
// Move each vertex toward the area-weighted average of its incident face
// centroids (alpha=0.5). This is a different smoothing operator than
// Laplacian - it weights by triangle area, giving flat regions more
// influence and preserving sharp features.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_AreaWeightedReposition = true;
static constexpr double HParam_Pineapple_AreaWeightedAlpha = 0.5;
static constexpr int HParam_Pineapple_AreaWeightedRounds = 1;
...
private:
    ...
    void compact()
    {
        ...
        if (HParam_Pineapple_AreaWeightedReposition)
        {
            for (int round = 0; round < HParam_Pineapple_AreaWeightedRounds; ++round)
            {
                vector<Vec3> newPos(verts.size(), Vec3());
                vector<double> weight(verts.size(), 0.0);
                for (int fi = 0; fi < (int)faces.size(); ++fi)
                {
                    ...
                    if (a == b || b == c || a == c)
                        continue;
                    if (a < (int)vdead.size() && vdead[a])
                        continue;
                    if (b < (int)vdead.size() && vdead[b])
                        continue;
                    if (c < (int)vdead.size() && vdead[c])
                        continue;
                    Vec3 n = cross(verts[b] - verts[a], verts[c] - verts[a]);
                    double area = 0.5 * norm(n);
                    if (area < 1e-30)
                        continue;
                    Vec3 centroid = (verts[a] + verts[b] + verts[c]) / 3.0;
                    newPos[a] = newPos[a] + centroid * area;
                    newPos[b] = newPos[b] + centroid * area;
                    newPos[c] = newPos[c] + centroid * area;
                    weight[a] += area;
                    weight[b] += area;
                    weight[c] += area;
                }
                for (int i = 0; i < (int)verts.size(); ++i)
                {
                    ...
                    if (weight[i] > 0.0)
                    {
                        Vec3 target = newPos[i] / weight[i];
                        verts[i] = verts[i] + (target - verts[i]) * HParam_Pineapple_AreaWeightedAlpha;
                    }
                }
            }
        }
