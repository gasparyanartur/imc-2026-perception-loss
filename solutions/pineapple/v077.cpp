// Pineapple v077: Cost cap coefficient raised 0.0375 → 0.0450 (+20%). The
// costCap gates per-edge QEM accept decisions in collapseLoop. A higher cap
// means more expensive (but still bounded) collapses can proceed during the
// main loop, freeing tail-batch time for other work. Hypothesis: v055 cap is
// tuned for screen-tier safety; for T5/T6/T7 the cap can be more permissive
// without breaking Hausdorff. Affects ALL tiers via initScale().
// We also tighten the costCap coefficient formula slightly to keep the cap
// linear with mesh scale (diag^2), so larger meshes don't get scaled caps.
#include <bits/stdc++.h>
