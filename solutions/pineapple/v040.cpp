// Pineapple v040: Vertex smoothing post-pass on screen-core meshes.
// Hypothesis: Nebula's screen-core path returns directly after collapseLoop.
// A Laplacian smoothing pass (move each vertex toward its 1-ring centroid)
// can reduce geometric error without changing topology, allowing the same
// number of vertices to better approximate the original.
// T2/T3/T4 only. Try alpha=0.3 (move 30% toward 1-ring centroid).
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_LaplacianSmoothing = true;
static constexpr double HParam_Pineapple_LaplacianAlpha = 0.30;
static constexpr int HParam_Pineapple_LaplacianRounds = 1;
...
private:
    ...
    void compact()
    {
        ...
        if (HParam_Pineapple_LaplacianSmoothing)
        {
            for (int round = 0; round < HParam_Pineapple_LaplacianRounds; ++round)
            {
                ...
                for (int fi = 0; fi < (int)faces.size(); ++fi)
                {
                    ...
                    if (c < (int)vdead.size() && vdead[c])
                        continue;
                    newPos[a] = newPos[a] + verts[b] + verts[c];
                    newPos[b] = newPos[b] + verts[a] + verts[c];
                    newPos[c] = newPos[c] + verts[a] + verts[b];
                    deg[a] += 2;
                    deg[b] += 2;
                    deg[c] += 2;
                }
                for (int i = 0; i < (int)verts.size(); ++i)
                {
                    ...
                    if (deg[i] > 0)
                    {
                        Vec3 ringCentroid = newPos[i] / (double)deg[i];
                        verts[i] = verts[i] + (ringCentroid - verts[i]) * HParam_Pineapple_LaplacianAlpha;
                    }
                }
            }
        }
