// Pineapple v076: Face weight formula. Lower the weight floor for unimportant
// faces (the constant 0.08 → 0.04) and reduce the constant offset for the
// non-linearity (0.92 → 0.96, larger fraction gated by r). Hypothesis: the
// weight formula caps at 1.0+ but the floor of 0.08 means even unimportant
// faces contribute meaningful quadric mass. Lowering the floor (and pushing
// more weight range into the gated non-linear term) makes unimportant faces
// collapse more aggressively while important faces are still well protected.
// Affects ALL faces on T2 and T4 (screen-tier 2 and 4 use the same formula).
#include <bits/stdc++.h>
