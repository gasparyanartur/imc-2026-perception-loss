// Pineapple v006: Silhouette plane quadric carried into MEMLESS rebuilds.
// Hypothesis: Nebula's MEMLESS rebuild calls weightedFaceQuadric(fi) which
// recomputes per-face energy but does NOT include the silhouette plane
// energy that the screen core weights onto each vertex. After MEMLESS
// rebuild, the silhouette protection is silently dropped. Carrying the
// silhouette plane energy into weightedFaceQuadric preserves it through
// the rebuild, while leaving non-screen tiers unaffected.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_MemlessSilhouette = true;
static constexpr double HParam_Pineapple_MemlessSilhouetteWeight = 0.40;
...
private:
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            // Pineapple: carry silhouette plane energy into MEMLESS rebuild.
            // The screen core adds these per-vertex quadrics in initFaceWeights,
            // but MEMLESS rebuild calls weightedFaceQuadric(fi) which recomputes
            // per-face energy from scratch. Without this, the silhouette plane
            // protection silently disappears on every collapse.
                ...
                fi < (int)faceSil.size() && faceSil[fi] > 0.0)
            {
                ...
                for (int k = 0; k < 3; ++k)
                {
                    ...
                    qs.j = pd * pd;
                    qs.scale(sw);
                    // MEMLESS rebuild adds weightedFaceQuadric(fi) to each of
                    // the three vertices. Adding half here is correct because
                    // the silhouette plane is per-edge (shared by exactly 2
                    // faces). So per-face, we add half the silhouette plane
                    // energy, then the neighbor face adds the other half.
                    qs.scale(0.5);
                    q += qs;
                }
            }
