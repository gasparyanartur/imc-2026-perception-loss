// Pineapple v044: Penalty based on both faceSil AND facePix (v042 redux).
// Hypothesis: facePix measures how much a face is visible (any view);
// faceSil measures silhouette. Combining both (penalty = 1 + 0.3 * (silRatio
// + 0.3 * pixRatio)) protects both visible and silhouette areas.
#include <bits/stdc++.h>
...
static constexpr double HParam_Pineapple_ImportancePenaltyPixRatio = 0.30;
...
private:
    ...
    CollapseCandidate computeQueueCandidate(int a, int b) const
    {
        ...
        {
            double maxSil = 0.0, maxPix = 0.0;
            for (int fi : vfaces[a])
            {
                if (fi < (int)faceSil.size() && !fdead[fi])
                {
            ...
            double pixRatio = maxPix / faceImpMean;
            penalty = 1.0 + HParam_Pineapple_ImportancePenaltyWeight * silRatio
                          + HParam_Pineapple_ImportancePenaltyPixRatio * pixRatio;
