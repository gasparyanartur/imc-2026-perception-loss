// Pineapple v078: Composite of v075 (Vega aggressiveness) + v076 (face weight
// floor) + v077 (cost cap +20%). Combines three tier-uniform levers:
// 1. Vega SSIM min 0.95→0.93 (T2), 0.94→0.92 (T3); maxDamage +0.015
// 2. Face weight formula: floor 0.08→0.04 (T2/T4), 0.15→0.10 (T3)
// 3. QemCostCapCoeff 0.0375 → 0.0450 (+20%)
// Hypothesis: these three changes are orthogonal: Vega loosens post-pass gate,
// face weights redistribute quadric mass, cost cap widens QEM accept window.
// All three should compound toward more aggressive collapses while keeping
// the high-importance protections (caps, anchor weights).
#include <bits/stdc++.h>
...
static constexpr double HParam_QemCostCapCoeff = 0.0450;
...
static constexpr int HParam_VegaCandidatePoolCap = 42000;
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(18.0, max(0.03, 0.04 + 0.96 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(7.0, max(0.08, 0.10 + 0.90 * sqrt(max(0.0, r))));
                else
                    w = min(16.0, max(0.03, 0.04 + 0.96 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
