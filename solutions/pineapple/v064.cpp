// Pineapple v064: extended v061 direction - even more aggressive
// keepRatios (-0.02) AND cap=28 uniform.
// Hypothesis: v061 was the best local performer (+1pt on T2 cases).
// Pushing further tests if we're under-utilizing the safety margin.
//   T2:  0.36, 0.33, 0.30 -> 0.34, 0.31, 0.28
//   T4:  0.14, 0.10, 0.08 -> 0.12, 0.08, 0.06
//   T3:  0.145 -> 0.130
//   Caps: T2/T3/T4 = 28 uniform
#include <bits/stdc++.h>
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.130), safeTarget = (int)floor(nV * 0.16);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.34, 0.31, 0.28} : vector<double>{0.12, 0.08, 0.06};
        for (double kr : st)
        {
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(28.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(28.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(28.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(28.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(28.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
