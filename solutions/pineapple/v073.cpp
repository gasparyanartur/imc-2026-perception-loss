// Pineapple v073: Anchor boost weakened. Lower the bw coefficients by 50%
// (T2: 0.0000035 → 0.0000018; T4: 0.0000050 → 0.0000025) and cap the per-vertex
// anchor weight at 9.0 instead of 18.0 (50% reduction). Hypothesis: current
// anchor protection is excessive - hidden mesh regions have zero screen-core
// slack, but anchor boost blocks acceptable QEM collapses. Weakening lets the
// QEM select more aggressive collapses while still protecting high-curvature
// regions. Affects ALL screen-core tiers (T2 and T4 since these are the only
// tiers that compute anchor weights).
#include <bits/stdc++.h>
...
private:
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier == 2 || screenTier == 4)
        {
            ...
            am /= max(1, ac);
            // v073: Anchor boost weakened by 50% (bw 0.5x) and cap reduced 18.0 → 9.0
            double bw = (screenTier == 2 ? 0.0000018 : 0.0000025) * diag * diag / (am + 1.0);
            for (int i = 0; i < nV; ++i)
                if (!vdead[i] && vertAnchor[i] > 0)
                    vquad[i] += anchorPointQuadric(verts[i], bw * min(9.0, vertAnchor[i] / (am + 1e-12)));
            if (screenTier == 4)
                for (int i = 0; i < nV; ++i)
                    if (!vdead[i])
                        vquad[i] += vmoment[i];
