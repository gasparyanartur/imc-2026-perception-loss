// Pineapple v030: Big multi-direction candidate #1.
// Combines: (a) T2/T4 stage restructuring (5 stages vs Nebula's 3),
// (b) T3 safe ratio tightening, (c) face weight cap boost,
// (d) T5/T6 keep ratio tightening, (e) post-screen-core Vega pass.
// Hypothesis: any single lever is at its limit, but multiple coordinated
// changes can collectively push the frontier. Each tier gets a distinct
// treatment, with the smallest individual changes per tier.
#include <bits/stdc++.h>

static constexpr double HParam_Pineapple_T2Stages[5] = {0.36, 0.34, 0.32, 0.30, 0.28};
static constexpr double HParam_Pineapple_T4Stages[5] = {0.14, 0.12, 0.10, 0.09, 0.075};
static constexpr double HParam_Pineapple_T3SafeRatio = 0.155;
static constexpr double HParam_Pineapple_T3FinalRatio = 0.142;
static constexpr double HParam_Pineapple_FaceWeightCapT2 = 24.0;
static constexpr double HParam_Pineapple_FaceWeightCapT4 = 22.0;
static constexpr double HParam_Pineapple_KeepRatio_UpTo400k = 0.023;
static constexpr double HParam_Pineapple_KeepRatio_Huge = 0.030;
static constexpr bool HParam_Pineapple_PostScreenVega = true;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * HParam_Pineapple_T3FinalRatio), safeTarget = (int)floor(nV * HParam_Pineapple_T3SafeRatio);
            ...
            if (HParam_Pineapple_PostScreenVega && HParam_EnableVegaSsimPass &&
                elapsed() < HParam_TimeBudgetSeconds - 0.65)
            {
                vegaSsimStarPass();
            }
        ...
        vector<double> st = screenTier == 2 ? vector<double>(HParam_Pineapple_T2Stages, HParam_Pineapple_T2Stages + 5) : vector<double>(HParam_Pineapple_T4Stages, HParam_Pineapple_T4Stages + 5);
        for (double kr : st)
        {
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(HParam_Pineapple_FaceWeightCapT2, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(7.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(HParam_Pineapple_FaceWeightCapT4, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(HParam_Pineapple_FaceWeightCapT2, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(HParam_Pineapple_FaceWeightCapT4, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
