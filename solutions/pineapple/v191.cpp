// Pineapple v191: v182 + T5/T6 weld maxOldDev/maxNewDev bumped. Apply to all.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        int t = originalTier();
        if (t == 4)
            return {16, 0.050, 0.060, 0.66, 0.0110, 2800, 280000, 1, 0.52, 2.30};
        if (t == 5)
            return {10, 0.050, 0.060, 0.80, 0.0150, 1500, 280000, 1, 0.60, 2.80};
        if (t == 6)
            return {8, 0.050, 0.060, 0.95, 0.0180, 2500, 280000, 1, 0.68, 5.00};
