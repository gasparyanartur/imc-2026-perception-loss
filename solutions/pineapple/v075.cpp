// Pineapple v075: Vega SSIM aggressiveness. Lower SSIM floors by 0.02 across
// all tiers, raise maxDamage by 0.015, increase candidate pool 28000 → 42000.
// Hypothesis: v055 Vega thresholds (minS 0.95/0.94) are conservative; lower
// thresholds allow Vega to accept more star/pair deletions during post-passes.
// Since Vega is the LAST post-pass (third pass at 20.08s), more aggressive
// Vega acceptance directly reduces final vertex count. Affects ALL tiers.
#include <bits/stdc++.h>
...
private:
    ...
    double vegaSsimMin() const
    {
        ...
        if (tier == 2)
            return 0.93; // v075: more aggressive

        if (tier == 3)
            return 0.92; // v075: more aggressive
    ...
    double vegaMaxDamage() const
    {
        ...
        if (tier == 2)
            return 0.067; // v075: +0.015

        if (tier == 3)
            return 0.072; // v075: +0.015
