// Pineapple v014: Vega gate with high-faceSil gate trigger only.
// Hypothesis: v002's gate fires on every collapse, but official collapses are
// usually perceptually fine; only the high-faceSil ones are risky. v014 only
// invokes the Vega check when at least one incident face has faceSil above
// 0.5*faceImpMean. This concentrates the budget on the dangerous cases.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_VegaGate = true;
static constexpr double HParam_Pineapple_VegaGateMinSsim = 0.965;
static constexpr double HParam_Pineapple_VegaGateMaxDamage = 0.040;
static constexpr int HParam_Pineapple_VegaGateResolution = 384;
static constexpr int HParam_Pineapple_VegaGatePaddingPixels = 4;
static constexpr int HParam_Pineapple_VegaGateMaxPixels = 36000;
static constexpr double HParam_Pineapple_VegaGateBudgetSeconds = 12.0;
static constexpr double HParam_Pineapple_VegaGateSilTriggerRatio = 0.5;
...
private:
    ...
    double pineappleVegaSpent = 0.0;
    int pineappleVegaChecks = 0;
    ...
    void collapseLoop()
    {
        ...
        while (accepted < collapseLimit && (!pq.empty() || tailMode()))
        {
            ...
            if (HParam_Pineapple_VegaGate && screenTier > 0 &&
                pineappleVegaBudgetLeft() > 0 &&
                pineappleHasSilTrigger(best.absorbed, best.kept))
            {
                auto gateStart = chrono::steady_clock::now();
                double s = localVegaSsimForCollapse(best.absorbed, best.kept, best.position);
                auto gateEnd = chrono::steady_clock::now();
                pineappleVegaSpent += chrono::duration<double>(gateEnd - gateStart).count();
                ++pineappleVegaChecks;
                if (s >= 0.0 && (s < HParam_Pineapple_VegaGateMinSsim ||
                                 1.0 - s > HParam_Pineapple_VegaGateMaxDamage))
                    continue;
            }
    ...
    // ---------- Pineapple: Vega-gated QEM acceptance ----------

    static constexpr double pineappleVegaBudgetTotal = HParam_Pineapple_VegaGateBudgetSeconds;

    bool pineappleVegaBudgetLeft() const
    {
        return pineappleVegaSpent < pineappleVegaBudgetTotal;
    }

    bool pineappleHasSilTrigger(int a, int b) const
    {
        if (faceSil.empty() || faceImpMean <= 0.0)
            return false;
        double thresh = HParam_Pineapple_VegaGateSilTriggerRatio * faceImpMean;
        for (int fi : vfaces[a])
        {
            if (fi < (int)faceSil.size() && !fdead[fi] && faceSil[fi] >= thresh)
                return true;
        }
        for (int fi : vfaces[b])
        {
    ...
    double localVegaSsimForCollapse(int ab, int kp, const Vec3 &np) const
    {
        if (ab < 0 || kp < 0 || ab >= (int)verts.size() || kp >= (int)verts.size())
            return -1.0;

        vector<VegaTri> oldTris, newTris;

        // Build old tris: triangles touching ab or kp (excluding the two common faces)
        // We need to include both common faces (their "before" is different from after collapse)
        // and the other incident faces that will be transformed.
        ...
        for (int fi : vfaces[kp])
        {
            if (fi < (int)fdead.size() && fdead[fi])
                continue;
            const Face &f = faces[fi];
            // skip faces already added (the 2 common faces appear in both vfaces lists)
            bool already = false;
            for (const VegaTri &tt : oldTris)
            {
                // cheap dedup by checking 3 vertices within epsilon
                if (norm2(tt.p[0] - verts[f.v[0]]) < 1e-30 &&
                    norm2(tt.p[1] - verts[f.v[1]]) < 1e-30 &&
                    norm2(tt.p[2] - verts[f.v[2]]) < 1e-30)
                {
                    already = true;
                    break;
                }
        ...
        // Build new tris: substitute kp -> np in faces touching ab, and
        // substitute ab -> np in faces touching kp (that weren't common).
        for (int fi : vfaces[ab])
        {
            if (fi < (int)fdead.size() && fdead[fi])
                continue;
        ...
        for (int fi : vfaces[kp])
        {
            ...
            // skip faces already added (the 2 common faces appear in both vfaces lists)
            bool already = false;
            for (const VegaTri &tt : newTris)
            {
                if (norm2(tt.p[0] - ((f.v[0] == ab) ? np : verts[f.v[0]])) < 1e-30 &&
                    norm2(tt.p[1] - ((f.v[1] == ab) ? np : verts[f.v[1]])) < 1e-30 &&
                    norm2(tt.p[2] - ((f.v[2] == ab) ? np : verts[f.v[2]])) < 1e-30)
                {
                    already = true;
                    break;
                }
        ...
        if (oldTris.empty() || newTris.empty())
            return -1.0;

        const int R = HParam_Pineapple_VegaGateResolution;
        ...
        for (int view = 0; view < 6; ++view)
        {
            ...
            int pad = HParam_Pineapple_VegaGatePaddingPixels;
            ...
            if (w * h > HParam_Pineapple_VegaGateMaxPixels)
                return -1.0;
