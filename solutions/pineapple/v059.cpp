// Pineapple v059: All face weight formulas unified to T3-style
// (+10*faceSil) AND all face weight caps uniformly raised to 12.0.
// Hypothesis: the T3 face formula is fundamentally better behaved;
// unifying to it should give more conservative collapse ordering on
// T2/T4 (less sil over-emphasis) while uniformly higher caps allow
// more aggressive protection on heavily-rasterized faces.
//   imp formula: T2/T4 -> +10*faceSil (was +12*faceSil+2*faceWin)
//   caps: T2 18->12, T3 7->12, T4 16->12 (uniform)
#include <bits/stdc++.h>
...
private:
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier)
            for (int fi = 0; fi < nF; ++fi)
                if (!(fi < (int)fdead.size() && fdead[fi]))
                {
                    faceImpMean += facePix[fi] + 10.0 * faceSil[fi];
                    ++activeF;
                }
        if (screenTier)
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                double imp = facePix[fi] + 10.0 * faceSil[fi];
                double r = (imp + 0.20 * faceImpMean) / (faceImpMean + 1e-12);
                double w;
                if (screenTier == 2)
                    w = min(12.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(12.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(12.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            double imp = facePix[fi] + 10.0 * faceSil[fi];
            double r = (imp + 0.20 * faceImpMean) / (faceImpMean + 1e-12);
            double w = screenTier == 2 ? min(12.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(12.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
