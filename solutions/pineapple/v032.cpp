// Pineapple v032: Aggressive Post-pass direction.
// Combines: (a) T2 stages 0.36, 0.32, 0.29 (bigger drop than Nebula),
// (b) T4 stages 0.14, 0.09, 0.075, (c) T2 face weight floor 0.10,
// (d) T5 keepRatio 0.025->0.022, (e) star-delete on screen-core T2/T4.
// Hypothesis: post-pass addition on screen core is the key. Tangerine
// tested activating these passes on screen core (v011-v015) but they
// all failed. Different combination here: tighter stages + floor boost.
#include <bits/stdc++.h>

static constexpr double HParam_Pineapple_T2Stages[3] = {0.36, 0.32, 0.29};
static constexpr double HParam_Pineapple_T4Stages[3] = {0.14, 0.09, 0.075};
static constexpr double HParam_Pineapple_FaceWeightFloorT2 = 0.10;
static constexpr double HParam_Pineapple_KeepRatio_UpTo400k = 0.022;
static constexpr bool HParam_Pineapple_PostScreenStar = true;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        vector<double> st = screenTier == 2 ? vector<double>(HParam_Pineapple_T2Stages, HParam_Pineapple_T2Stages + 3) : vector<double>(HParam_Pineapple_T4Stages, HParam_Pineapple_T4Stages + 3);
        for (double kr : st)
        {
        ...
        if (HParam_Pineapple_PostScreenStar && elapsed() < HParam_TimeBudgetSeconds - 0.7)
        {
            collapseInvisibleEdges();
        }
    }
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(18.0, max(HParam_Pineapple_FaceWeightFloorT2, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(18.0, max(HParam_Pineapple_FaceWeightFloorT2, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(16.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
