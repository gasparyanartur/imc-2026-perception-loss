// Pineapple v049: Tighter Hausdorff bound (0.055 -> 0.060 diag).
// Hypothesis: Nebula uses Hausdorff bound 0.055 * diag. The judges' actual
// bound is 0.05 * diag. The extra 0.005 of margin may be unnecessary.
// Tightening the bound (0.060) allows slightly larger collapses that
// still fit within the judge's actual 0.05 limit, gaining compression.
// Applied only on T5/T6 (large tiers where Hausdorff cap is binding).
#include <bits/stdc++.h>

static constexpr double HParam_Pineapple_HausdorffDiagFraction = 0.060;

using namespace std;

static constexpr double CParam_HausdorffDiagFraction = HParam_Pineapple_HausdorffDiagFraction;
...
private:
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier == 2 || screenTier == 4)
        {
            ...
            am /= max(1, ac);
            double bw = (screenTier == 2 ? 0.0000035 : 0.0000060) * diag * diag / (am + 1.0);
            for (int i = 0; i < nV; ++i)
    ...
    StarParams starParams() const
    {
        ...
        StarParams p1 = {12, 0.160, 0.220, 1.18, 0.0600, 30000, 820000, 8, 0.90, 6.20};
    ...
    StarParams vegaSsimParams() const
    {
        ...
        StarParams p2 = {8, 0.024, 0.060, 0.90, 0.0200, 1250, 175000, 1, 0.44, 1.95};
    ...
    RootNudgeParams rootNudgeParams() const
    {
        ...
        else if (HParam_RootNudgeProfile == 2)
        {
            ...
            if (tier == 4)
                return {0.060, 0.0011, 0.94, 0.993, 34};
        }
        else if (HParam_RootNudgeProfile == 3)
        {

            if (tier == 2)
                return {0.060, 0.0010, 0.94, 0.993, 30};
