// Pineapple v005: silhouette-weighted placement (5th candidate position).
// For each collapse candidate, compute a 5th placement point: weighted centroid
// of the endpoints biased toward the visible silhouette ring centroid on the
// highest-faceSil view. Compare its quadric cost and select if it improves
// the placement quality.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_SilhouettePlacement = true;
...
private:
    ...
    void getCandidatePositions(int a, int b, const Quadric &q, Vec3 pos[5], int &np) const
    {
        ...
        if (HParam_Pineapple_SilhouettePlacement && !faceSil.empty())
        {
            Vec3 ringAccum = Vec3();
            ...
            for (int fi : vfaces[a])
            {
                if (fi < (int)fdead.size() && fdead[fi])
                    continue;
                if (fi >= (int)faceSil.size())
                    continue;
                double w = faceSil[fi] + 0.3 * facePix[fi];
                if (w <= 0.0)
                    continue;
                const Face &f = faces[fi];
                for (int k = 0; k < 3; ++k)
                {
                    if (f.v[k] == a)
                        continue;
                    ringAccum = ringAccum + verts[f.v[k]] * w;
                    ringWeight += w;
                }
            }
            for (int fi : vfaces[b])
            ...
            if (ringWeight > 1e-12)
            {
                Vec3 silhouetteCentroid = ringAccum / ringWeight;
                // bias: 0.4 * silhouette + 0.6 * midpoint
                Vec3 biased = silhouetteCentroid * 0.4 + (verts[a] + verts[b]) * 0.3;
                pos[np++] = biased;
            }
        }
