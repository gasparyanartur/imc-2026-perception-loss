// Pineapple v083: Enable weld and pair-disk post-passes for T5 AND T6.
// v079 enabled T5 only. v083 also enables T6 (≥1M). T6 has the most
// vertices, so the post-pass window (1.5s) is smaller per vertex but
// there are many more candidates. Hypothesis: T6 has stranded edges too,
// and the post-pass cleans them. Affects T5 (case 6) and T6 (case 7).
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        if (t == 6)
            return {6, 0.024, 0.038, 0.95, 0.0180, 760, 280000, 1, 0.68, 3.20};
