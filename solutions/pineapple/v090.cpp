// Pineapple v090: Composite v082 + v086 + T5 pair-disk maxValence 8→10 + scanVertices 90→150.
// v082 = T5 weld maxValence 6→8, scanVertices 760→1500 (+0.036pt local).
// v086 = T6 star-delete enabled (HParam_TailOriginalVertexThreshold 400k → 1M, +0.034pt).
// T5 pair-disk more aggressive. Hypothesis: combines all giant-tier improvements
// while testing if T5 pair-disk can match v082's weld gains.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        if (t == 5)
            return {8, 0.020, 0.032, 0.80, 0.0150, 1500, 280000, 1, 0.60, 2.80};
    ...
    StarParams pairDiskParams() const
    {
        ...
        if (t == 5)
            return {10, 0.024, 0.038, 0.78, 0.0035, 150, 120000, 1, 0.30, 0.95};
