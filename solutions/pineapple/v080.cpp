// Pineapple v080: Composite of v077 (cost cap +20%) + v079 (enable weld/pair-disk
// for T5). Hypothesis: combine widened QEM accept window with enabled giant-tier
// post-pass topology cleanup. Cost cap +20% means more expensive collapses
// survive main loop; weld/pair-disk on T5 then clean up residual stranded edges.
// Affects T5 directly (case 6), T6 indirectly via cost cap, T2-T4 via cost cap.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        if (t == 5)
            return {6, 0.020, 0.032, 0.80, 0.0150, 760, 280000, 1, 0.60, 2.80};
