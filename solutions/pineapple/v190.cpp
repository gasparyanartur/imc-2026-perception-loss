// Pineapple v190: v182 + T4 weld rounds=2 + T4 pair-disk scanVertices 90 → 500.
// Push v182 with all knobs.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        int t = originalTier();
        if (t == 4)
            return {16, 0.050, 0.060, 0.66, 0.0110, 2800, 280000, 2, 0.52, 2.30};
        if (t == 5)
    ...
    StarParams pairDiskParams() const
    {
        ...
        int t = originalTier();
        if (t == 4)
            return {8, 0.018, 0.029, 0.66, 0.0025, 500, 120000, 1, 0.28, 2.00};
        if (t == 5)
