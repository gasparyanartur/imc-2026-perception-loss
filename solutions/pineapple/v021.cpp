// Pineapple v021: Higher face weight floor (T2 0.06->0.10, T4 0.06->0.10).
// Hypothesis: raising the floor forces all faces to contribute at least
// 0.10x to the quadric. Low-importance faces become less collapsible. This
// may redistribute collapse budget toward less important regions.
// Soft lever: change the floor instead of the cap.
#include <bits/stdc++.h>

static constexpr double HParam_Pineapple_FaceWeightCapT2 = 18.0;
static constexpr double HParam_Pineapple_FaceWeightCapT4 = 16.0;
static constexpr double HParam_Pineapple_FaceWeightFloorT2 = 0.10;
static constexpr double HParam_Pineapple_FaceWeightFloorT4 = 0.10;
...
private:
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(HParam_Pineapple_FaceWeightCapT2, max(HParam_Pineapple_FaceWeightFloorT2, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(7.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(HParam_Pineapple_FaceWeightCapT4, max(HParam_Pineapple_FaceWeightFloorT4, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(HParam_Pineapple_FaceWeightCapT2, max(HParam_Pineapple_FaceWeightFloorT2, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(HParam_Pineapple_FaceWeightCapT4, max(HParam_Pineapple_FaceWeightFloorT4, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
