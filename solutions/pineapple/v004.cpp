// Pineapple v004: Persistent silhouette/crease line-quadric energy in screen
// core (T2/T3/T4). For each face with non-zero faceSil or faceWin, add an
// additional perpendicular quadric to the silhouette plane passing through
// the edge. This protects contour edges from being collapsed away.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_SilhouettePlane = true;
static constexpr double HParam_Pineapple_SilhouetteWeight = 0.45;
...
private:
    ...
    void initFaceWeights()
    {
        ...
        if (HParam_Pineapple_SilhouettePlane && screenTier > 0)
            for (int fi = 0; fi < nF; ++fi)
            {
                if (fi < (int)fdead.size() && fdead[fi])
                    continue;
                if (fi >= (int)faceSil.size())
                    continue;
                double sil = faceSil[fi];
                if (sil <= 0.0)
                    continue;
                const Face &f = faces[fi];
                Vec3 fn = cross(verts[f.v[1]] - verts[f.v[0]], verts[f.v[2]] - verts[f.v[0]]);
                double fl = norm(fn);
                if (fl < CParam_MinNormalNorm)
                    continue;
                Vec3 nn = fn / fl;
                double w = HParam_Pineapple_SilhouetteWeight * sil;
                for (int k = 0; k < 3; ++k)
                {
                    int a = f.v[k], b = f.v[(k + 1) % 3];
