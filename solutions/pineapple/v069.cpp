// Pineapple v069: Post-pass reordering. Move Vega SSIM (safest, gated) to run
// FIRST after collapseLoop. Then weld/pair-disk collapse residual structures.
// Hypothesis: running the perception-gated Vega pass first removes perceptually
// risky regions early; subsequent weld/pair-disk passes have fewer risky
// candidates to chew on, reducing floor violations at the same Hausdorff cost.
// This is a tier-uniform change since all post-passes only affect T5/T6/T7.
#include <bits/stdc++.h>
...
public:
    void run()
    {
        ...
        // v069: Reordered post-pass schedule. Vega SSIM (perception-gated, safest)
        // runs FIRST after collapseLoop, before any topology-changing passes.
        // Then weld/pair-disk collapse residual structures left by Vega.
        // Star-delete runs after Vega since it requires relatively intact topology.

        if (HParam_EnableVegaSsimPass &&
            elapsed() < HParam_TimeBudgetSeconds - 1.20)
        {
        ...
        if (elapsed() < HParam_TimeBudgetSeconds - 1.00)
        {
        ...
        if (nV <= HParam_TailOriginalVertexThreshold &&
            ...
            elapsed() < HParam_TimeBudgetSeconds - 0.95)
        {
        ...
        if (elapsed() < HParam_TimeBudgetSeconds - 0.65)
        {
        ...
        if (HParam_EnableVegaSsimPass &&
            ...
            elapsed() < HParam_TimeBudgetSeconds - 0.40)
        {
