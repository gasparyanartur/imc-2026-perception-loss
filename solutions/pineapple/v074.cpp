// Pineapple v074: Composite - aggressive tail batch (v072) + weakened anchor
// boost (v073). Combines v072's 2x scan/2x target/extended stop with v073's
// 50% anchor bw reduction + 18.0→9.0 cap. Hypothesis: the two changes target
// different parts of the pipeline - tail batch handles stranded edges in the
// QEM queue (T5/T6/T7), while anchor weakening lets the QEM be more aggressive
// in screen-core (T2/T4). Together they cover all tiers without screen-core
// keep-ratio tightening (which Kattis rejects for hidden meshes).
#include <bits/stdc++.h>
...
private:
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier == 2 || screenTier == 4)
        {
            ...
            am /= max(1, ac);
            // v074: Anchor boost weakened by 50% (bw 0.5x) and cap reduced 18.0 → 9.0
