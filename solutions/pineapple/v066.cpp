// Pineapple v066: v061 keepRatios AND tightened anchor boost (1.4x/1.2x
// -> 1.8x/1.5x) AND raise face weight floor (0.06 -> 0.08).
// Hypothesis: stronger protection of silhouette/anchor vertices + the
// v061 tightening produces better collapse ordering on visible regions.
//   Keep ratios: same as v061
//   Caps: 25 uniform (same as v061)
//   Anchor boost: T2 1.4 -> 1.8, T4 1.2 -> 1.5
//   Face weight floor: 0.06 -> 0.08 (was: 0.06 / 0.12)
#include <bits/stdc++.h>
...
static constexpr double HParam_Pineapple_AnchorBoostT2 = 1.8;
static constexpr double HParam_Pineapple_AnchorBoostT4 = 1.5;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.135), safeTarget = (int)floor(nV * 0.16);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.35, 0.32, 0.29} : vector<double>{0.13, 0.09, 0.07};
        for (double kr : st)
        {
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(25.0, max(0.08, 0.10 + 0.90 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(25.0, max(0.14, 0.17 + 0.83 * sqrt(max(0.0, r))));
                else
                    w = min(25.0, max(0.08, 0.10 + 0.90 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(25.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(25.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
