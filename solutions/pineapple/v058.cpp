// Pineapple v058: All screen-core keep ratios tightened by 0.005
// AND all face weight caps raised by 2.0.
// Hypothesis: tighter retention + higher importance separation. Fewer
// final vertices on T2/T4 stages but stronger protection on important
// faces. Tests whether more aggressive collapse + higher cap is safer
// than the baseline.
//   T2:  0.36, 0.33, 0.30 -> 0.355, 0.325, 0.295
//   T4:  0.14, 0.10, 0.08 -> 0.135, 0.095, 0.075
//   T3:  0.145 -> 0.140
//   Caps: T2 18->20, T4 16->18 (T3 7.0 unchanged)
#include <bits/stdc++.h>
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.140), safeTarget = (int)floor(nV * 0.16);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.355, 0.325, 0.295} : vector<double>{0.135, 0.095, 0.075};
        for (double kr : st)
        {
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(20.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(7.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(18.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(20.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(18.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
