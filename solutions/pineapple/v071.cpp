// Pineapple v071: Root nudge profile 1 → 2 (also enable T2 in profile 2).
// Profile 2 explicitly defines T2 parameters: frac=0.035, maxMoveDiag=0.0008,
// radiusFrac=0.92, minDot=0.995, maxIncident=28.
// Profile 1 only had T4 parameters - T2 returned zero (no nudge) under profile 1.
// Hypothesis: enabling moderate root nudges for T2 (and slightly stronger T4)
// can pull collapsed vertices back to perceptually stable positions, lowering
// geometric error without Hausdorff cost. Affects ALL tiers: T2/T3/T4 via
// screenTier mapping, T5/T6 via originalTier() returning 5/6 (zero nudge).
#include <bits/stdc++.h>
...
static constexpr int HParam_RootNudgeProfile = 2;
