// Pineapple v031: Big multi-direction candidate #2 (asymmetric direction).
// Combines: (a) T2 4-stage asymmetric (0.40, 0.34, 0.30, 0.27),
// (b) T3 final -0.5pt, (c) T4 4-stage (0.16, 0.12, 0.09, 0.075),
// (d) anchor boost 2.5x/2.0x, (e) T5 keepRatio 0.025->0.024.
// Different staging than v030: bigger initial drop, smaller final drop.
// Hypothesis: aggressive early collapse + protective anchor may find a
// better collapse ordering than Nebula's symmetric drop.
#include <bits/stdc++.h>

static constexpr double HParam_Pineapple_T2Stages[4] = {0.40, 0.34, 0.30, 0.27};
static constexpr double HParam_Pineapple_T4Stages[4] = {0.16, 0.12, 0.09, 0.075};
static constexpr double HParam_Pineapple_T3FinalRatio = 0.140;
static constexpr double HParam_Pineapple_AnchorBoostT2 = 2.5;
static constexpr double HParam_Pineapple_AnchorBoostT4 = 2.0;
static constexpr double HParam_Pineapple_KeepRatio_UpTo400k = 0.024;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * HParam_Pineapple_T3FinalRatio), safeTarget = (int)floor(nV * 0.16);
        ...
        vector<double> st = screenTier == 2 ? vector<double>(HParam_Pineapple_T2Stages, HParam_Pineapple_T2Stages + 4) : vector<double>(HParam_Pineapple_T4Stages, HParam_Pineapple_T4Stages + 4);
        for (double kr : st)
        {
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier == 2 || screenTier == 4)
        {
            ...
            double boost = screenTier == 2 ? HParam_Pineapple_AnchorBoostT2 : HParam_Pineapple_AnchorBoostT4;
            for (int i = 0; i < nV; ++i)
                if (!vdead[i] && vertAnchor[i] > 0)
                    vquad[i] += anchorPointQuadric(verts[i], bw * boost * min(18.0, vertAnchor[i] / (am + 1e-12)));
            if (screenTier == 4)
                for (int i = 0; i < nV; ++i)
                    if (!vdead[i])
                        vquad[i] += vmoment[i];
