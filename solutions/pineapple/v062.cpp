// Pineapple v062: MEMLESS=false on screen-core path (T2/T3/T4) AND
// tighter cost cap on screen-core stages via direct init.
// Hypothesis: a safer rebuild strategy + tighter cost cap prevents
// destructive collapses while still allowing the screen-core to
// execute its full 3-stage sequence. Tests if the memoryless quadric
// rebuild is dropping important weights on these tiers.
//   MEMLESS: forced false on T2/T3/T4 (was (nV > 5000))
//   costCap: scale by 0.85 on T2/T3/T4 (multiply initScale output
//            for those tiers only).
#include <bits/stdc++.h>
...
private:
    ...
    void initScale()
    {
        ...
        MEMLESS = (screenTier == 0 && nV > 5000); // v062: disable MEMLESS on screen-core path
