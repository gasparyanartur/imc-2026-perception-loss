// Pineapple v047: Cost cap decay over time.
// Hypothesis: Nebula uses constant costCap = 0.0375 * diag^2 throughout
// collapseLoop. Allowing more expensive collapses early but forcing cheaper
// ones late may enable safer collapse ordering near the SSIM cliff.
// cap(t) = 0.0375 * (1 - 0.5*t/T) * diag^2 (linear decay to half).
// This only affects the break-out condition c.cost > costCap, not the
// quadric energy itself.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_CostCapDecay = true;
static constexpr double HParam_Pineapple_CostCapDecayEndFraction = 0.50;
...
private:
    ...
    void collapseLoop()
    {
        ...
        while (accepted < collapseLimit && (!pq.empty() || tailMode()))
        {
            ...
            if (HParam_Pineapple_CostCapDecay && screenTier > 0 && collapseLimit > 0)
            {
                double t = (double)accepted / (double)collapseLimit;
                if (t > 1.0) t = 1.0;
                double decayFactor = 1.0 - HParam_Pineapple_CostCapDecayEndFraction * t;
                double currentCap = costCap * decayFactor;
                if (c.cost > currentCap)
                    continue;
            }
