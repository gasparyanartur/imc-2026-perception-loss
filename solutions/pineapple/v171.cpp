// Pineapple v171: v162 + T5/T6 weld maxSeconds 2.80/5.00 → 5.00/8.00. More giant time.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        int t = originalTier();
        if (t == 4)
            return {16, 0.015, 0.024, 0.66, 0.0110, 2800, 280000, 1, 0.52, 2.30};
        if (t == 5)
            return {10, 0.020, 0.032, 0.80, 0.0150, 1500, 280000, 1, 0.60, 5.00};
        if (t == 6)
            return {8, 0.024, 0.038, 0.95, 0.0180, 2500, 280000, 1, 0.68, 8.00};
