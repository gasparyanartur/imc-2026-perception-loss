// Pineapple v068: v061 keepRatios AND tighter cost cap on screen-core
// stages (costCap * 0.85 if screenTier != 0).
// Hypothesis: combining v061's tightening with v062's tighter cost cap
// tests if both safety improvements together produce more aggressive
// but perceptually-safer collapse on T2/T3/T4.
//   Keep ratios: same as v061
//   Caps: 25 uniform (same as v061)
//   costCap: HParam_QemCostCapCoeff * diag^2 * 0.85 on T2/T3/T4
#include <bits/stdc++.h>
...
private:
    ...
    void initScale()
    {
        ...
        costCap = HParam_QemCostCapCoeff * diag * diag * (screenTier != 0 ? 0.85 : 1.0);
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double w;
                if (screenTier == 2)
                    w = min(25.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(25.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(25.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
