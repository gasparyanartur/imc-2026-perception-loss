// Pineapple v070: MEMLESS strategy change. Disable MEMLESS for screen-tier 2 (T2).
// Current: MEMLESS = (nV > 5000) for everything 5k-50k (T2/T3/T4).
// New: MEMLESS = (nV > 5000 && screenTier != 2). T2 uses additive quadric
// accumulation (no per-collapse rebuild). Hypothesis: screen-core T2 is the
// highest-value tier for cases 3 (5k-25k meshes), and the local evaluator
// showed that additive quadrics are more perceptually stable. This affects
// ALL tiers via the screenTier variable - but only T2 (nV <= 25000) sees a
// behavior change since screenTier 2 is the only one that uses additive.
#include <bits/stdc++.h>
...
private:
    ...
    void initScale()
    {
        ...
        // Per-tier algorithm dispatch: memoryless QEM for medium tiers EXCEPT T2.

        // T2 (nV 5k-25k, screenTier==2) uses additive quadric accumulation
        // (MEMLESS=false) for better perceptual stability on case 3.

        // T3/T4 still use MEMLESS=true (per-collapse rebuild).

        MEMLESS = (nV > 5000 && screenTier != 2);
