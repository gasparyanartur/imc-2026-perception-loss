// Pineapple v033: Bias direction with cost cap + T4 cap boost.
// Combines: (a) T2 4-stage (0.38, 0.33, 0.30, 0.28),
// (b) T4 4-stage (0.15, 0.11, 0.085, 0.072),
// (c) T4 face weight cap 16->24,
// (d) QEM cost cap coefficient 0.0375->0.045 (allow expensive collapses),
// (e) T6 keepRatio 0.032->0.028.
// Hypothesis: cost cap relaxation allows more aggressive collapse selection;
// T4 cap boost biases toward unimportant faces.
#include <bits/stdc++.h>

static constexpr double HParam_Pineapple_T2Stages[4] = {0.38, 0.33, 0.30, 0.28};
static constexpr double HParam_Pineapple_T4Stages[4] = {0.15, 0.11, 0.085, 0.072};
static constexpr double HParam_Pineapple_FaceWeightCapT4 = 24.0;
static constexpr double HParam_Pineapple_QemCostCapCoeff = 0.045;
...
static constexpr double HParam_QemCostCapCoeff = HParam_Pineapple_QemCostCapCoeff;
...
private:
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double w = screenTier == 2 ? min(18.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(HParam_Pineapple_FaceWeightCapT4, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
