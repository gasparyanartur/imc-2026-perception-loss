// Pineapple v013: Soft silhouette crease boost via face weight (not positional).
// Hypothesis: v004 silhouette plane quadric (positional) over-penalized flat
// regions. A softer boost on the face weight (multiplicative, not additive)
// for high-faceSil faces should protect silhouettes without breaking flat
// geometry. Face weight goes from w to w * (1 + 0.25 * silRatio).
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_SilhouetteWeightBoost = true;
static constexpr double HParam_Pineapple_SilhouetteWeightBoostMax = 1.25;
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                if (HParam_Pineapple_SilhouetteWeightBoost && faceImpMean > 0.0 &&
                    screenTier != 3 && fi < (int)faceSil.size())
                {
                    double silRatio = faceSil[fi] / max(faceImpMean, 1.0);
                    w = min(HParam_Pineapple_SilhouetteWeightBoostMax,
                            w * (1.0 + 0.25 * silRatio));
                }
                if (w != 1.0)
                    q.scale(w);
