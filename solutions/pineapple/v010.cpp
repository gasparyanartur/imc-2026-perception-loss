// Pineapple v010: Composite - Vega-aware cap (v003) + MEMLESS silhouette
// (v006). Hypothesis: cheap cap modulation prioritizes perceptually safer
// collapses for the queue, while MEMLESS silhouette carry-over preserves
// crease energy across rebuilds. Together they should improve over either
// alone.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_VegaAwareCap = true;
static constexpr double HParam_Pineapple_VegaAwareCapRiskPow = 1.4;
static constexpr double HParam_Pineapple_VegaAwareCapRiskMax = 1.6;

static constexpr bool HParam_Pineapple_MemlessSilhouette = true;
static constexpr double HParam_Pineapple_MemlessSilhouetteWeight = 0.30;
...
private:
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            if (HParam_Pineapple_MemlessSilhouette && screenTier != 3 &&
                fi < (int)faceSil.size() && faceSil[fi] > 0.0)
            {
                Vec3 nn = n / max(area * 2.0, CParam_MinNormalNorm);
                double sw = HParam_Pineapple_MemlessSilhouetteWeight * faceSil[fi];
                for (int k = 0; k < 3; ++k)
                {
                    int a = f.v[k], b = f.v[(k + 1) % 3];
                    Vec3 e = verts[b] - verts[a];
                    double el = norm(e);
                    if (el < 1e-12)
                        continue;
                    Vec3 ed = e / el;
                    Vec3 n2 = nn;
                    double proj = dot(ed, n2);
                    n2 = n2 - ed * proj;
                    double nl = norm(n2);
                    if (nl < 1e-12)
                        continue;
                    n2 = n2 / nl;
                    double pd = -dot(n2, verts[a]);
                    Quadric qs;
                    qs.a = n2.x * n2.x;
                    qs.b = n2.x * n2.y;
                    qs.c = n2.x * n2.z;
    ...
    void collapseLoop()
    {
        ...
        while (accepted < collapseLimit && (!pq.empty() || tailMode()))
        {
            ...
            if (HParam_Pineapple_VegaAwareCap && screenTier > 0)
            {
                double risk = pineappleEdgeRisk(a, b);
                if (risk > 0.0)
                {
                    double tightCap = costCap / max(1.0, pow(risk, HParam_Pineapple_VegaAwareCapRiskPow));
                    if (c.cost > tightCap)
                        continue;
                }
            }
    ...
    // ---------- Pineapple: Vega-aware QEM cost cap modulation ----------

    double pineappleEdgeRisk(int a, int b) const
    {
        if (facePix.empty() || faceSil.empty())
            return 0.0;
        double maxPix = 0.0, maxSil = 0.0;
        for (int fi : vfaces[a])
        {
            if (fi < (int)fdead.size() && fdead[fi])
                continue;
            if (fi >= (int)facePix.size())
                continue;
            if (facePix[fi] > maxPix) maxPix = facePix[fi];
            if (faceSil[fi] > maxSil) maxSil = faceSil[fi];
        }
        for (int fi : vfaces[b])
        {
            if (fi < (int)fdead.size() && fdead[fi])
        ...
        if (maxPix <= 0.0 && maxSil <= 0.0)
            return 0.0;
        double mean = max(faceImpMean, 1.0);
        double pixRatio = maxPix / mean;
        double silRatio = maxSil / mean;
        double risk = sqrt(pixRatio * pixRatio + 4.0 * silRatio * silRatio);
        if (risk < 0.0) risk = 0.0;
        if (risk > HParam_Pineapple_VegaAwareCapRiskMax) risk = HParam_Pineapple_VegaAwareCapRiskMax;
        return risk;
    }
