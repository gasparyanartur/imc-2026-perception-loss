// Pineapple v029: Anchor carry + T2 final target 0.30 -> 0.28 (-2pt).
// Hypothesis: with anchor carry (v025), the screen core can tolerate a
// tighter T2 final keep ratio. Tangerine showed -1pt on T2 alone fails
// case 3, but the MEMLESS anchor carry adds protection.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_AnchorMemlessCarry = true;
static constexpr double HParam_Pineapple_AnchorBoostT2 = 1.4;
static constexpr double HParam_Pineapple_AnchorBoostT4 = 1.2;
static constexpr double HParam_Pineapple_T2FinalRatio = 0.28;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.36, 0.33, HParam_Pineapple_T2FinalRatio} : vector<double>{0.14, 0.10, 0.08};
        for (double kr : st)
        {
    ...
    void applyCollapse(int ab, int kp, const Vec3 &np, double nr)
    {
        ...
        if (MEMLESS)
        {
            ...
            // Pineapple: carry anchor quadric into MEMLESS rebuild to preserve
            // visible-vertex protection across collapses.
            if (HParam_Pineapple_AnchorMemlessCarry && screenTier > 0 &&
                kp < (int)vertAnchor.size() && vertAnchor[kp] > 0 && faceImpMean > 0.0)
            {
                double am = faceImpMean;
                double bw = (screenTier == 2 ? 0.0000035 : 0.0000050) * diag * diag / (am + 1.0);
                fresh += anchorPointQuadric(verts[kp], bw * min(18.0, vertAnchor[kp] / (am + 1e-12)));
            }
