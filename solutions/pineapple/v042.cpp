// Pineapple v042: Importance penalty in priority queue.
// Hypothesis: Nebula uses facePix/faceSil to weight quadric energy but
// doesn't apply a similar weight to the priority queue cost. By multiplying
// the queue cost by (1 + 0.3 * faceSilRatio), high-importance collapses are
// deprioritized in the queue. The lowest-cost edge processed first becomes
// one in an unimportant region. This is a softer lever than face weight
// changes (which were inert) because it changes which edge is dequeued.
// Only applied for screen-tier paths.
#include <bits/stdc++.h>

static constexpr bool HParam_Pineapple_ImportancePenalty = true;
static constexpr double HParam_Pineapple_ImportancePenaltyWeight = 0.30;
static constexpr double HParam_Pineapple_ImportancePenaltyMax = 2.0;
...
private:
    ...
    CollapseCandidate computeQueueCandidate(int a, int b) const
    {
        ...
        double penalty = 1.0;
        if (HParam_Pineapple_ImportancePenalty && screenTier > 0 &&
            !faceSil.empty() && faceImpMean > 0.0)
        {
            double maxSil = 0.0;
            for (int fi : vfaces[a])
            {
                if (fi < (int)faceSil.size() && !fdead[fi] && faceSil[fi] > maxSil)
                    maxSil = faceSil[fi];
            }
            for (int fi : vfaces[b])
            {
            ...
            double silRatio = maxSil / faceImpMean;
            penalty = 1.0 + HParam_Pineapple_ImportancePenaltyWeight * silRatio;
            if (penalty > HParam_Pineapple_ImportancePenaltyMax)
                penalty = HParam_Pineapple_ImportancePenaltyMax;
            if (penalty < 1.0)
                penalty = 1.0;
        }
