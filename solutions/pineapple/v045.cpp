// Pineapple v045: Inverse penalty (v042 flipped) - penalty for LOW importance.
// Hypothesis: Inverse direction - multiply cost by (1 - 0.3 * silRatio).
// Low-importance edges become cheaper and get processed first.
// Wait - that's already what v042 does. The penalty should be on HIGH
// importance. v045 makes it so high-importance edges cost LESS (cheaper
// first), so they collapse first. This may fail SSIM but tests the
