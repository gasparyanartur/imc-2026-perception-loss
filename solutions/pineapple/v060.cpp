// Pineapple v060: keep ratios UNCHANGED but raise all face weight
// caps by 5.0 AND boost faceImpMean weight (0.12 -> 0.20) in r calc
// for stronger visible-region protection across all tiers.
// Hypothesis: stronger visible-region protection lets us keep the
// baseline keep ratios (no perceptual cliff) while still reducing
// safer deletes via sharper ordering.
//   Caps: T2 18->23, T3 7->12, T4 16->21
//   faceImpMean weight: T2/T4 0.12 -> 0.20, T3 already 0.20 unchanged
//   keepRatios: unchanged from baseline
#include <bits/stdc++.h>
...
private:
    ...
    void initFaceWeights()
    {
        ...
        for (int fi = 0; fi < nF; ++fi)
        {
            ...
            if (screenTier)
            {
                ...
                double r = screenTier == 3 ? (imp + 0.20 * faceImpMean) / (faceImpMean + 1e-12) : (imp + 0.20 * faceImpMean) / (faceImpMean + 1e-12);
                double w;
                if (screenTier == 2)
                    w = min(23.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90)));
                else if (screenTier == 3)
                    w = min(12.0, max(0.12, 0.15 + 0.85 * sqrt(max(0.0, r))));
                else
                    w = min(21.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
                if (w != 1.0)
                    q.scale(w);
            }
            else if (area >= 1e-30)
    ...
    Quadric weightedFaceQuadric(int fi) const
    {
        ...
        if (screenTier && fi < (int)facePix.size())
        {
            ...
            double r = (imp + 0.20 * faceImpMean) / (faceImpMean + 1e-12);
            double w = screenTier == 2 ? min(23.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.90))) : min(21.0, max(0.06, 0.08 + 0.92 * pow(max(0.0, r), 0.72)));
            if (w != 1.0)
                q.scale(w);
        }
        else if (area >= 1e-30)
