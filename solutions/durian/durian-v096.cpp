// Durian v096: v094 + runLargeCameraTx uses the original input mesh as SSIM reference.
// Save origVerts/origFaces for inputV>50000 before the QEM loop; render reference images
// from the original mesh instead of the already-simplified safe mesh. Adjust guards for
// absolute original-vs-final SSIM so more aggressive end-game ratios can be tried safely.
#include <bits/stdc++.h>
...
double stop=elapsed()+min(2.5,left*0.48);
vector<EdgeVisualCandidate> cs;cs.reserve(1536);int scanned=0;
for(int a=0;a<(int)verts.size()&&scanned<5000&&elapsed()<stop;++a)if(!vdead[a]){
for(int b:vneigh[a]){
if(b<=a||vdead[b]||scanned>=5000||elapsed()>=stop)continue;
++scanned;
...
double s=localVegaSsimForEdge(c);
if(s>=0.99900)cs.push_back({a,b,1.0-s,c.cost});
}
}
sort(cs.begin(),cs.end());
vector<char> lock(verts.size(),0);int cap=max(16,min(500,(int)floor(inputV*0.007))),done=0;
for(const auto& e:cs){
...
if(!c.valid()||c.cost>costCap||localVegaSsimForEdge(c)<0.99900)continue;
if(independent){
