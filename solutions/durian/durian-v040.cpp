// Durian v040: v002 + T6 runLargeCameraTx tries: larger range that starts at 0.031.
// If a tighter then-existing ratio passes the guard, this still hits 0.031 first.
// Then we test 0.028 if 0.031 fails. This biases the algorithm toward the
// looser option (case 7 safe) unless a tighter one passes.
#include <bits/stdc++.h>
...
vector<double>tries={.031,.030,.028,.026};
bool qemKept=false;
for(double ratio:tries){
