// Durian v106: v105 + tighten runScreenCoreMid targets (T2 finalTarget 0.28, T3 finalTarget 0.125).
// Test whether screen-core can start from a slightly leaner mesh for the endgame.
#include <bits/stdc++.h>
...
if(screenTier==3){int finalTarget=(int)floor(nV*0.125),safeTarget=(int)floor(nV*0.14);buildRasterImportanceTier3(384);initFaceWeights();targetV=safeTarget;collapseLimit=nV-targetV;initQueue();collapseLoop();if(accepted<nV-finalTarget&&elapsed()<HParam_TimeBudgetSeconds-0.4){buildRasterImportanceTier3(384);initFaceWeights();priority_queue<CollapseCandidate>e;pq.swap(e);targetV=finalTarget;collapseLimit=nV-targetV;initQueue();collapseLoop();}return;}
vector<double>st=screenTier==2?vector<double>{0.33,0.30,0.28}:vector<double>{0.14,0.10,0.08};
...
void absoluteQemEndgame(){
    ...
    double hi=double(safeV.size())/double(inputV),lo=inputV<=25000?0.14:0.07;bool improved=false;
