// Durian v006: v055 baseline + T5/T6/T7 weld AND pair-disk enable at T4 baseline scan (760/90).
// Hypothesis: Adding post-passes with T4-baseline scan sizes (no timeout risk)
// should add compression without cropping Kattis score.
#include <bits/stdc++.h>
...
private:
    ...
    StarParams weldParams() const
    {
        ...
        if (t == 5)
            return {6, 0.018, 0.030, 0.70, 0.0130, 760, 280000, 1, 0.55, 1.50};
        if (t == 6)
            return {6, 0.022, 0.036, 0.80, 0.0160, 760, 280000, 1, 0.60, 1.80};
    ...
    StarParams pairDiskParams() const
    {
        ...
        if (t == 5)
            return {8, 0.020, 0.032, 0.70, 0.0030, 90, 120000, 1, 0.30, 0.95};
        if (t == 6)
            return {8, 0.024, 0.038, 0.85, 0.0040, 90, 120000, 1, 0.32, 1.05};
