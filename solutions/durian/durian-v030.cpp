// Durian v030: v002 + lower txGuard thresholds (the T2/T3 SSIM barriers).
// txGuard T2: final 0.9965->0.9940, minView 0.9955->0.9930, etc.
#include <bits/stdc++.h>
...
bool txGuard(const RefScore&s,int tier)const{if(tier==2)return s.finalScore>=.9940&&s.minView>=.9930&&s.minNormal>=.9900&&s.minDepth>=.9970;if(tier==3)return s.finalScore>=.9925&&s.minView>=.9915&&s.minNormal>=.9850&&s.minDepth>=.9970;return s.finalScore>=.9925&&s.minView>=.9915&&s.minNormal>=.9850&&s.minDepth>=.9970;}
