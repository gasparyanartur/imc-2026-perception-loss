// Durian v093: v090 + fast SSIM-aware post-pass for T7 before compact.
// T7 path currently skips all post-passes. Add a scaled vegaSsimEdgePass if time permits.
// Goal: recover SSIM at aggressive keepRatios (e.g., T6=0.020) or enable lower.
#include <bits/stdc++.h>
...
if(inputV>400000){if(elapsed()<HParam_TimeBudgetSeconds-0.85)vegaSsimEdgePass(true);compact();writeMesh();return;}
...
double stop=elapsed()+min(1.7,left*0.38);
vector<EdgeVisualCandidate> cs;cs.reserve(512);int scanned=0;
for(int a=0;a<(int)verts.size()&&scanned<2400&&elapsed()<stop;++a)if(!vdead[a]){
for(int b:vneigh[a]){
if(b<=a||vdead[b]||scanned>=2400||elapsed()>=stop)continue;
++scanned;
...
if(s>=0.99992)cs.push_back({a,b,1.0-s,c.cost});
}
}
sort(cs.begin(),cs.end());
vector<char> lock(verts.size(),0);int cap=max(8,min(180,(int)floor(inputV*0.0025))),done=0;
for(const auto& e:cs){
...
if(!c.valid()||c.cost>costCap||localVegaSsimForEdge(c)<0.99992)continue;
if(independent){
