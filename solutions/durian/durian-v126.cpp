// Durian v126: v119 + vegaSsimEdgePass scan 4800 and cap 400.
// Larger search window on top of the improved medium-tier endgame.
#include <bits/stdc++.h>
...
for(int a=0;a<(int)verts.size()&&scanned<4800&&elapsed()<stop;++a)if(!vdead[a]){
for(int b:vneigh[a]){
if(b<=a||vdead[b]||scanned>=4800||elapsed()>=stop)continue;
++scanned;
...
vector<char> lock(verts.size(),0);int cap=max(16,min(400,(int)floor(inputV*0.0056))),done=0;
for(const auto& e:cs){
...
void absoluteQemEndgame(){
    ...
    double hi=double(safeV.size())/double(inputV),lo=inputV<=25000?0.125:0.060;bool improved=false;
    for(int it=0;it<7&&elapsed()<HParam_TimeBudgetSeconds-1.25;++it){
