// Tangerine v005: above-95 nominal staged screen-space target profile.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_UpTo5k = 0.06;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.06), safeTarget = (int)floor(nV * 0.10);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.18, 0.14, 0.10} : vector<double>{0.060, 0.045, 0.035};
        for (double kr : st)
        {
