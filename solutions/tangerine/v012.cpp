// Tangerine v012: all-tier targets plus the formerly dead screen-tier Vega finisher.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_UpTo400k = 0.0235;

static constexpr double HParam_KeepRatio_Huge = 0.0305;
...
static constexpr int HParam_MinTargetVertices = 7;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.138), safeTarget = (int)floor(nV * 0.156);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.345, 0.315, 0.286} : vector<double>{0.125, 0.090, 0.073};
        for (double kr : st)
        {
    ...
    void runMidPostPasses()
    {
        if (HParam_TangerineProfile == 1)
        {
            collapseInvisibleEdges();
            return;
        }
        if (HParam_TangerineProfile == 2)
        {
            vegaSsimStarPass();
            vegaSsimStarPass();
            return;
        }
        if (HParam_TangerineProfile == 3)
        {
            collapseInvisibleEdges();
            vegaSsimStarPass();
            vegaSsimStarPass();
    ...
    void initScale()
    {
        ...
        targetV = max(HParam_MinTargetVertices, (int)floor(nV * kr));
    ...
    void getCandidatePositions(int a, int b, const Quadric &q, Vec3 pos[8], int &np) const
    {
        ...
        if (HParam_TangerineProfile >= 5)
        {
            ...
            if (length > CParam_MinNormalNorm)
            {
                double t = (crad[b] + length - crad[a]) / (2.0 * length);
                t = max(0.0, min(1.0, t));
                pos[np++] = verts[a] + edge * t;
                pos[np++] = verts[a] + edge * 0.25;
                pos[np++] = verts[a] + edge * 0.75;
            }
        }
    ...
    bool passesLocalNormalGuard(int ab, int kp, const Vec3 &p) const
    {
        if (HParam_TangerineProfile < 4)
            return true;
        static constexpr double moderate[6] = {0.75, 0.90, 0.94, 0.95, 0.88, 0.82};
        static constexpr double strict[6] = {0.88, 0.95, 0.975, 0.98, 0.94, 0.90};
        double minDot = HParam_TangerineProfile >= 5
                            ? strict[tangerineTierBin()]
                            : moderate[tangerineTierBin()];
        vector<int> inc = vfaces[ab];
        inc.insert(inc.end(), vfaces[kp].begin(), vfaces[kp].end());
        ...
        for (int fi : inc)
        {
            ...
            if (oldLen < CParam_MinNormalNorm || newLen < oldLen * 0.02)
                return false;
    ...
    CollapseCandidate computeBestValid(int a, int b) const
    {
        ...
        for (int i = 0; i < np; ++i)
        {
            ...
            for (int dir = 0; dir < 2; ++dir)
            {
                ...
                if (!passesLocalNormalGuard(ab, kp, pos[i]))
                    continue;
    ...
    StarParams starParams() const
    {
        ...
        double boost = 1.0 + 0.08 * (HParam_TangerineProfile - 1);
        p1.extraFrac *= boost;
        p2.extraFrac *= boost;
        p3.extraFrac *= boost;
        p4.extraFrac *= boost;
    ...
    StarParams vegaSsimParams() const
    {
        ...
        StarParams p4 = {7, 0.018, 0.030, 0.72, 0.0060, 500, 100000, 1, 0.34, 1.30};

        double boost = 1.0 + 0.06 * (HParam_TangerineProfile - 1);
        p2.extraFrac *= boost;
        p3.extraFrac *= boost;
        p4.extraFrac *= boost;
        ...
        if (tier == 4)
            return HParam_TangerineProfile >= 3 ? p4 : p0;
    ...
    double vegaSsimMin() const
    {
        ...
        if (tier == 4)
            return HParam_TangerineProfile >= 3 ? 0.970 : 1.01;
    ...
    double vegaMaxDamage() const
    {
        ...
        if (tier == 4)
            return HParam_TangerineProfile >= 3 ? 0.030 : 0.0;
