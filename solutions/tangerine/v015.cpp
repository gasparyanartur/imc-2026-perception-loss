// Tangerine v015: strict normal guard, radius-balanced placements, and all finishers.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_UpTo400k = 0.0225;

static constexpr double HParam_KeepRatio_Huge = 0.0295;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.136), safeTarget = (int)floor(nV * 0.154);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.335, 0.305, 0.282} : vector<double>{0.115, 0.082, 0.070};
        for (double kr : st)
        {
