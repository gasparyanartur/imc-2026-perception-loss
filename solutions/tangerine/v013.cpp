// Tangerine v013: combined all-tier star and Vega finishing with tier-4 Vega.
#include <bits/stdc++.h>
...
static constexpr double HParam_KeepRatio_UpTo400k = 0.0238;

static constexpr double HParam_KeepRatio_Huge = 0.0308;
...
static constexpr int HParam_MinTargetVertices = 8;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.139), safeTarget = (int)floor(nV * 0.157);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.350, 0.320, 0.288} : vector<double>{0.130, 0.092, 0.074};
        for (double kr : st)
        {
