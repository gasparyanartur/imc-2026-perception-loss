// Tangerine v003: strong staged screen-space target reduction.
#include <bits/stdc++.h>
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.10), safeTarget = (int)floor(nV * 0.14);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.26, 0.22, 0.18} : vector<double>{0.09, 0.07, 0.055};
        for (double kr : st)
        {
