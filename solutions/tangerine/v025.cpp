// Tangerine v025: persistent axial-silhouette/crease quadrics, profile 5/5.
#include <bits/stdc++.h>
...
private:
    ...
    vector<Quadric> vquad, vmoment, vfeature;
    ...
    double tangerineHausFraction() const
    {
        static constexpr double x[5][6] = {
            {0.0440, 0.0465, 0.0465, 0.0465, 0.0490, 0.0490},
            {0.0450, 0.0470, 0.0470, 0.0470, 0.0491, 0.0491},
            {0.0460, 0.0475, 0.0475, 0.0475, 0.0492, 0.0492},
            {0.0470, 0.0480, 0.0480, 0.0480, 0.0493, 0.0493},
            {0.0480, 0.0485, 0.0485, 0.0485, 0.0494, 0.0494}};
    ...
    {
        static constexpr double x[5][6] = {
            {0.0, 0.322, 0.162, 0.102, 0.02530, 0.03240},
            {0.0, 0.320, 0.160, 0.100, 0.02522, 0.03232},
            {0.0, 0.319, 0.159, 0.099, 0.02515, 0.03224},
            {0.0, 0.318, 0.158, 0.098, 0.02508, 0.03216},
            {0.0, 0.317, 0.157, 0.097, 0.02502, 0.03208}};
        return x[tangerineProfileIndex()][tangerineTierBin()];
    }

    int tangerineMinTarget() const
    {
        static constexpr int x[5] = {18, 16, 14, 12, 11};
        return x[tangerineProfileIndex()];
    }

    double tangerineFeatureStrength() const
    {
        static constexpr double x[5][6] = {
            {0.00, 0.02, 0.03, 0.04, 0.00, 0.00},
            {0.00, 0.04, 0.06, 0.08, 0.00, 0.00},
            {0.00, 0.08, 0.12, 0.16, 0.00, 0.00},
            {0.00, 0.16, 0.24, 0.32, 0.00, 0.00},
            {0.00, 0.32, 0.48, 0.64, 0.00, 0.00}};
        return x[tangerineProfileIndex()][tangerineTierBin()];
    }
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            static constexpr double safeRatio[5] = {0.1620, 0.1600, 0.1590, 0.1580, 0.1570};
            static constexpr double finalRatio[5] = {0.1450, 0.1449, 0.14475, 0.1445, 0.1440};
        ...
        static constexpr double tier2Stages[5][3] = {
            {0.362, 0.332, 0.3000}, {0.360, 0.330, 0.29975},
            {0.358, 0.328, 0.2995}, {0.356, 0.326, 0.2990},
            {0.354, 0.324, 0.2980}};
        static constexpr double tier4Stages[5][3] = {
            {0.142, 0.102, 0.0800}, {0.140, 0.100, 0.07995},
            {0.139, 0.099, 0.0799}, {0.138, 0.098, 0.07975},
            {0.136, 0.096, 0.0795}};
        int pi = tangerineProfileIndex();
        vector<double> st(3);
        for (int i = 0; i < 3; ++i)
    ...
    void buildConnectivity()
    {
        ...
        vfeature.assign(nV, Quadric());
        double featureStrength = tangerineFeatureStrength();
        if (featureStrength > 0.0)
        {
            vector<Vec3> faceNormal(nF);
            vector<double> faceScale(nF, 0.0);
            for (int fi = 0; fi < nF; ++fi)
            {
                ...
                if (len < CParam_MinNormalNorm)
                    continue;
                faceNormal[fi] = raw / len;
                faceScale[fi] = sqrt(0.5 * len);
            }
            if (HParam_TangerineProfile >= 4)
            {
                for (int fi = 0; fi < nF; ++fi)
                {
                    const Face &f = faces[fi];
                    Vec3 n = faceNormal[fi];
                    double viewSupport = 0.25 + 0.75 * (fabs(n.x) + fabs(n.y) + fabs(n.z)) / sqrt(3.0);
                    Quadric q = Quadric::fromTriangle(verts[f.v[0]], verts[f.v[1]], verts[f.v[2]]);
                    q.scale(featureStrength * viewSupport);
                    for (int k = 0; k < 3; ++k)
                        vfeature[f.v[k]] += q;
                }
            }
            else for (int a = 0; a < nV; ++a)
                for (int b : vneigh[a])
                {
                    if (a >= b)
                        continue;
                    int inc[2] = {-1, -1}, count = 0;
                    for (int fi : vfaces[a])
                    {
                        const Face &f = faces[fi];
                    ...
                    double feature = count == 1 ? 1.0 : 0.0;
                    if (count >= 2)
                    {
                        double crease = max(0.0, min(1.0, (0.94 - dot(faceNormal[inc[0]], faceNormal[inc[1]])) / 0.44));
                        int silhouetteAxes = 0;
                        const double n0[3] = {faceNormal[inc[0]].x, faceNormal[inc[0]].y, faceNormal[inc[0]].z};
                        const double n1[3] = {faceNormal[inc[1]].x, faceNormal[inc[1]].y, faceNormal[inc[1]].z};
                        for (int axis = 0; axis < 3; ++axis)
                            if (n0[axis] * n1[axis] <= 0.0)
                                ++silhouetteAxes;
                        feature = max(crease, min(1.0, 0.5 * silhouetteAxes));
                    }
                    if (feature < 0.02 || count <= 0)
                        continue;
                    Vec3 edge = verts[b] - verts[a];
                    double edgeLen = norm(edge);
                    if (edgeLen < CParam_MinNormalNorm)
                        continue;
                    Vec3 edgeDir = edge / edgeLen;
                    int usable = min(count, 2);
                    double localScale = 0.0;
                    for (int t = 0; t < usable; ++t)
                        localScale += faceScale[inc[t]];
                    localScale /= max(1, usable);
                    for (int t = 0; t < usable; ++t)
                    {
                        Vec3 pn = cross(edgeDir, faceNormal[inc[t]]);
                        double pnLen = norm(pn);
                        if (pnLen < CParam_MinNormalNorm)
                            continue;
                        Quadric q = planePointQuadric(pn / pnLen, verts[a], featureStrength * feature * localScale / usable);
                        vfeature[a] += q;
                        vfeature[b] += q;
                    }
                    Quadric pinA = anchorPointQuadric(verts[a], 0.0625 * featureStrength * feature * localScale);
                    Quadric pinB = anchorPointQuadric(verts[b], 0.0625 * featureStrength * feature * localScale);
                    vfeature[a] += pinA;
                    vfeature[b] += pinB;
                }
        }
    ...
    Quadric planePointQuadric(const Vec3 &n, const Vec3 &p, double w) const
    {
        double d = -dot(n, p);
        Quadric q;
        q.a = n.x * n.x;
    ...
    void initFaceWeights()
    {
        ...
        if (screenTier)
            for (int i = 0; i < nV; ++i)
                if (!vdead[i] && i < (int)vfeature.size())
                    vquad[i] += vfeature[i];
    }
    ...
    void applyCollapse(int ab, int kp, const Vec3 &np, double nr)
    {
        ...
        if (kp < (int)vfeature.size() && ab < (int)vfeature.size())
            vfeature[kp] += vfeature[ab];
        ...
        if (MEMLESS)
        {
            ...
            if (screenTier == 4 && kp < (int)vmoment.size())
                fresh += vmoment[kp];
            if (screenTier && kp < (int)vfeature.size())
                fresh += vfeature[kp];
            vquad[kp] = fresh;
        }
        else
    ...
    StarParams starParams() const
    {
        ...
        if (tier == 1)
        {
            static constexpr double extraScale[5] = {0.50, 0.60, 0.70, 0.80, 0.90};
            static constexpr double devScale[5] = {0.55, 0.65, 0.75, 0.85, 0.92};
            static constexpr double distScale[5] = {0.70, 0.75, 0.80, 0.86, 0.92};
            static constexpr int rounds[5] = {1, 1, 2, 2, 3};
            int pi = tangerineProfileIndex();
            p1.maxOldDev *= devScale[pi];
            p1.maxNewDev *= devScale[pi];
            p1.distFrac *= distScale[pi];
            p1.extraFrac *= extraScale[pi];
            p1.hardCap = max(1, (int)floor(p1.hardCap * extraScale[pi]));
            p1.rounds = rounds[pi];
            return p1;
        }
