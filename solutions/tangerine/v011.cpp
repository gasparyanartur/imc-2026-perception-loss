// Tangerine v011: all-tier targets plus the formerly dead screen-tier star finisher.
#include <bits/stdc++.h>
...
static constexpr int HParam_MinTargetVertices = 9;
...
private:
    ...
    void runScreenCoreMid()
    {
        ...
        if (screenTier == 3)
        {
            int finalTarget = (int)floor(nV * 0.141), safeTarget = (int)floor(nV * 0.158);
        ...
        vector<double> st = screenTier == 2 ? vector<double>{0.355, 0.325, 0.292} : vector<double>{0.135, 0.095, 0.076};
        for (double kr : st)
        {
