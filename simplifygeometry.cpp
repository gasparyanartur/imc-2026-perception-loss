...
private:
    ...
    StarCandidate computeStarCandidate(int v)const{
        ...
        StarCandidate best;
        vector<int> ring,inc;
        if(!orientedRingForVertex(v,ring,inc))return best;
        ...
        for(int root=0;root<(int)ring.size();++root){
            StarCandidate c;
            if(evaluateStarRoot(v,ring,inc,root,oldDev,avg,c)&&c.score<best.score)best=c;
        }
        return best;
    }
    bool applyStarDelete(int v,int root){
        ...
        root=best.root;
        vector<int> ring,inc;
        if(!orientedRingForVertex(v,ring,inc))return false;
        int m=(int)ring.size();
        vector<int> rr; rr.reserve(m);
        for(int i=0;i<m;++i)rr.push_back(ring[(root+i)%m]);
        for(int fi:inc){
