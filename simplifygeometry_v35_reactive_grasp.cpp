...
/*
IMC2026 Problem B — simplifygeometry_v35_reactive_grasp.cpp

V25 tail batching with deterministic reactive GRASP-style lock/target policy.
...
private:
    ...
    double lastTailBatchAcceptanceRate = 0.75;
...
private:
    ...
    int reactiveConflictMode() const {
        double elapsed = elapsedSeconds();
        if (elapsed > 16.0 && lastTailBatchAcceptanceRate > 0.55) return 2; // endpoints only
        if (lastTailBatchAcceptanceRate < 0.25) return 0; // full one-ring
        return 1; // endpoints + common neighbors
    }

    int reactiveBatchTarget() const {
        double elapsed = elapsedSeconds();
        if (elapsed > 16.0 && lastTailBatchAcceptanceRate > 0.55) return 4096;
        if (lastTailBatchAcceptanceRate < 0.25) return 1024;
        return HParam_TailBatchTargetAccepts;
    }

    bool batchNeighborhoodFree(int a, int b) const {
        if (tailLocks[a] == tailLockStamp || tailLocks[b] == tailLockStamp) return false;
        int mode = reactiveConflictMode();
    ...
    int runTailBatch() {
        ...
        int batchTarget = reactiveBatchTarget();
        selected.reserve(batchTarget);
        for (const EdgeBatchCandidate& e : candidates) {
            if ((int)selected.size() >= batchTarget) break;
        ...
        if (!selected.empty()) {
            lastTailBatchAcceptanceRate = (double)acceptedInBatch / (double)selected.size();
        }
