...
private:
    ...
    bool lensOriginalScoresForStarCandidate(int v,int root,const StarDeleteParams& sp,double& currentSsim,double& originalNewSsim)const{
        currentSsim=1.0;
        originalNewSsim=1.0;
    ...
    void lensOriginalStarDeletePass(){
        ...
        for(int v=0;v<(int)verts.size()&&scanned<sp.scanVertices&&elapsed()<stopTime;++v){
            ...
            StarCandidate geom=computeStarCandidateWithParams(v,sp);
            if(!geom.valid())continue;
            double curS=1.0,origS=1.0;
            if(!lensOriginalScoresForStarCandidate(geom.v,geom.root,sp,curS,origS))continue;
            if(curS<lensCurrentMin||origS<lensOrigMin)continue;
            SsimDeleteCandidate dc;
            dc.v=geom.v;
            dc.root=geom.root;
            dc.ssim=origS;
            dc.score=(1.0-origS)+0.25*(1.0-curS)+hparam_LensScoreGeomWeight*geom.score;
            cands.push_back(dc);
        ...
        for(const SsimDeleteCandidate& c:cands){
            ...
            if(c.v<0||c.v>=(int)vdead.size()||vdead[c.v])continue;
            double curS=1.0,origS=1.0;
            if(!lensOriginalScoresForStarCandidate(c.v,c.root,sp,curS,origS))continue;
            if(curS<lensCurrentMin||origS<lensOrigMin)continue;
            if(applyStarDeleteExactWithParams(c.v,c.root,sp)){
                ++accepted;
                ++extra;
