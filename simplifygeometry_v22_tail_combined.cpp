...
/*
IMC2026 Problem B — simplifygeometry_v22_tail_combined.cpp

V20 tail radius/cap schedule plus huge-tier conflict-free tail batching.
...
static constexpr double CParam_TailQuadraticEps = 1e-14;
static constexpr double CParam_CandidateDuplicateDistSq = 1e-30;
...
static constexpr double HParam_TimeBudgetSeconds = 18.8;
...
static constexpr double HParam_TailElapsedStart = 11.5;
static constexpr int    HParam_TailAliveStart = 220000;
static constexpr double HParam_TailCostCapStage1 = 0.016;
static constexpr double HParam_TailCostCapStage2 = 0.018;
static constexpr double HParam_TailBatchElapsedStart = 12.0;
static constexpr double HParam_TailBatchStopElapsed = 18.8;
...
private:
    ...
    CollapseCandidate computeBestValidCandidate(int a, int b) const {
        if (tailMode()) return computeBestValidTailCandidate(a, b);
    ...
    bool appendUniquePosition(vector<Vec3>& positions, const Vec3& p) const {
        if (!finiteVec(p)) return false;
        for (const Vec3& qpos : positions) {
            if (norm2(p - qpos) < CParam_CandidateDuplicateDistSq) return false;
        }
        positions.push_back(p);
        return true;
    }

    double segmentQemValue(double A, double B, double C, double t) const {
        return (A * t + B) * t + C;
    }

    bool closestFeasibleSegmentT(double A, double B, double C, double cap, double targetT, double& outT) const {
        auto feasible = [&](double t) {
            return t >= -1e-12 && t <= 1.0 + 1e-12 && segmentQemValue(A, B, C, clamp01(t)) <= cap;
        };

        if (feasible(targetT)) {
            outT = clamp01(targetT);
            return true;
        }

        vector<double> candidates;
        candidates.reserve(6);
        if (feasible(0.0)) candidates.push_back(0.0);
        if (feasible(1.0)) candidates.push_back(1.0);

        if (fabs(A) < CParam_TailQuadraticEps) {
            if (fabs(B) >= CParam_TailQuadraticEps) {
                double root = (cap - C) / B;
                if (root >= -1e-12 && root <= 1.0 + 1e-12) candidates.push_back(clamp01(root));
            }
        } else {
            double D = B * B - 4.0 * A * (C - cap);
            if (D >= 0.0) {
                double s = sqrt(max(0.0, D));
                double r0 = (-B - s) / (2.0 * A);
                double r1 = (-B + s) / (2.0 * A);
                if (r0 >= -1e-12 && r0 <= 1.0 + 1e-12) candidates.push_back(clamp01(r0));
                if (r1 >= -1e-12 && r1 <= 1.0 + 1e-12) candidates.push_back(clamp01(r1));
                if (A > 0.0) {
                    double lo = max(0.0, min(r0, r1));
                    double hi = min(1.0, max(r0, r1));
                    if (lo <= hi) candidates.push_back(min(max(targetT, lo), hi));
                }
            }
        }
        ...
        bool found = false;
        double bestT = 0.0;
        double bestDistance = CParam_Inf;
        for (double t : candidates) {
            t = clamp01(t);
            if (!feasible(t)) continue;
            double d = fabs(t - targetT);
            if (d < bestDistance) {
                bestDistance = d;
                bestT = t;
                found = true;
    ...
    vector<Vec3> tailCandidatePositionsForEdge(int a, int b, const Quadric& q, double activeCap) const {
        vector<Vec3> positions;
        positions.reserve(7);
        ...
        const double edgeLenSq = norm2(edge);
        const Vec3 midpoint = (pa + pb) * 0.5;

        if (edgeLenSq <= CParam_CandidateDuplicateDistSq) {
            appendUniquePosition(positions, midpoint);
            return positions;
        }

        const double edgeLen = sqrt(edgeLenSq);
        const double tRadius = clamp01((edgeLen + clusterRadius[b] - clusterRadius[a]) / (2.0 * edgeLen));

        Vec3 qemPoint;
        bool hasQem = solveQem3x3(q, qemPoint);
        double tQem = 0.5;
        if (hasQem) tQem = clamp01(dot(qemPoint - pa, edge) / edgeLenSq);

        const double q0 = q.evaluate(pa);
        const double q1 = q.evaluate(pb);
        const double qm = q.evaluate(midpoint);
        const double A = 2.0 * (q1 + q0 - 2.0 * qm);
        const double B = q1 - q0 - A;
        const double C = q0;

        double tFeasible = 0.0;
        if (closestFeasibleSegmentT(A, B, C, activeCap, tRadius, tFeasible)) {
            appendUniquePosition(positions, pa + edge * tFeasible);
        }

        appendUniquePosition(positions, pa + edge * tRadius);
        appendUniquePosition(positions, pa + edge * tQem);
        appendUniquePosition(positions, midpoint);
        appendUniquePosition(positions, pa);
        appendUniquePosition(positions, pb);
        if (hasQem) appendUniquePosition(positions, qemPoint);

        return positions;
    }

    static bool tailCandidateBetter(const CollapseCandidate& c, const CollapseCandidate& best) {
        if (!c.valid()) return false;
        if (!best.valid()) return true;
        double radiusEps = 1e-12 * max(1.0, best.mergedRadius);
        if (c.mergedRadius < best.mergedRadius - radiusEps) return true;
        if (fabs(c.mergedRadius - best.mergedRadius) <= radiusEps && c.cost < best.cost) return true;
        return false;
    }

    CollapseCandidate computeBestValidTailCandidate(int a, int b) const {
        Quadric q = vertexQuadrics[a] + vertexQuadrics[b];
        const double activeCap = currentCostCap();
        vector<Vec3> positions = tailCandidatePositionsForEdge(a, b, q, activeCap);
        ...
        for (const Vec3& p : positions) {
            for (int dir = 0; dir < 2; ++dir) {
                ...
                CollapseCandidate c = makeCandidate(absorbed, kept, p, q);
                c.mergedRadius = mergedRadius;
                if (c.cost > activeCap) continue;
                if (tailCandidateBetter(c, best)) best = c;
            }
        }
        return best;
    }
    ...
    int aliveVertexCount() const {
        return (int)vertices.size() - acceptedCollapses;
    }

    double currentCostCap() const {
        if ((int)vertices.size() <= HParam_TailOriginalVertexThreshold) return costCap;
        double coeff = HParam_QemCostCapCoeff;
        double elapsed = elapsedSeconds();
        int alive = aliveVertexCount();
        if (elapsed > 15.0 && alive > 90000) coeff = HParam_TailCostCapStage2;
        else if (elapsed > 13.0 && alive > 120000) coeff = HParam_TailCostCapStage1;
        return coeff * diagonal * diagonal;
    }

    bool tailMode() const {
        if ((int)vertices.size() <= HParam_TailOriginalVertexThreshold) return false;
        return elapsedSeconds() > HParam_TailElapsedStart || aliveVertexCount() < HParam_TailAliveStart;
    }
    ...
    bool batchNeighborhoodFree(int a, int b) const {
        ...
        for (int nb : vertexNeighbors[a]) {
            if (!vertexDead[nb] && tailLocks[nb] == tailLockStamp) return false;
        }
        for (int nb : vertexNeighbors[b]) {
            if (!vertexDead[nb] && tailLocks[nb] == tailLockStamp) return false;
        }
        return true;
    }
    ...
    void lockBatchNeighborhood(int a, int b) {
        ...
        for (int nb : vertexNeighbors[a]) if (!vertexDead[nb]) tailLocks[nb] = tailLockStamp;
        for (int nb : vertexNeighbors[b]) if (!vertexDead[nb]) tailLocks[nb] = tailLockStamp;
    }
