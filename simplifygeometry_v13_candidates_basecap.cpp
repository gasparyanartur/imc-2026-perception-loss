...
/*
IMC2026 Problem B — simplifygeometry_v13_candidates_basecap.cpp

Strawberry-v8 base plus projected-QEM and min-radius placement candidates, with baseline cap/time.
...
static constexpr double CParam_CostTieRelativeEps = 1e-10;
...
static constexpr double HParam_QemCostCapCoeff = 0.010;
...
struct CollapseCandidate {
    ...
    bool operator<(const CollapseCandidate& o) const {
        double eps = CParam_CostTieRelativeEps * max(1.0, fabs(o.cost));
        if (fabs(cost - o.cost) <= eps) return mergedRadius > o.mergedRadius;
...
private:
    ...
    vector<Vec3> candidatePositionsForEdge(int a, int b, const Quadric& q) const {
        ...
        Vec3 qemPoint;
        if (solveQem3x3(q, qemPoint)) {
            positions.push_back(qemPoint);
            if (edgeLenSq > CParam_CandidateDuplicateDistSq) {
                double t = clamp01(dot(qemPoint - pa, edge) / edgeLenSq);
                positions.push_back(pa + edge * t);
            } else {
        ...
        if (edgeLenSq > CParam_CandidateDuplicateDistSq) {
            double edgeLen = sqrt(edgeLenSq);
            double t = clamp01((edgeLen + clusterRadius[b] - clusterRadius[a]) / (2.0 * edgeLen));
            positions.push_back(pa + edge * t);
        } else {
            positions.push_back(midpoint);
        }
        ...
        for (const Vec3& p : positions) {
            ...
            for (const Vec3& qpos : uniquePositions) {
                if (norm2(p - qpos) < CParam_CandidateDuplicateDistSq) { duplicate = true; break; }
            }
    ...
    double estimateMergedRadius(int absorbed, int kept, const Vec3& p) const {
        return max(clusterRadius[absorbed] + norm(vertices[absorbed] - p),
                   clusterRadius[kept] + norm(vertices[kept] - p));
    }

    static bool candidateBetter(const CollapseCandidate& c, const CollapseCandidate& best) {
        if (!c.valid()) return false;
        if (!best.valid()) return true;
        double eps = CParam_CostTieRelativeEps * max(1.0, fabs(best.cost));
        if (c.cost < best.cost - eps) return true;
        if (fabs(c.cost - best.cost) <= eps && c.mergedRadius < best.mergedRadius) return true;
        return false;
    }
    ...
    CollapseCandidate computeQueueCandidate(int a, int b) const {
        ...
        for (const Vec3& p : positions) {
            ...
            c1.mergedRadius = estimateMergedRadius(a, b, p);
            if (candidateBetter(c1, best)) best = c1;
            CollapseCandidate c2 = makeCandidate(b, a, p, q);
            c2.mergedRadius = estimateMergedRadius(b, a, p);
            if (candidateBetter(c2, best)) best = c2;
        }
        return best;
    }
    ...
    CollapseCandidate computeBestValidCandidate(int a, int b) const {
        ...
        for (const Vec3& p : positions) {
            for (int dir = 0; dir < 2; ++dir) {
                ...
                if (candidateBetter(c, best)) best = c;
            }
        }
        return best;
    }
