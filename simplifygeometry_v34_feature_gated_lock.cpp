...
/*
IMC2026 Problem B — simplifygeometry_v34_feature_gated_lock.cpp

V25 tail batching with feature-gated common-neighbor conflict locks.
...
static constexpr double HParam_FeatureLooseLockRisk = 0.75;
static constexpr double HParam_FeatureLooseLockRadiusFrac = 0.65;
...
private:
    ...
    void buildInitialConnectivityAndQuadrics() {
        ...
        for (int v = 0; v < nV; ++v) {
            ...
            for (int fi : vertexFaces[v]) {
                ...
                if (norm2(n) < 1e-30) continue;
                if (norm2(avgNormal) > 1e-30) {
                    curvatureRisk = max(curvatureRisk, 1.0 - fabs(dot(n, avgNormal)));
                }
                double nearAxisSilhouette = 0.0;
                if (fabs(n.x) < 0.18) nearAxisSilhouette += 1.0;
                if (fabs(n.y) < 0.18) nearAxisSilhouette += 1.0;
                if (fabs(n.z) < 0.18) nearAxisSilhouette += 1.0;
                axisRisk = max(axisRisk, nearAxisSilhouette / 3.0);
            }

            double valence = (double)vertexNeighbors[v].size();
            double valenceRisk = valence <= 8.0 ? 0.0 : min(1.0, (valence - 8.0) / 8.0);
            vertexRisk[v] = 1.5 * curvatureRisk + 0.4 * axisRisk + 0.4 * valenceRisk;
        }
    }
    ...
    bool useLooseLockForEdge(int a, int b) const {
        if (a < 0 || b < 0 || a >= (int)vertexRisk.size() || b >= (int)vertexRisk.size()) return false;
        if (max(vertexRisk[a], vertexRisk[b]) > HParam_FeatureLooseLockRisk) return false;
        if (max(clusterRadius[a], clusterRadius[b]) > HParam_FeatureLooseLockRadiusFrac * hausdorffLimit) return false;
        return true;
    }

    bool commonNeighborLocked(int a, int b) const {
        const auto& na = vertexNeighbors[a];
        const auto& nb = vertexNeighbors[b];
        if (na.size() < nb.size()) {
            for (int x : na) {
                if (x == a || x == b || vertexDead[x]) continue;
                if (nb.find(x) != nb.end() && tailLocks[x] == tailLockStamp) return true;
            }
        } else {
            for (int x : nb) {
                if (x == a || x == b || vertexDead[x]) continue;
                if (na.find(x) != na.end() && tailLocks[x] == tailLockStamp) return true;
            }
        }
        return false;
    }

    void lockCommonNeighbors(int a, int b) {
        const auto& na = vertexNeighbors[a];
        const auto& nb = vertexNeighbors[b];
        if (na.size() < nb.size()) {
            for (int x : na) {
    ...
    bool batchNeighborhoodFree(int a, int b) const {
        ...
        if (useLooseLockForEdge(a, b)) return !commonNeighborLocked(a, b);
    ...
    void lockBatchNeighborhood(int a, int b) {
        ...
        if (useLooseLockForEdge(a, b)) {
            lockCommonNeighbors(a, b);
            return;
        }
