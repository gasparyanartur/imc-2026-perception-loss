...
// v29.1 Flip-Unlock state. The v23 core and its timing remain unchanged.
int fuOriginalN=0;
...
bool fuBuildCandidate(int center,const vector<Vec3>&vv,const vector<Face>&ff,
    ...
    int tier=fuTier(),m=(int)ring.size();
    double oldDevLimit=tier==2?0.018:(tier==3?0.026:0.032);
    double newDevLimit=tier==2?0.034:(tier==3?0.044:0.052);
    double distFrac=tier==2?0.28:(tier==3?0.34:0.40);
    double areaLimit=tier==2?0.085:(tier==3?0.11:0.14);
    ...
    for(int i=0;i<m;++i){
        const Vec3&p=vv[ring[i]];
        p2[i]=drop==0?FUPoint2{p.y,p.z}:(drop==1?FUPoint2{p.x,p.z}:FUPoint2{p.x,p.y});
    }
    double polyArea=0.0;
    for(int i=0;i<m;++i)polyArea+=p2[i].x*p2[(i+1)%m].y-p2[i].y*p2[(i+1)%m].x;
    double eps2=max(1e-18,diag*diag*1e-13);
    if(polyArea<0.0){reverse(ring.begin(),ring.end());reverse(p2.begin(),p2.end());polyArea=-polyArea;}
    if(polyArea<=eps2)return false;
    // The projected one-ring must be a simple polygon. Non-simple rings are
    // left to v23 because any diagonalization would be ambiguous and risky.
    for(int i=0;i<m;++i){
        int in=(i+1)%m;
        for(int j=i+1;j<m;++j){
            int jn=(j+1)%m;
    ...
    double maxDist=hausd*distFrac;
    double maxDist2=maxDist*maxDist;
    auto triCost=[&](int i,int k,int j,double&devOut)->double{
        if(!diagOK[i][k]||!diagOK[k][j]||!diagOK[i][j])return INF;
        double o=fuOrient2(p2[i],p2[k],p2[j]);
        if(o<=eps2)return INF;
        ...
        double d=dot(nr/nl,avgN);if(d<=0.0)return INF;
        double dev=1.0-d;if(dev>newDevLimit)return INF;
        double d2=fuPointTriangleDist2(vv[center],a,b,c);if(d2>maxDist2)return INF;
        double l0=norm2(b-a),l1=norm2(c-b),l2=norm2(a-c);
        double quality=(l0+l1+l2)/(nl+diag*diag*1e-15);
        double sliver=max(0.0,quality-5.0);
        devOut=dev;
        return (nl/(diag*diag+CParam_MinNormalNorm))*dev +
               0.00025*sliver + 0.020*d2/(maxDist2+diag*diag*1e-15);
    };
    for(int len=2;len<m;++len){
        for(int i=0;i+len<m;++i){
            ...
            for(int k=i+1;k<j;++k){
                ...
                double dev=0.0,tc=triCost(i,k,j,dev);if(tc>=INF)continue;
    ...
    out.score=dp[0][m-1]+0.06*areaRel+0.025*maxNewDev+0.01*oldDev;
    return out.valid();
}
...
bool fuScoreGuard(const RefScore&cand,int tier,int R)const{
    // The reference is the exact v23 result, so this is a delta guard rather
    // than an imperfect estimate of the official absolute score.
    if(R<=256){
        return cand.finalScore>=0.9990&&cand.minView>=0.9983&&
               cand.minNormal>=0.9976&&cand.minDepth>=0.9993;
    }
    if(tier==2)return cand.finalScore>=0.9972&&cand.minView>=0.9960&&
                     cand.minNormal>=0.9940&&cand.minDepth>=0.9990;
    return cand.finalScore>=0.9965&&cand.minView>=0.9952&&
           cand.minNormal>=0.9925&&cand.minDepth>=0.9987;
}
...
__attribute__((noinline,cold)) void postProcessFlipUnlock(){
    ...
    if(left<1.30||verts.size()<20||faces.size()<20)return;
    ...
    for(int v=0;v<(int)baseV.size();++v){
        if((v&255)==0&&elapsed()>HParam_TimeBudgetSeconds-0.72)break;
        FUCandidate c;if(fuBuildCandidate(v,baseV,baseF,adj,c))all.push_back(move(c));
    }
    if(all.empty()){
#ifdef FU_DEBUG
        fprintf(stderr,"[v29] no flip-unlock candidates, entry=%.3f now=%.3f\n",HParam_TimeBudgetSeconds-left,elapsed());
#endif
        return;
    }
    sort(all.begin(),all.end());
    int tier=fuTier();
    int R=(HParam_TimeBudgetSeconds-elapsed()>2.15)?384:256;
    int cap=R<=256?(tier==2?10:(tier==3?14:16)):(tier==2?24:(tier==3?36:40));
...
#ifdef FU_DEBUG
    fprintf(stderr,"[v29] tier=%d generated=%zu independent=%zu R=%d entry=%.3f\n",tier,all.size(),chosen.size(),R,HParam_TimeBudgetSeconds-left);
#endif
    if(chosen.empty()||elapsed()>HParam_TimeBudgetSeconds-0.60)return;
    vector<RefImage>ref(6);
    for(int view=0;view<6;++view){
        if(elapsed()>HParam_TimeBudgetSeconds-0.38)return;
        ref[view]=renderReference(baseV,baseF,view,R);
    }
    int maxTake=(int)chosen.size();
    vector<int>tries{maxTake};
    if(R>256&&maxTake>=4)tries.push_back(maxTake/2);
    tries.push_back(1);
    sort(tries.begin(),tries.end(),greater<int>());
    tries.erase(unique(tries.begin(),tries.end()),tries.end());
    for(int take:tries){
        if(elapsed()>HParam_TimeBudgetSeconds-0.24)break;
...
#ifdef FU_DEBUG
        fprintf(stderr,"[v29] try=%d final=%.7f view=%.7f normal=%.7f depth=%.7f t=%.3f\n",take,sc.finalScore,sc.minView,sc.minNormal,sc.minDepth,elapsed());
#endif
        if(fuScoreGuard(sc,tier,R)){
            verts.swap(candV);faces.swap(candF);nV=(int)verts.size();nF=(int)faces.size();
#ifdef FU_DEBUG
            fprintf(stderr,"[v29] accepted=%d outputV=%d outputF=%d\n",take,nV,nF);
#endif
            return;
        }
    }
#ifdef FU_DEBUG
    fprintf(stderr,"[v29] rollback: no batch passed\n");
#endif
}
void initScale(){
