...
struct Face{int v[3];};
static vector<double> gFaceLearnMid;
static vector<Face> gLearnOrigFaces;
static vector<vector<int>> gLearnOrigVF;
...
if(nV>5000&&nV<=50000){runIsolatedLearnedMid();writeMesh();return;}
...
__attribute__((noinline,cold)) void midPlain(vector<Vec3>&vv,vector<Face>&ff)const{if(vdead.size()==verts.size()&&fdead.size()==faces.size())extractActiveMesh(vv,ff);else{vv=verts;ff=faces;}}
struct LearnImageMid{vector<float>nx,ny,nz,d,z;vector<unsigned char>fg;vector<int>id;LearnImageMid(int R):nx((size_t)R*R,127.5f),ny((size_t)R*R,127.5f),nz((size_t)R*R,127.5f),d((size_t)R*R,255.0f),z((size_t)R*R,1e30f),fg((size_t)R*R,0),id((size_t)R*R,-1){}};
__attribute__((noinline,cold)) LearnImageMid renderLearnMid(const vector<Vec3>&vv,const vector<Face>&ff,int view,int R)const{LearnImageMid im(R);for(int fi=0;fi<(int)ff.size();++fi){const Face&fc=ff[fi];auto p0=projectImp(vv[fc.v[0]],view,R),p1=projectImp(vv[fc.v[1]],view,R),p2=projectImp(vv[fc.v[2]],view,R);if(!p0.ok||!p1.ok||!p2.ok)continue;double den=(p1.v-p2.v)*(p0.u-p2.u)+(p2.u-p1.u)*(p0.v-p2.v);if(fabs(den)<1e-18)continue;Vec3 nr=cross(vv[fc.v[1]]-vv[fc.v[0]],vv[fc.v[2]]-vv[fc.v[0]]);double nl=norm(nr);if(nl<1e-15)continue;nr=nr/nl;int x0=max(0,(int)floor(min({p0.u,p1.u,p2.u}))),x1=min(R-1,(int)ceil(max({p0.u,p1.u,p2.u}))),y0=max(0,(int)floor(min({p0.v,p1.v,p2.v}))),y1=min(R-1,(int)ceil(max({p0.v,p1.v,p2.v})));for(int y=y0;y<=y1;++y)for(int x=x0;x<=x1;++x){double X=x+.5,Y=y+.5,w0=((p1.v-p2.v)*(X-p2.u)+(p2.u-p1.u)*(Y-p2.v))/den,w1=((p2.v-p0.v)*(X-p2.u)+(p0.u-p2.u)*(Y-p2.v))/den,w2=1-w0-w1;if(w0<-1e-9||w1<-1e-9||w2<-1e-9)continue;double iz=w0/p0.z+w1/p1.z+w2/p2.z;if(iz<=0)continue;float zz=1.0/iz;int q=y*R+x;if(zz>=im.z[q])continue;im.z[q]=zz;im.nx[q]=(nr.x+1)*127.5;im.ny[q]=(nr.y+1)*127.5;im.nz[q]=(nr.z+1)*127.5;im.d[q]=zz;im.fg[q]=1;im.id[q]=fi;}}return im;}
__attribute__((noinline,cold)) RefScore compareLearnMid(const vector<LearnImageMid>&ref,const vector<Vec3>&vv,const vector<Face>&ff,int R)const{RefScore o;for(int v=0;v<6;++v){RefImage b=renderReference(vv,ff,v,R);double sn=(refSsim(ref[v].nx,b.nx,ref[v].fg,b.fg,R)+refSsim(ref[v].ny,b.ny,ref[v].fg,b.fg,R)+refSsim(ref[v].nz,b.nz,ref[v].fg,b.fg,R))/3,sd=refSsim(ref[v].d,b.d,ref[v].fg,b.fg,R),cv=.5*(sn+sd);o.finalScore+=cv;o.minView=min(o.minView,cv);o.minNormal=min(o.minNormal,sn);o.minDepth=min(o.minDepth,sd);}o.finalScore/=6;return o;}
__attribute__((noinline,cold)) void learnMidErrors(const vector<LearnImageMid>&ref,const vector<Vec3>&vv,const vector<Face>&ff,int R,vector<double>&boost,double gain)const{vector<double>e(boost.size()),cnt(boost.size());for(int v=0;v<6;++v){RefImage b=renderReference(vv,ff,v,R);for(int q=0;q<R*R;++q){int fi=ref[v].id[q];if(fi<0||fi>=(int)e.size())continue;double z=0;if(ref[v].fg[q]!=b.fg[q])z=10;else if(ref[v].fg[q]){double dx=(ref[v].nx[q]-b.nx[q])/255,dy=(ref[v].ny[q]-b.ny[q])/255,dz=(ref[v].nz[q]-b.nz[q])/255,dd=fabs(ref[v].d[q]-b.d[q]);z=2.2*(dx*dx+dy*dy+dz*dz)+.45*min(1.0,dd/.025);}e[fi]+=z;cnt[fi]++;}}for(int i=0;i<(int)boost.size();++i){double a=e[i]/(cnt[i]+2);boost[i]=min(8.0,max(1.0,1+gain*sqrt(max(0.0,a))));}vector<double>sm=boost;for(int fi=0;fi<(int)gLearnOrigFaces.size();++fi){double s=boost[fi],w=1;const Face&f=gLearnOrigFaces[fi];for(int k=0;k<3;++k)for(int fj:gLearnOrigVF[f.v[k]]){s+=.18*boost[fj];w+=.18;}sm[fi]=s/w;}boost.swap(sm);}
__attribute__((noinline,cold)) Quadric learnedFaceQ(int fi)const{const Face&f=faces[fi];Vec3 n=cross(verts[f.v[1]]-verts[f.v[0]],verts[f.v[2]]-verts[f.v[0]]);double area=.5*norm(n);Quadric q=Quadric::fromTriangle(verts[f.v[0]],verts[f.v[1]],verts[f.v[2]]);if(screenTier&&fi<(int)facePix.size()){double imp=facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi],r=(imp+.12*faceImpMean)/(faceImpMean+1e-12);double w=screenTier==2?min(18.0,max(.06,.08+.92*pow(max(0.0,r),.90))):screenTier==3?min(7.0,max(.12,.15+.85*sqrt(max(0.0,r)))):min(16.0,max(.06,.08+.92*pow(max(0.0,r),.72)));if(fi<(int)gFaceLearnMid.size())w*=gFaceLearnMid[fi];if(w!=1)q.scale(w);}else if(area>=1e-30){n=n/(2*area);double w=faceWeightFor(n,area);if(w!=1)q.scale(w);}return q;}
__attribute__((noinline,cold)) void learnedInitWeights(){vquad.assign(nV,Quadric());faceImpMean=0;int af=0;if(screenTier)for(int fi=0;fi<nF;++fi)if(!fdead[fi]){faceImpMean+=screenTier==3?(facePix[fi]+10.0*faceSil[fi]):(facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi]);++af;}if(screenTier)faceImpMean/=max(1,af);vertAnchor.assign(nV,0);if(screenTier==2||screenTier==4)for(int fi=0;fi<nF;++fi)if(!fdead[fi])for(int k=0;k<3;++k)vertAnchor[faces[fi].v[k]]+=faceSil[fi]+.05*facePix[fi]+.10*faceWin[fi];for(int fi=0;fi<nF;++fi)if(!fdead[fi]){Quadric q=learnedFaceQ(fi);for(int k=0;k<3;++k)vquad[faces[fi].v[k]]+=q;}if(screenTier==2||screenTier==4){double am=0;int ac=0;for(int i=0;i<nV;++i)if(!vdead[i]){am+=vertAnchor[i];++ac;}am/=max(1,ac);double bw=(screenTier==2?.0000035:.0000050)*diag*diag/(am+1);for(int i=0;i<nV;++i)if(!vdead[i]&&vertAnchor[i]>0)vquad[i]+=anchorPointQuadric(verts[i],bw*min(18.0,vertAnchor[i]/(am+1e-12)));if(screenTier==4)for(int i=0;i<nV;++i)if(!vdead[i])vquad[i]+=vmoment[i];}}
__attribute__((noinline,cold)) void learnedApply(int ab,int kp,const Vec3&np,double nr){verts[kp]=np;crad[kp]=nr;crad[ab]=0;vdead[ab]=1;++vver[ab];++vver[kp];auto fs=vfaces[ab];vector<int>dead;for(int fi:fs){if(fdead[fi])continue;bool t=false;for(int k=0;k<3;++k)if(faces[fi].v[k]==ab){faces[fi].v[k]=kp;t=true;}if(!t)continue;if(faces[fi].v[0]==faces[fi].v[1]||faces[fi].v[1]==faces[fi].v[2]||faces[fi].v[0]==faces[fi].v[2]){fdead[fi]=1;dead.push_back(fi);}else vfaces[kp].push_back(fi);}for(int fi:dead)for(int k=0;k<3;++k){int v=faces[fi].v[k];if(v>=0&&v<(int)vfaces.size())eraseVal(vfaces[v],fi);}vfaces[ab].clear();if(kp<(int)vmoment.size()&&ab<(int)vmoment.size())vmoment[kp]+=vmoment[ab];if(MEMLESS){Quadric fresh;for(int fi:vfaces[kp])if(!fdead[fi])fresh+=((screenTier==2||screenTier==4)?learnedFaceQ(fi):Quadric::fromTriangle(verts[faces[fi].v[0]],verts[faces[fi].v[1]],verts[faces[fi].v[2]]));if(screenTier==4&&kp<(int)vmoment.size())fresh+=vmoment[kp];vquad[kp]=fresh;}else vquad[kp]+=vquad[ab];for(int nb:vneigh[ab])if(nb!=kp&&!vdead[nb]){vneigh[nb].erase(ab);vneigh[nb].insert(kp);vneigh[kp].insert(nb);}vneigh[ab].clear();vneigh[kp].erase(ab);vneigh[kp].erase(kp);for(int nb:vneigh[kp])if(nb!=kp&&!vdead[nb]){auto c=computeQueueCandidate(kp,nb);if(c.valid())pq.push(c);}}
__attribute__((noinline,cold)) void learnedCollapseLoop(){int tick=0;while(accepted<collapseLimit&&!pq.empty()){if((++tick&8191)==0&&elapsed()>HParam_TimeBudgetSeconds)break;auto c=pq.top();pq.pop();int a=c.absorbed,b=c.kept;if(!edgeExists(a,b))continue;if(c.versionAbsorbed!=vver[a]||c.versionKept!=vver[b]){auto fr=computeQueueCandidate(a,b);if(fr.valid())pq.push(fr);continue;}if(c.cost>costCap)break;if(countCommonFaces(a,b)!=2||countCommonNeighbors(a,b)!=2)continue;auto best=computeBestValid(a,b);if(!best.valid()||best.cost>costCap)continue;learnedApply(best.absorbed,best.kept,best.position,best.mergedRadius);++accepted;}}
__attribute__((noinline,cold)) void resetLearnMid(const MidSnap&orig,int tier,const vector<double>&boost,int originalN,double ratio){restoreMid(orig);screenTier=tier;gFaceLearnMid=boost;costCap=HParam_QemCostCapCoeff*diag*diag*30;auto stage=[&](double r,int R){if(tier==3)buildRasterImportanceTier3(R);else buildRasterImportance(R);learnedInitWeights();priority_queue<CollapseCandidate>e;pq.swap(e);targetV=max(10,(int)floor(originalN*r));collapseLimit=originalN-targetV;initQueue();learnedCollapseLoop();};double mid=tier==2?.32:(tier==3?.16:.10);if(ratio<mid-.001)stage(mid,tier==2?640:(tier==3?384:512));if(elapsed()<HParam_TimeBudgetSeconds-1.2)stage(ratio,tier==2?768:(tier==3?512:640));}
__attribute__((noinline,cold)) void rawResetMid(const vector<Vec3>&ov,const vector<Face>&of){verts=ov;faces=of;nV=verts.size();nF=faces.size();vdead.clear();fdead.clear();vver.clear();vquad.clear();vmoment.clear();crad.clear();vfaces.clear();vneigh.clear();tailLocks.clear();facePix.clear();faceSil.clear();faceWin.clear();vertAnchor.clear();gFaceLearnMid.clear();screenTier=0;faceImpMean=0;accepted=0;targetV=collapseLimit=0;tailCursor=0;tailStamp=1;lastFailBatch=-1;starCursor=vegaCursor=0;priority_queue<CollapseCandidate>e;pq.swap(e);}

struct FitQMid{double a[3][3]{};double b[3]{};void plane(const Vec3&n,const Vec3&p,double w){double d=dot(n,p),c[3]={n.x,n.y,n.z};for(int i=0;i<3;++i){b[i]+=w*c[i]*d;for(int j=0;j<3;++j)a[i][j]+=w*c[i]*c[j];}}void point(const Vec3&p,double w){a[0][0]+=w;a[1][1]+=w;a[2][2]+=w;b[0]+=w*p.x;b[1]+=w*p.y;b[2]+=w*p.z;}};
__attribute__((noinline,cold)) bool solveFitMid(FitQMid q,Vec3&x)const{double A[3][4];for(int i=0;i<3;++i){for(int j=0;j<3;++j)A[i][j]=q.a[i][j];A[i][3]=q.b[i];}for(int c=0;c<3;++c){int p=c;for(int r=c+1;r<3;++r)if(fabs(A[r][c])>fabs(A[p][c]))p=r;if(fabs(A[p][c])<1e-12)return false;for(int j=c;j<4;++j)swap(A[c][j],A[p][j]);double d=A[c][c];for(int j=c;j<4;++j)A[c][j]/=d;for(int r=0;r<3;++r)if(r!=c){double t=A[r][c];for(int j=c;j<4;++j)A[r][j]-=t*A[c][j];}}x={A[0][3],A[1][3],A[2][3]};return finiteVec(x);}
__attribute__((noinline,cold)) bool imageFitMid(const vector<Vec3>&ov,const vector<Face>&of,vector<Vec3>&cv,const vector<Face>&cf,int R=192)const{vector<LearnImageMid>ref,cur;for(int v=0;v<6;++v){ref.push_back(renderLearnMid(ov,of,v,R));cur.push_back(renderLearnMid(cv,cf,v,R));}vector<vector<int>>vf(cv.size());for(int fi=0;fi<(int)cf.size();++fi)for(int k=0;k<3;++k)vf[cf[fi].v[k]].push_back(fi);vector<FitQMid>qs(cv.size());vector<double>cnt(cv.size());for(int view=0;view<6;++view){Vec3 eye,rr,uu,fw;cameraBasisImp(view,eye,rr,uu,fw);double F=800.0*R/1024.0;for(int y=0;y<R;++y)for(int x=0;x<R;++x){int q=y*R+x;if(!ref[view].fg[q]||!cur[view].fg[q])continue;int fi=cur[view].id[q];if(fi<0||fi>=(int)cf.size())continue;Vec3 nt={ref[view].nx[q]/127.5-1,ref[view].ny[q]/127.5-1,ref[view].nz[q]/127.5-1},nc={cur[view].nx[q]/127.5-1,cur[view].ny[q]/127.5-1,cur[view].nz[q]/127.5-1};double zt=ref[view].d[q],zc=cur[view].d[q];Vec3 pt=eye+rr*((x+.5-R*.5)*zt/F)+uu*((y+.5-R*.5)*zt/F)+fw*zt;double en=max(0.0,1-dot(nt,nc)),ed=fabs(zt-zc),wp=.12+8*en+12*min(.03,ed),wd=.025+2*min(.03,ed);const Face&f=cf[fi];for(int k=0;k<3;++k){int id=f.v[k];qs[id].plane(nt,pt,wp);qs[id].plane(fw,pt,wd);cnt[id]+=wp+wd;}}}vector<Vec3>old=cv,prop=cv;double step=.00025;int moved=0;for(int i=0;i<(int)cv.size();++i){qs[i].point(old[i],100+.02*cnt[i]);Vec3 p;if(!solveFitMid(qs[i],p))continue;Vec3 d=p-old[i];double dl=norm(d);if(dl>step)d=d*(step/dl);Vec3 np=old[i]+d;bool ok=true;for(int fi:vf[i]){const Face&f=cf[fi];Vec3 a=old[f.v[0]],b=old[f.v[1]],c=old[f.v[2]],on=cross(b-a,c-a);if(f.v[0]==i)a=np;if(f.v[1]==i)b=np;if(f.v[2]==i)c=np;Vec3 nn=cross(b-a,c-a);double l1=norm(on),l2=norm(nn);if(l1<CParam_MinNormalNorm||l2<CParam_MinNormalNorm||dot(on,nn)<.25*l1*l2){ok=false;break;}}if(ok&&dl>1e-12){prop[i]=np;++moved;}}if(!moved)return false;for(const Face&f:cf){Vec3 n=cross(prop[f.v[1]]-prop[f.v[0]],prop[f.v[2]]-prop[f.v[0]]);if(norm(n)<CParam_MinNormalNorm)return false;}cv.swap(prop);return true;}
__attribute__((noinline,cold)) void runIsolatedLearnedMid(){int originalN=nV,tier=nV<=25000?2:(nV<=45000?3:4);vector<Vec3>ov=verts,safeV,candV,bestV;vector<Face>of=faces,safeF,candF,bestF;
 if(tier==3){runScreenCoreMid();midPlain(safeV,safeF);}else{runTransactionalScreenMid();midPlain(safeV,safeF);}if(elapsed()>HParam_TimeBudgetSeconds-7.2){verts=safeV;faces=safeF;nV=verts.size();nF=faces.size();vdead.clear();fdead.clear();return;}
 rawResetMid(ov,of);screenTier=tier;initScale();buildConnectivity();vmoment.assign(nV,Quadric());if(tier==4)for(int i=0;i<nV;++i)vmoment[i]=anchorPointQuadric(verts[i],1e-6);MidSnap orig=midSnapshot();gLearnOrigFaces=of;gLearnOrigVF.assign(originalN,{});for(int fi=0;fi<(int)of.size();++fi)for(int k=0;k<3;++k)gLearnOrigVF[of[fi].v[k]].push_back(fi);int RL=512,RX=768;vector<LearnImageMid>refL;vector<RefImage>refX(6);for(int v=0;v<6;++v){refL.push_back(renderLearnMid(ov,of,v,RL));refX[v]=renderReference(ov,of,v,RX);}double ratio=tier==2?.18:(tier==3?.095:.060);vector<double>boost(of.size(),1);double bs=-1;for(int it=0;it<2&&elapsed()<HParam_TimeBudgetSeconds-2.4;++it){resetLearnMid(orig,tier,boost,originalN,ratio);midPlain(candV,candF);RefScore ls=compareLearnMid(refL,candV,candF,RL);if(ls.finalScore>bs){bs=ls.finalScore;bestV=candV;bestF=candF;}if(it==0)learnMidErrors(refL,candV,candF,RL,boost,8);}bool use=false;if(!bestV.empty()&&elapsed()<HParam_TimeBudgetSeconds-1.0){RefScore before=compareToReference(refX,bestV,bestF,RX);if(elapsed()<HParam_TimeBudgetSeconds-1.6){vector<Vec3>fitV=bestV;if(imageFitMid(ov,of,fitV,bestF)){RefScore fs=compareToReference(refX,fitV,bestF,RX);if(fs.finalScore>before.finalScore&&fs.minView>=before.minView-.0003&&fs.minNormal>=before.minNormal-.0003){bestV.swap(fitV);before=fs;}}}RefScore ex=before;use=tier==2?(ex.finalScore>=.940&&ex.minView>=.910&&ex.minNormal>=.880&&ex.minDepth>=.975):tier==3?(ex.finalScore>=.935&&ex.minView>=.900&&ex.minNormal>=.860&&ex.minDepth>=.970):(ex.finalScore>=.925&&ex.minView>=.890&&ex.minNormal>=.835&&ex.minDepth>=.960);}if(use){verts.swap(bestV);faces.swap(bestF);}else{verts.swap(safeV);faces.swap(safeF);}nV=verts.size();nF=faces.size();vdead.clear();fdead.clear();}
