// Pineapple v072 + cold Nebula Atomic Region v33
// T2 only uses the atomic transaction; T3-T7 retain the Pineapple code path.
// The added branch is noinline/cold to keep large-tier code generation isolated.
...
public:
    void run()
    {
        ...
        if (nV > 5000 && nV <= 50000)
        {
            if (nV <= 25000)
            {
                runTransactionalScreenT2Atomic();
            }
            else
            {
                runScreenCoreMid();
...
private:
    ...
    // ---- Cold T2-only atomic-region branch (v33) ----
...
__attribute__((noinline,cold)) MidSnap midSnapshot()const{return {verts,faces,vdead,fdead,vver,vquad,vmoment,crad,vfaces,vneigh,tailLocks,facePix,faceSil,faceWin,vertAnchor,targetV,collapseLimit,accepted,tailCursor,tailStamp,lastFailBatch,starCursor,vegaCursor,screenTier,faceImpMean};}
__attribute__((noinline,cold)) void restoreMid(const MidSnap&s){verts=s.verts;faces=s.faces;vdead=s.vdead;fdead=s.fdead;vver=s.vver;vquad=s.vquad;vmoment=s.vmoment;crad=s.crad;vfaces=s.vfaces;vneigh=s.vneigh;tailLocks=s.tailLocks;facePix=s.facePix;faceSil=s.faceSil;faceWin=s.faceWin;vertAnchor=s.vertAnchor;targetV=s.targetV;collapseLimit=s.collapseLimit;accepted=s.accepted;tailCursor=s.tailCursor;tailStamp=s.tailStamp;lastFailBatch=s.lastFailBatch;starCursor=s.starCursor;vegaCursor=s.vegaCursor;screenTier=s.screenTier;faceImpMean=s.faceImpMean;priority_queue<CollapseCandidate>e;pq.swap(e);}
__attribute__((noinline,cold)) void continueScreenTo(int tier,int originalN,double ratio,int freeRemoved){if(tier==3)buildRasterImportanceTier3(512);else buildRasterImportance(tier==2?1024:768);initFaceWeights();priority_queue<CollapseCandidate>e;pq.swap(e);targetV=max(10,(int)floor(originalN*ratio)-freeRemoved);collapseLimit=originalN-targetV;initQueue();collapseLoop();}
__attribute__((noinline,cold)) bool txGuard(const RefScore&s,int tier)const{if(tier==2)return s.finalScore>=.9965&&s.minView>=.9955&&s.minNormal>=.9930&&s.minDepth>=.9988;if(tier==3)return s.finalScore>=.9950&&s.minView>=.9942&&s.minNormal>=.9892&&s.minDepth>=.9985;return s.finalScore>=.9950&&s.minView>=.9942&&s.minNormal>=.9892&&s.minDepth>=.9985;}
