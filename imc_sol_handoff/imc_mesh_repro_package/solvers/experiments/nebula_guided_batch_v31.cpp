...
if(nV>5000&&nV<=50000){if(nV>25000&&nV<=45000){runScreenCoreMid();compact();writeMesh();return;}runTransactionalScreenMid();writeMesh();return;}
...
// v31: evaluator-guided micro-batch continuation for the 5k-25k tier.
// Rejected batches are bisected down to a single collapse; only the harmful
// edge is banned, allowing the rest of the lower-count trajectory to survive.
unordered_set<unsigned long long> guidedBannedEdges;
...
unsigned long long guidedEdgeKey(int a,int b)const{
if(a>b)swap(a,b);
return (unsigned long long)(unsigned int)a<<32 | (unsigned int)b;
}
bool guidedEdgeBanned(int a,int b)const{return guidedBannedEdges.find(guidedEdgeKey(a,b))!=guidedBannedEdges.end();}
void guidedRebuildQueue(){priority_queue<CollapseCandidate>e;pq.swap(e);initQueue();}
int guidedCollapseBatch(int maxTake,vector<pair<int,int>>&used){
used.clear();int took=0,pops=0;
while(took<maxTake&&accepted<collapseLimit&&!pq.empty()){
if((++pops&2047)==0&&elapsed()>HParam_TimeBudgetSeconds-1.35)break;
auto c=pq.top();pq.pop();int a=c.absorbed,b=c.kept;
if(guidedEdgeBanned(a,b))continue;
if(!edgeExists(a,b))continue;
if(c.versionAbsorbed!=vver[a]||c.versionKept!=vver[b]){auto fr=computeQueueCandidate(a,b);if(fr.valid()&&!guidedEdgeBanned(fr.absorbed,fr.kept))pq.push(fr);continue;}
...
if(guidedEdgeBanned(best.absorbed,best.kept))continue;
used.push_back({best.absorbed,best.kept});
applyCollapse(best.absorbed,best.kept,best.position,best.mergedRadius);++accepted;++took;
}
return took;
}
bool guidedCoarseGuard(const RefScore&s)const{
return s.finalScore>=.9962&&s.minView>=.9948&&s.minNormal>=.9918&&s.minDepth>=.9980;
}
...
vector<Vec3>safeV,candV,bestV;vector<Face>safeF,candF,bestF;
extractActiveMesh(safeV,safeF);bestV=safeV;bestF=safeF;
int R=1024;
...
// Exact v23 continuation and fallback.
vector<double>tries={.27,.28,.285,.29,.295};
...
if(txGuard(sc,tier)){bestV=candV;bestF=candF;break;}
}

// v31: preserve good collapses from a lower-count continuation.  A failed
// batch is rolled back and bisected; at size one, only that edge is banned.
if(elapsed()<HParam_TimeBudgetSeconds-5.2){
restoreMid(safe);guidedBannedEdges.clear();
buildRasterImportance(768);initFaceWeights();
priority_queue<CollapseCandidate>e;pq.swap(e);
targetV=max(10,(int)floor(originalN*.250));collapseLimit=originalN-targetV;initQueue();
const int RC=256;vector<RefImage>coarseRef(6);for(int v=0;v<6;++v)coarseRef[v]=renderReference(safeV,safeF,v,RC);
int batch=24,rejects=0;bool haveGuided=false;MidSnap guidedBest;
vector<pair<int,int>>used;
while(elapsed()<HParam_TimeBudgetSeconds-3.00&&accepted<collapseLimit&&!pq.empty()&&rejects<96){
MidSnap before=midSnapshot();
int got=guidedCollapseBatch(batch,used);if(got<=0)break;
extractActiveMesh(candV,candF);RefScore cs=compareToReference(coarseRef,candV,candF,RC);
if(guidedCoarseGuard(cs)){
if(candV.size()<bestV.size()){guidedBest=midSnapshot();haveGuided=true;}
if(batch<48)batch=min(48,batch+4);
}else{
restoreMid(before);++rejects;
if(batch>1){batch=max(1,batch/2);}else{
for(auto &ed:used)guidedBannedEdges.insert(guidedEdgeKey(ed.first,ed.second));
batch=8;
}
guidedRebuildQueue();
}
}
if(haveGuided&&elapsed()<HParam_TimeBudgetSeconds-2.10){
restoreMid(guidedBest);extractActiveMesh(candV,candF);
RefScore gs=compareToReference(ref,candV,candF,R);
if(candV.size()<bestV.size()&&txGuard(gs,tier)){bestV=candV;bestF=candF;}
}
}
verts=bestV;faces=bestF;nV=(int)verts.size();nF=(int)faces.size();guidedBannedEdges.clear();
return;
}
runScreenCoreMid();
