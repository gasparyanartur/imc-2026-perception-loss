...
if(nV>5000&&nV<=50000){
    // v29.1 preserves the exact v23 core trajectory. The connectivity-changing
    // postprocess is opportunistic and runs only from genuine spare time.
    fuOriginalN=nV;
    if(nV>25000&&nV<=45000){
        runScreenCoreMid();
        compact();
        if(elapsed()<HParam_TimeBudgetSeconds-1.30)postProcessFlipUnlock();
        writeMesh();
        return;
    }
...
// v29.2 exact-coplanar Flip-Unlock state. The v23 core and its timing remain unchanged.
int fuOriginalN=0;
...
// ============================================================================
// v29.2 EXACT-COPLANAR FLIP-UNLOCK POSTPROCESSOR
//
// This stage runs only after the exact v23 T2-T4 path has produced a compact,
// valid mesh. It removes a vertex by replacing its entire one-ring with the
// best globally valid triangulation of the boundary polygon. Enumerating the
// polygon triangulation is equivalent to considering all local edge-flip
// sequences that reduce the center valence before deletion. Candidate batches
// are compared directly with the original six-view rendering; any rejected
// batch leaves the byte-for-byte v23 result untouched.
// ============================================================================
struct FUPoint2 { double x=0.0,y=0.0; };
struct FUAdjData {
    vector<vector<int>> vf;
    unordered_map<unsigned long long,vector<int>> ef;
};
struct FUCandidate {
    int center=-1;
    vector<int> incident;
    ...
    double maxNewDev=CParam_Inf;
    double areaRel=CParam_Inf;
    bool valid()const{return center>=0&&!incident.empty()&&ring.size()>=4&&tris.size()+2==ring.size()&&score<CParam_Inf;}
    bool operator<(const FUCandidate&o)const{
        if(score!=o.score)return score<o.score;
        return center<o.center;
    }
};

static unsigned long long fuEdgeKey(int a,int b){
    if(a>b)swap(a,b);
    return (static_cast<unsigned long long>(static_cast<unsigned int>(a))<<32) |
           static_cast<unsigned int>(b);
}
static double fuOrient2(const FUPoint2&a,const FUPoint2&b,const FUPoint2&c){
    return (b.x-a.x)*(c.y-a.y)-(b.y-a.y)*(c.x-a.x);
}
static bool fuPointOnSegment(const FUPoint2&p,const FUPoint2&a,const FUPoint2&b,double eps){
    if(fabs(fuOrient2(a,b,p))>eps)return false;
    return p.x>=min(a.x,b.x)-eps&&p.x<=max(a.x,b.x)+eps&&
           p.y>=min(a.y,b.y)-eps&&p.y<=max(a.y,b.y)+eps;
}
static bool fuSegmentsIntersect(const FUPoint2&a,const FUPoint2&b,
                                const FUPoint2&c,const FUPoint2&d,double eps){
    double o1=fuOrient2(a,b,c),o2=fuOrient2(a,b,d),o3=fuOrient2(c,d,a),o4=fuOrient2(c,d,b);
    if(((o1>eps&&o2<-eps)||(o1<-eps&&o2>eps))&&
       ((o3>eps&&o4<-eps)||(o3<-eps&&o4>eps)))return true;
    if(fabs(o1)<=eps&&fuPointOnSegment(c,a,b,eps))return true;
    if(fabs(o2)<=eps&&fuPointOnSegment(d,a,b,eps))return true;
    if(fabs(o3)<=eps&&fuPointOnSegment(a,c,d,eps))return true;
    if(fabs(o4)<=eps&&fuPointOnSegment(b,c,d,eps))return true;
    return false;
}
static bool fuPointInPolygon(const FUPoint2&p,const vector<FUPoint2>&poly){
    bool inside=false;
    int n=(int)poly.size();
    for(int i=0,j=n-1;i<n;j=i++){
        const FUPoint2&a=poly[i],&b=poly[j];
...
static double fuPointTriangleDist2(const Vec3&p,const Vec3&a,const Vec3&b,const Vec3&c){
    ...
    if(vc<=0.0&&d1>=0.0&&d3<=0.0){double v=d1/(d1-d3);return norm2(p-(a+ab*v));}
    Vec3 cp=p-c;
    double d5=dot(ab,cp),d6=dot(ac,cp);
    if(d6>=0.0&&d5<=d6)return norm2(cp);
    double vb=d5*d2-d1*d6;
    if(vb<=0.0&&d2>=0.0&&d6<=0.0){double w=d2/(d2-d6);return norm2(p-(a+ac*w));}
...
int fuTier()const{
    if(fuOriginalN<=25000)return 2;
    if(fuOriginalN<=45000)return 3;
    return 4;
}
void fuBuildAdj(const vector<Vec3>&vv,const vector<Face>&ff,FUAdjData&adj)const{
    adj.vf.assign(vv.size(),{});
    adj.ef.clear();
    adj.ef.reserve(ff.size()*2+1);
    for(int fi=0;fi<(int)ff.size();++fi){
        const Face&f=ff[fi];
        if(f.v[0]<0||f.v[1]<0||f.v[2]<0||f.v[0]>=(int)vv.size()||f.v[1]>=(int)vv.size()||f.v[2]>=(int)vv.size())continue;
        adj.vf[f.v[0]].push_back(fi);adj.vf[f.v[1]].push_back(fi);adj.vf[f.v[2]].push_back(fi);
        for(int k=0;k<3;++k)adj.ef[fuEdgeKey(f.v[k],f.v[(k+1)%3])].push_back(fi);
    }
}
bool fuBuildOrientedRing(int center,const vector<Face>&ff,const FUAdjData&adj,
                         vector<int>&incident,vector<int>&ring)const{
    incident.clear();ring.clear();
    if(center<0||center>=(int)adj.vf.size())return false;
    incident=adj.vf[center];
    int m=(int)incident.size();
    if(m<4||m>10)return false;
    unordered_map<int,int>nxt;
    unordered_map<int,int>indeg;
    nxt.reserve(m*2);indeg.reserve(m*2);
    for(int fi:incident){
        if(fi<0||fi>=(int)ff.size())return false;
        const Face&f=ff[fi];
        int p=-1;for(int k=0;k<3;++k)if(f.v[k]==center){p=k;break;}
        if(p<0)return false;
        int a=f.v[(p+1)%3],b=f.v[(p+2)%3];
        if(a==b||a==center||b==center||nxt.count(a))return false;
        nxt[a]=b;++indeg[b];
    }
    if((int)nxt.size()!=m)return false;
    for(auto&kv:nxt)if(indeg[kv.first]!=1)return false;
    int start=nxt.begin()->first;
    for(auto&kv:nxt)start=min(start,kv.first);
    unordered_set<int>seen;seen.reserve(m*2);
    int cur=start;
    for(int i=0;i<m;++i){
        if(seen.count(cur)||!nxt.count(cur))return false;
        seen.insert(cur);ring.push_back(cur);cur=nxt[cur];
    }
    return cur==start&&(int)seen.size()==m;
}

bool fuBuildCandidate(int center,const vector<Vec3>&vv,const vector<Face>&ff,
                      const FUAdjData&adj,FUCandidate&out)const{
    out=FUCandidate();
    vector<int>incident,ring;
    if(!fuBuildOrientedRing(center,ff,adj,incident,ring))return false;
    const int m=(int)ring.size();

    // v29.2 is deliberately limited to numerically coplanar one-rings.  The
    // old star and the new polygon triangulation must represent the same local
    // planar surface, so the operation consumes essentially no SSIM or
    // Hausdorff budget.  This is a topology unlock, not another approximation.
    const double oldDevLimit=2.0e-8;
    const double newDevLimit=2.0e-8;
    const double planeTol=max(1.0e-12,diag*2.0e-7);
    const double planeTol2=planeTol*planeTol;
    const double areaLimit=2.0e-7;

    Vec3 avgN;double oldArea=0.0,oldDev=0.0;
    unordered_set<int>incSet;incSet.reserve(incident.size()*2);
    vector<array<Vec3,3>>oldTris;oldTris.reserve(incident.size());
    for(int fi:incident){
        incSet.insert(fi);
        const Face&f=ff[fi];
        Vec3 nr=cross(vv[f.v[1]]-vv[f.v[0]],vv[f.v[2]]-vv[f.v[0]]);
        double nl=norm(nr);if(nl<=CParam_MinNormalNorm)return false;
        avgN=avgN+nr;oldArea+=0.5*nl;
        oldTris.push_back({vv[f.v[0]],vv[f.v[1]],vv[f.v[2]]});
    }
    double an=norm(avgN);if(an<=CParam_MinNormalNorm||oldArea<=0.0)return false;
    avgN=avgN/an;
    for(int fi:incident){
        const Face&f=ff[fi];
        Vec3 nr=cross(vv[f.v[1]]-vv[f.v[0]],vv[f.v[2]]-vv[f.v[0]]);
        double nl=norm(nr);double d=dot(nr/nl,avgN);
        if(d<=0.0)return false;
        oldDev=max(oldDev,1.0-d);
    }
    if(oldDev>oldDevLimit)return false;

    // Every boundary point must lie on the center plane.  Since all old
    // triangles share center, this plus the normal test makes the old star a
    // single planar polygon to numerical precision.
    const Vec3 planePoint=vv[center];
    for(int x:ring)if(fabs(dot(vv[x]-planePoint,avgN))>planeTol)return false;

    Vec3 ringN;
    for(int i=0;i<m;++i)ringN=ringN+cross(vv[ring[i]],vv[ring[(i+1)%m]]);
    if(dot(ringN,avgN)<0.0)reverse(ring.begin(),ring.end());

    int drop=0;
    if(fabs(avgN.y)>fabs(avgN.x))drop=1;
    if(fabs(avgN.z)>fabs(drop==0?avgN.x:avgN.y))drop=2;
    auto proj2=[&](const Vec3&p)->FUPoint2{
        return drop==0?FUPoint2{p.y,p.z}:(drop==1?FUPoint2{p.x,p.z}:FUPoint2{p.x,p.y});
    };
    vector<FUPoint2>p2(m);
    for(int i=0;i<m;++i)p2[i]=proj2(vv[ring[i]]);
    FUPoint2 center2=proj2(vv[center]);

    double polyArea=0.0;
    for(int i=0;i<m;++i)polyArea+=p2[i].x*p2[(i+1)%m].y-p2[i].y*p2[(i+1)%m].x;
    double eps2=max(1e-20,diag*diag*1e-15);
    if(polyArea<0.0){reverse(ring.begin(),ring.end());reverse(p2.begin(),p2.end());polyArea=-polyArea;}
    if(polyArea<=eps2||!fuPointInPolygon(center2,p2))return false;
    ...
    for(int i=0;i<m;++i){
        ...
        for(int j=i+1;j<m;++j){
            ...
            if(fuSegmentsIntersect(p2[i],p2[in],p2[j],p2[jn],eps2))return false;
        }
    }

    vector<vector<char>>diagOK(m,vector<char>(m,0));
    for(int i=0;i<m;++i)for(int j=i+1;j<m;++j){
        bool adjacent=(j==i+1)||(i==0&&j==m-1);
        if(adjacent){diagOK[i][j]=diagOK[j][i]=1;continue;}
        auto eit=adj.ef.find(fuEdgeKey(ring[i],ring[j]));
        if(eit!=adj.ef.end()){
            bool outside=false;for(int fi:eit->second)if(!incSet.count(fi)){outside=true;break;}
            if(outside)continue;
        }
        bool crossEdge=false;
        for(int e=0;e<m;++e){
            int q=(e+1)%m;
            if(e==i||q==i||e==j||q==j)continue;
            if(fuSegmentsIntersect(p2[i],p2[j],p2[e],p2[q],eps2)){crossEdge=true;break;}
        }
        if(crossEdge)continue;
        FUPoint2 mid{0.5*(p2[i].x+p2[j].x),0.5*(p2[i].y+p2[j].y)};
        if(!fuPointInPolygon(mid,p2))continue;
        diagOK[i][j]=diagOK[j][i]=1;
    }

    const double INF=CParam_Inf;
    vector<vector<double>>dp(m,vector<double>(m,INF));
    vector<vector<int>>split(m,vector<int>(m,-1));
    for(int i=0;i+1<m;++i)dp[i][i+1]=0.0;
    auto triCost=[&](int i,int k,int j)->double{
        if(!diagOK[i][k]||!diagOK[k][j]||!diagOK[i][j])return INF;
        if(fuOrient2(p2[i],p2[k],p2[j])<=eps2)return INF;
        const Vec3&a=vv[ring[i]],&b=vv[ring[k]],&c=vv[ring[j]];
        Vec3 nr=cross(b-a,c-a);double nl=norm(nr);
        if(nl<=CParam_MinNormalNorm)return INF;
        double d=dot(nr/nl,avgN);if(d<=0.0||1.0-d>newDevLimit)return INF;
        double l0=norm2(b-a),l1=norm2(c-b),l2=norm2(a-c);
        double quality=(l0+l1+l2)/(nl+diag*diag*1e-18);
        return 1.0e-6*max(0.0,quality-4.0)+(1.0-d);
    };
    for(int len=2;len<m;++len)for(int i=0;i+len<m;++i){
        int j=i+len;
        for(int k=i+1;k<j;++k){
            if(dp[i][k]>=INF||dp[k][j]>=INF)continue;
            double tc=triCost(i,k,j);if(tc>=INF)continue;
            double val=dp[i][k]+dp[k][j]+tc;
            if(val<dp[i][j]){dp[i][j]=val;split[i][j]=k;}
        }
    }
    if(split[0][m-1]<0)return false;

    vector<array<int,3>>newTris;
    vector<array<Vec3,3>>newGeom;
    double maxNewDev=0.0,newArea=0.0;
    function<bool(int,int)>emit=[&](int i,int j)->bool{
        if(j<=i+1)return true;
        int k=split[i][j];if(k<=i||k>=j)return false;
        if(!emit(i,k)||!emit(k,j))return false;
        int a=ring[i],b=ring[k],c=ring[j];
        Vec3 nr=cross(vv[b]-vv[a],vv[c]-vv[a]);double nl=norm(nr);
        if(nl<=CParam_MinNormalNorm)return false;
        if(dot(nr,avgN)<0.0){swap(b,c);nr=nr*(-1.0);}
        maxNewDev=max(maxNewDev,1.0-dot(nr/nl,avgN));
        newArea+=0.5*nl;newTris.push_back({a,b,c});
        newGeom.push_back({vv[a],vv[b],vv[c]});
        return true;
    };
    if(!emit(0,m-1)||newTris.size()+2!=(size_t)m)return false;
    double areaRel=fabs(newArea-oldArea)/(oldArea+CParam_MinNormalNorm);
    if(maxNewDev>newDevLimit||areaRel>areaLimit)return false;

    // Dense local equivalence check in both directions.  This catches any DP
    // or concavity mistake even when the polygon passed the 2-D tests.
    auto worstSamples=[&](const vector<array<Vec3,3>>&A,const vector<array<Vec3,3>>&B){
        double worst=0.0;
        for(const auto&t:A){
            Vec3 s[10]={t[0],t[1],t[2],(t[0]+t[1])*.5,(t[1]+t[2])*.5,
                        (t[2]+t[0])*.5,(t[0]+t[1]+t[2])/3.0,
                        (t[0]*2.0+t[1]+t[2])*.25,
                        (t[0]+t[1]*2.0+t[2])*.25,
                        (t[0]+t[1]+t[2]*2.0)*.25};
            for(const Vec3&p:s){
                double best=CParam_Inf;
                for(const auto&q:B)best=min(best,fuPointTriangleDist2(p,q[0],q[1],q[2]));
                worst=max(worst,best);
            }
        }
        return worst;
    };
    double localWorst2=max(worstSamples(oldTris,newGeom),worstSamples(newGeom,oldTris));
    if(!isfinite(localWorst2)||localWorst2>16.0*planeTol2)return false;
    double centerBest=CParam_Inf;
    for(const auto&t:newGeom)centerBest=min(centerBest,fuPointTriangleDist2(vv[center],t[0],t[1],t[2]));
    if(centerBest>4.0*planeTol2)return false;

    out.center=center;out.incident=move(incident);out.ring=move(ring);out.tris=move(newTris);
    out.maxNewDev=maxNewDev;out.areaRel=areaRel;
    out.score=dp[0][m-1]+0.01*areaRel+0.001*m;
    return out.valid();
}

bool fuValidateCompactMesh(const vector<Vec3>&vv,const vector<Face>&ff)const{
    if(vv.size()<4||ff.size()<4)return false;
    unordered_map<unsigned long long,pair<int,int>>edges;
    edges.reserve(ff.size()*2+1);
    set<array<int,3>>seenFaces;
    for(const Face&f:ff){
        int a=f.v[0],b=f.v[1],c=f.v[2];
        if(a<0||b<0||c<0||a>=(int)vv.size()||b>=(int)vv.size()||c>=(int)vv.size()||a==b||b==c||a==c)return false;
        if(norm(cross(vv[b]-vv[a],vv[c]-vv[a]))<=CParam_MinNormalNorm)return false;
        array<int,3>key{a,b,c};sort(key.begin(),key.end());
        if(!seenFaces.insert(key).second)return false;
        int q[3]={a,b,c};
        for(int k=0;k<3;++k){
            int x=q[k],y=q[(k+1)%3];auto&v=edges[fuEdgeKey(x,y)];
            ++v.first;v.second+=(x<y)?1:-1;
        }
    }
    for(auto&kv:edges)if(kv.second.first!=2||kv.second.second!=0)return false;
    return true;
}

bool fuApplyPrefix(const vector<Vec3>&baseV,const vector<Face>&baseF,
                   const vector<FUCandidate>&chosen,int take,
                   vector<Vec3>&outV,vector<Face>&outF)const{
    vector<char>aliveV(baseV.size(),1),aliveF(baseF.size(),1);
    vector<Face>workF=baseF;
    for(int ci=0;ci<take;++ci){
        const FUCandidate&c=chosen[ci];
        if(c.center<0||c.center>=(int)aliveV.size()||!aliveV[c.center])return false;
        for(int fi:c.incident)if(fi<0||fi>=(int)aliveF.size()||!aliveF[fi])return false;
        for(int fi:c.incident)aliveF[fi]=0;
        aliveV[c.center]=0;
        for(auto t:c.tris){Face nf{{t[0],t[1],t[2]}};workF.push_back(nf);aliveF.push_back(1);}
    }
    vector<Face>oldIndexFaces;oldIndexFaces.reserve(workF.size());
    vector<char>used(baseV.size(),0);
    for(int fi=0;fi<(int)workF.size();++fi){
        if(!aliveF[fi])continue;
        const Face&f=workF[fi];
        if(f.v[0]<0||f.v[1]<0||f.v[2]<0||f.v[0]>=(int)baseV.size()||f.v[1]>=(int)baseV.size()||f.v[2]>=(int)baseV.size())return false;
        if(!aliveV[f.v[0]]||!aliveV[f.v[1]]||!aliveV[f.v[2]])return false;
        oldIndexFaces.push_back(f);used[f.v[0]]=used[f.v[1]]=used[f.v[2]]=1;
    }
    vector<int>map(baseV.size(),-1);outV.clear();outF.clear();
    for(int i=0;i<(int)baseV.size();++i)if(aliveV[i]&&used[i]){map[i]=(int)outV.size();outV.push_back(baseV[i]);}
    outF.reserve(oldIndexFaces.size());
    for(Face f:oldIndexFaces){
        f.v[0]=map[f.v[0]];f.v[1]=map[f.v[1]];f.v[2]=map[f.v[2]];
        if(f.v[0]<0||f.v[1]<0||f.v[2]<0)return false;
        outF.push_back(f);
    }
    return outV.size()+take==baseV.size()&&fuValidateCompactMesh(outV,outF);
}

bool fuScoreGuard(const RefScore&cand,int,int)const{
    // A coplanar retriangulation should be render-equivalent.  Any measurable
    // delta indicates a numerical, projection, or polygon-coverage mismatch,
    // so roll back rather than spending v23's perceptual margin.
    return cand.finalScore>=0.999995&&cand.minView>=0.999990&&
           cand.minNormal>=0.999985&&cand.minDepth>=0.999997;
}

__attribute__((noinline,cold)) void postProcessFlipUnlock(){
    if(fuOriginalN<=5000||fuOriginalN>50000)return;
    double left=HParam_TimeBudgetSeconds-elapsed();
    if(left<0.95||verts.size()<20||faces.size()<20)return;
    vector<Vec3>baseV=verts;vector<Face>baseF=faces;
    FUAdjData adj;fuBuildAdj(baseV,baseF,adj);
    vector<FUCandidate>all;all.reserve(min<size_t>(baseV.size(),4096));
    for(int v=0;v<(int)baseV.size();++v){
        if((v&255)==0&&elapsed()>HParam_TimeBudgetSeconds-0.55)break;
        FUCandidate c;if(fuBuildCandidate(v,baseV,baseF,adj,c))all.push_back(move(c));
    }
    if(all.empty())return;
    sort(all.begin(),all.end());
    int tier=fuTier();
    int cap=tier==2?48:(tier==3?72:80);
    vector<FUCandidate>chosen;chosen.reserve(cap);
    vector<char>locked(baseV.size(),0);
    for(const FUCandidate&c:all){
        if((int)chosen.size()>=cap)break;
        bool clash=locked[c.center];for(int x:c.ring)clash=clash||locked[x];
        if(clash)continue;
        locked[c.center]=1;for(int x:c.ring)locked[x]=1;
        chosen.push_back(c);
    }
    if(chosen.empty()||elapsed()>HParam_TimeBudgetSeconds-0.48)return;

    // One low-resolution delta render is enough here because the geometric
    // tests already require the old and new patches to be the same plane and
    // polygon.  The render is only a final bug/coverage detector.
    const int R=192;
    vector<RefImage>ref(6);
    for(int view=0;view<6;++view){
        if(elapsed()>HParam_TimeBudgetSeconds-0.30)return;
        ref[view]=renderReference(baseV,baseF,view,R);
    }
    vector<int>tries{(int)chosen.size()};
    if(chosen.size()>=4)tries.push_back((int)chosen.size()/2);
    tries.push_back(1);
    sort(tries.begin(),tries.end(),greater<int>());
    tries.erase(unique(tries.begin(),tries.end()),tries.end());
    for(int take:tries){
        if(elapsed()>HParam_TimeBudgetSeconds-0.16)break;
        vector<Vec3>candV;vector<Face>candF;
        if(!fuApplyPrefix(baseV,baseF,chosen,take,candV,candF))continue;
        RefScore sc=compareToReference(ref,candV,candF,R);
        if(fuScoreGuard(sc,tier,R)){
