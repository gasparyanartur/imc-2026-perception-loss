...
double cost=CParam_Inf,rawCost=CParam_Inf,mergedRadius=0.0;
...
// v30: normal-aware alternative is enabled only for the final continuation
// of the 5k-25k mid-tier. Every other path remains v23.
bool normalBranchMode=false;
int normalBranchSeed=1;
...
// Exact v23 continuation first. This remains the fallback.
vector<double>tries={.27,.28,.285,.29,.295};
for(double ratio:tries){
if(elapsed()>HParam_TimeBudgetSeconds-1.8)break;
restoreMid(safe);normalBranchMode=false;continueScreenTo(tier,originalN,ratio,0);
...
// New angle: branch only the last continuation. Rank collapses by predicted
// visible face-normal rotation, then use the exact same v23 transaction guard.
// If no smaller branch passes, output is exactly the standard v23 candidate.
vector<double>altTries={.265,.270,.275,.280};
for(double ratio:altTries){
if(elapsed()>HParam_TimeBudgetSeconds-1.05)break;
restoreMid(safe);normalBranchMode=true;normalBranchSeed=1;
continueScreenTo(tier,originalN,ratio,0);normalBranchMode=false;
extractActiveMesh(candV,candF);RefScore sc=compareToReference(ref,candV,candF,R);
if(candV.size()<bestV.size()&&txGuard(sc,tier)){bestV=candV;bestF=candF;break;}
}
normalBranchMode=false;
verts=bestV;faces=bestF;nV=(int)verts.size();nF=(int)faces.size();
return;
}
runScreenCoreMid();
...
double normalBranchDamage(int ab,int kp,const Vec3&p)const{
vector<int>patch;patch.reserve(vfaces[ab].size()+vfaces[kp].size());
for(int fi:vfaces[ab])if(!fdead[fi])patch.push_back(fi);
for(int fi:vfaces[kp])if(!fdead[fi])patch.push_back(fi);
sort(patch.begin(),patch.end());patch.erase(unique(patch.begin(),patch.end()),patch.end());
double sum=0.0,ws=0.0,mx=0.0;
for(int fi:patch){
    const Face&f=faces[fi];bool ha=false,hk=false;
    for(int k=0;k<3;++k){ha|=f.v[k]==ab;hk|=f.v[k]==kp;}
    if(ha&&hk)continue;
    Vec3 op[3]={verts[f.v[0]],verts[f.v[1]],verts[f.v[2]]};
    Vec3 np[3]={op[0],op[1],op[2]};
    for(int k=0;k<3;++k)if(f.v[k]==ab||f.v[k]==kp)np[k]=p;
    Vec3 on=cross(op[1]-op[0],op[2]-op[0]);
    Vec3 nn=cross(np[1]-np[0],np[2]-np[0]);
    double ol=norm(on),nl=norm(nn);
    if(ol<CParam_MinNormalNorm||nl<CParam_MinNormalNorm)return 10.0;
    double cs=max(-1.0,min(1.0,dot(on,nn)/(ol*nl)));
    double d=max(0.0,1.0-cs),w=1.0;
    if(screenTier&&fi<(int)facePix.size()){
        double imp=screenTier==3?(facePix[fi]+10.0*faceSil[fi]):
                   (facePix[fi]+12.0*faceSil[fi]+2.0*faceWin[fi]);
        w=0.35+min(5.0,imp/(faceImpMean+1e-12));
    }
    sum+=w*d;ws+=w;mx=max(mx,d);
}
return mx+0.30*(ws>0.0?sum/ws:0.0);
}

double normalBranchRank(int ab,int kp,const Vec3&p,double raw)const{
if(!normalBranchMode)return raw;
double nd=normalBranchDamage(ab,kp,p);
double len=norm2(verts[ab]-verts[kp])*invDiag2;
uint64_t h=(uint64_t)(unsigned)min(ab,kp)*0x9E3779B185EBCA87ULL;
h^=(uint64_t)(unsigned)max(ab,kp)*0xC2B2AE3D27D4EB4FULL;
h^=(uint64_t)(unsigned)normalBranchSeed*0x165667B19E3779F9ULL;
h^=h>>29;h*=0x94D049BB133111EBULL;h^=h>>31;
double jitter=double(h&0xffffu)/65535.0-0.5;
return raw+costCap*(0.070*nd+0.000015*len+0.000010*jitter);
}
...
c.position=p;c.rawCost=q.evaluate(p);c.cost=normalBranchRank(ab,kp,p,c.rawCost);return c;
...
auto best=computeBestValid(e.a,e.b);if(!best.valid()||best.rawCost>costCap)continue;
...
if(c.rawCost>costCap){if(normalBranchMode)continue;break;}

if(countCommonFaces(a,b)!=2)continue;

if(countCommonNeighbors(a,b)!=2)continue;

auto best=computeBestValid(a,b);if(!best.valid()||best.rawCost>costCap)continue;
