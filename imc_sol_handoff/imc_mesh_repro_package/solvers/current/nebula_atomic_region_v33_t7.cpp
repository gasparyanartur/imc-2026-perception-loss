...
if(nV>5000&&nV<=50000){if(nV>25000&&nV<=45000){runScreenCoreMid();compact();writeMesh();return;}if(nV<=25000)runTransactionalScreenT2Atomic();else runTransactionalScreenMid();writeMesh();return;}
...
bool tailMode()const{if(nV<=HParam_TailOriginalVertexThreshold)return false;double e=elapsed();double stop=hugeInputMode?19.8:HParam_TailBatchStopElapsed;return e>HParam_TailBatchElapsedStart&&e<stop;}
...
if(accepted>=collapseLimit||elapsed()>tailStop)break;
...
__attribute__((noinline,cold,section(".text.unlikely.atomic_t7"))) void QemSimplifier::runHugeAtomicRegionTxPlain(){
    if(nV<100||elapsed()>HParam_TimeBudgetSeconds-0.28)return;
    const vector<Vec3>baseV=verts;const vector<Face>baseF=faces;
    vector<vector<int>>vf,fa;macroBuildTopology(baseF,nV,vf,fa);
    MacroRegionCandidate best;
    const int samples=min(640,max(128,nV/64));
    const int stride=max(1,nV/samples);
    const double stop=HParam_TimeBudgetSeconds-0.16;
    for(int seed=0;seed<nV&&elapsed()<stop;seed+=stride){
        MacroRegionCandidate c;
        MacroRegionCandidate c2;bool ok=macroCandidate(baseV,baseF,vf,fa,seed,1,c);
        if(elapsed()<stop&&macroCandidate(baseV,baseF,vf,fa,seed,2,c2)&&(!ok||c2.score<c.score)){c=move(c2);ok=true;}
        if(!ok)continue;
        if(c.geom>0.024*hausd||c.normalDamage>0.0075)continue;
        if(c.boundary.size()>20||c.interior.size()>22||c.gain<2)continue;
        if(!best.valid()||c.score<best.score||(c.score==best.score&&c.gain>best.gain))best=move(c);
    }
    if(!best.valid()||elapsed()>HParam_TimeBudgetSeconds-0.10)return;
    ...
    if(elapsed()<HParam_TimeBudgetSeconds-0.05){
